...
export interface ContestationPolicy {
  ...
  // ── Arc 13 additions ──────────────────────────────────────────────────────
  /** Hours the evaluated agent has to contest after publication. Default 72. */
  contest_deadline_hours?: number
  /** Count deadlines only during business hours (requires calendar). */
  use_business_hours?:     boolean
  /** Who resolves contestations: "ai" | "human" | "ai_then_human". */
  reviewer_type?:          'ai' | 'human' | 'ai_then_human'
  /** Gate AI pre-review before publishing result to evaluated agent. */
  pre_review_enabled?:     boolean
  /** Pool ID of the pre-reviewer AI agent. */
  pre_review_agent_pool?:  string | null
}

// ── Arc 13 — CurationSamplingRule ─────────────────────────────────────────────

export type CurationRuleType =
  | 'score_extremes'    // sample when score is very high or very low
  | 'deploy_baseline'   // always sample the first N after a deploy
  | 'score_outlier'     // sample when score deviates significantly from pool avg
  | 'na_excess'         // sample when too many N/A answers
  | 'random_baseline'   // random sampling for ground-truth calibration
  | 'reviewer_signal'   // always sample when pre-reviewer emitted calibration_signal

/** Per-campaign curation rule — controls which evaluations go to curation queue. */
export interface CurationSamplingRule {
  rule_id?:    string   // generated by server; absent on new rules
  campaign_id: string
  rule_type:   CurationRuleType
  enabled:     boolean
  priority:    number   // lower = evaluated first
  params:      CurationRuleParams
}

/** Rule-type-specific params (discriminated union collapsed to Record for simplicity). */
export type CurationRuleParams = {
  // score_extremes
  threshold_low?:    number   // default 0.30 — score below this triggers curation
  threshold_high?:   number   // default 0.90 — score above this triggers curation
  sample_pct?:       number   // fraction of triggered evaluations to curate (0-1)
  // deploy_baseline
  sample_n?:         number   // first N evaluations after a deploy
  // score_outlier
  std_devs?:         number   // default 2.0 — standard deviations from pool avg
  // na_excess
  na_threshold_pct?: number   // default 0.30 — fraction of N/A answers that triggers
  // random_baseline
  rate?:             number   // default 0.05 — random sample rate (0-1)
  // reviewer_signal — no params (always samples)
}
...
// ── Arc 13 — ContestationThread (dimension-level) ─────────────────────────────

/**
 * Visual state of a dimension in the review/contestation cycle.
 * Displayed as a colored indicator in the dimension thread view.
 */
export type DimensionState =
  | 'neutral'       // evaluator AI submitted, no pre-review/contestation yet
  | 'pre_reviewed'  // pre-reviewer AI approved or adjusted without contestation
  | 'contested'     // human agent contested this dimension (round 2 open)
  | 'upheld'        // reviewer upheld original score after contestation
  | 'revised'       // reviewer revised the score after contestation
  | 'timeout'       // round deadline passed without action

/** A single evidence item attached to a thread entry. */
export interface EvidenceEntry {
  stream_entry_id: string
  excerpt:         string   // ≤200 chars from the session stream
  relevance_note:  string
}

/** One entry in a dimension's contestation thread (append-only, immutable). */
export interface ContestationThreadEntry {
  /** Round number — 1=evaluator_ai, 1.5=pre_reviewer_ai, 2=human_agent, 3=reviewer_ai/human */
  round:            number
  author_role:      'evaluator_ai' | 'pre_reviewer_ai' | 'human_agent' | 'reviewer_ai' | 'human_reviewer'
  /** Action taken in this entry, e.g. "approved", "adjusted", "contested", "upheld", "revised". */
  action?:          string | null
  score:            number | null
  justification:    string
...
/** Full thread for a single dimension — all rounds, append-only. */
export interface ContestationThread {
  dimension_id:     string
  /** Resolved from evaluation form — present when loaded via instances endpoint. */
  dimension_label?: string
  current_state:    DimensionState
  original_score:   number
  current_score:    number
  entries:          ContestationThreadEntry[]
}

/** Response from GET /v1/evaluation/instances/{id}/threads */
export interface InstanceThreads {
  instance_id:   string
...
/** Payload for a single dimension decision submitted by the human reviewer. */
export interface HumanDimensionDecision {
  dimension_id:      string
  decision:          'upheld' | 'revised'
  /** Required when decision='revised'. Must differ from the original score. */
  score_override?:   number | null
  /** Required when decision='revised', min 1 entry. */
  evidence_entries?: EvidenceEntry[]
  /** Always required, ≥20 words. */
  justification:     string
}

/** Response from POST /v1/evaluation/instances/{id}/review */
export interface HumanReviewResponse {
  submitted:          boolean
  contestation_state: string
  current_round:      number
  finalized:          boolean
  dimensions_upheld:  string[]
  dimensions_revised: string[]
}

/** Payload for dimension-level contestation by evaluated human agent. */
export interface DimensionContestationPayload {
  /** IDs of the dimensions being contested. */
  dimension_ids: string[]
  /** Map of dimension_id → reason text. */
  reasons:       Record<string, string>
  /** Anti-replay: must match result.current_round. */
  round:         number
}

/** Response from POST /v1/evaluation/instances/{id}/contest */
export interface DimensionContestationResponse {
  submitted:            boolean
