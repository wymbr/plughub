...
class WebhookAdapter(ChannelAdapter):
    ...
    async def handle_resume(
        ...
        identity_clearance: str | None = None,
    ) -> str | None:
        """
        Resolve a resume_token to a session_id and publish a session_resumed event.

        PID-13 — `identity_clearance` diz por que ESTE chamador pode retomar uma pendência
        com `resume_requires`: `"session_evidence"` (o mcp-server julgou a evidência da
        sessão chamadora e atestou com a credencial de serviço) ou `"timeout"` (o scanner
        de prazo, em processo). O principal humano verificado (`approver`) é a terceira
        via. Nunca vem do payload: o corpo da rota interna é repassado cru.
    ...
    async def _enforce_resume_requirement(
        ...
        actor:              str,
        approver:           dict[str, Any] | None,
        identity_clearance: str | None,
    ) -> None:
        """PID-13 — quem pode retomar uma pendência que declara `resume_requires`.

        A PID-06 fez o `pending_workflow_get` reter o token de quem não provou; mas o
        token em mãos (link, D10, ou vazado) chegava ao resume por SETE portas, uma delas
        pública e anônima, e nenhuma conferia nada. A regra mora aqui porque todas passam
        por aqui. Três atores passam, e só eles (decisão do dono):

          - `identity_clearance="session_evidence"` — o mcp-server julgou a evidência da
            sessão chamadora contra esta exigência e o atestou com credencial de serviço;
          - `approver` — principal HUMANO verificado (aprovador, supervisor). A exigência é
            sobre o CLIENTE retomar, não sobre o operador decidir;
          - `identity_clearance="timeout"` — o scanner de prazo, em processo.

        Pendência sem exigência (sem registro, sem o campo, ou `[]`) passa como sempre.
        Registro ilegível com o campo é recusa: dúvida sobre a exigência não libera.
        """
        meta = await self._read_resume_meta(tenant_id, resume_token)
        exigencia = (meta or {}).get("resume_requires")
        if not exigencia:
            return
        if identity_clearance in ("session_evidence", "timeout") or approver is not None:
            logger.info(
                "PID-13: retomada com exigência %s liberada token=%s session=%s via %s",
                exigencia, resume_token, session_id,
                identity_clearance or f"approver:{approver.get('decided_by') if approver else '?'}",
            )
            return
        logger.warning(
            "PID-13 403: token=%s (session=%s tenant=%s) exige %s e o chamador %s não "
            "apresentou prova — nem evidência atestada pelo mcp-server, nem principal humano",
            resume_token, session_id, tenant_id, exigencia, actor,
        )
        raise PermissionError(
            "resume_requires_unproven: this pending requires identity proof "
            f"({', '.join(map(str, exigencia))}) from the resuming session"
        )
    ...
    async def _write_resume_meta(
        ...
        PID-13 — `resume_requires` mora AQUI, no registro do token, porque é o que o
        `handle_resume` lê por token, venha a retomada por qual porta vier. Com exigência
        a escrita deixa de ser best-effort: sem o registro a exigência some e o token
        retoma sem prova — a falha SOBE, e a pendência não nasce.
        """
        ...
        if resume_requires is not None:
            registro["resume_requires"] = list(resume_requires)
        try:
            await self._redis.set(
            ...
            if resume_requires:
                logger.error(
                    "resume_meta: registro do token=%s (tenant=%s) NÃO gravado e ele "
                    "carrega exigência %s — recusando criar a pendência: %s",
                    resume_token, tenant_id, resume_requires, exc,
                )
                raise
    ...
    async def _handle_resume_locked(
        ...
        # ── PID-13 — exigência de identidade da retomada, em TODAS as portas ────
        await self._enforce_resume_requirement(
            tenant_id, resume_token, session_id, actor, approver, identity_clearance,
        )
    ...
    async def handle_collect(
        ...
        *,
        ...
        await self._write_resume_meta(
            ...
            ttl_s=ttl_s,
            resume_requires=resume_requires,   # PID-13
        )
    ...
    async def handle_delegate_conference(
        ...
        try:
            ...
            if resume_requires:
                # PID-13 — sem o registro a exigência não existe na retomada: falha
                # FECHADO (o engine segue o on_timeout) em vez de parquear sem prova.
                raise
            ...
            for raw_token, raw_value in entries.items():
                ...
                try:
                    await self.handle_resume(
                        ...
                        identity_clearance="timeout",   # PID-13 — o prazo não prova identidade, e não precisa
                    )
