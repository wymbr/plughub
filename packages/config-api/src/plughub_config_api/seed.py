...
_SEED: list[tuple[str, str, object, str]] = [
    ...
    #
    # ⚠️ **O valor MUDOU DE CASA em 2026-09-02 (ALW-02 passo 3)**: mora em
    # `plughub_contextstore.default_map`, e este seed o IMPORTA. Continua sendo cópia
    # gerada de `DEFAULT_CONTEXT_MAP` em `@plughub/schemas/context-map.ts`, que segue
    # sendo a AUTORIDADE — nunca digitado à mão.
    #
    # O número de cópias não mudou (eram TS + esta; continuam TS + a da lib). O que
    # mudou é que o carregador Python do mapa precisa do MESMO fallback que o TS tem,
    # e mantê-lo aqui dentro faria uma lib depender de um serviço. Sem o fallback
    # compartilhado, as duas metades divergiriam durante uma queda do config-api —
    # e o ruído cairia dentro do `unknown`, que é o número que autoriza a V4.
    #
    # A divergência contra a TS não é confiada à disciplina:
    # `infra/test/probe_context_map_audit.sh` compara a config VIVA com a TS e REPROVA.
    #
