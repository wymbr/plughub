...
// e por isso o tool nasceu sem política nenhuma (ADR ALLOWLIST §1.5).
import {
  getContextMaskingConfig,
...
//
// Mudaram-se de casa em 2026-08-26 → `lib/context-masking.ts`. Aqui ficou apenas
// `applyContextMaskingDynamic`, que é a política da porta HUMANA (endpoint da
// Console) e carrega o portão de namespace do pool. Ver ADR §D4.
//
// Nada de código foi reescrito na mudança: `ruleSpecificity`,
// `resolveContextMaskingRule`, `applyMaskingTypeToValue`, `getContextMaskingConfig`
// e `maskContextForPersistence` foram para lá **com os seus comentários inteiros**,
// que são a documentação de por que o glob de sufixo existe e de por que o snapshot
// durável é grau operator.
...
// `maskContextForPersistence` (+ `PersistenceSnapshot`) mudou-se para
// `lib/context-masking.ts` em 2026-08-26, com toda a sua documentação — e ganhou um
// SEGUNDO consumidor lá: o tool MCP `supervisor_state`, que devolvia o hash cru.
// A razão de eles caberem na mesma função está no cabeçalho dela: nenhum dos dois
// tem visualizador com papel.
