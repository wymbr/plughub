...
interface CloseModalProps {
  ...
  onCancel: () => void;
  /** Pre-fills the "Status do problema" field (e.g. on client disconnect). */
  defaultIssueStatus?: string;
  /** Pre-selects the outcome (e.g. "abandoned" on client disconnect). */
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto p-6 flex flex-col gap-4">
        ...
        {/* Handoff reason — only for non-resolved */}
        {outcome !== "resolved" && (
          <div className="flex flex-col gap-1">
            <label className="text-xs font-medium text-gray-600">
              Motivo do handoff
