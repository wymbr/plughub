"""
main.py
Scheduler / Agenda API FastAPI application.

Startup sequence:
  1. Create asyncpg connection pool (with retry)
  2. Ensure DB schema (CREATE TABLE IF NOT EXISTS) — Camada 2 (fonte de verdade)
  3. (task 3) Re-hydrate Camada 1 (Redis sorted-set) + start the poller
  4. Serve requests
"""
...
# Ensure plughub.* INFO logs reach stdout (uvicorn does not configure custom loggers).
_plughub_logger = logging.getLogger("plughub")
if not _plughub_logger.handlers:
    _h = logging.StreamHandler(sys.stdout)
    _h.setFormatter(logging.Formatter("%(levelname)s: %(name)s: %(message)s"))
    _plughub_logger.addHandler(_h)
_plughub_logger.setLevel(logging.INFO)
_plughub_logger.propagate = False

logger = logging.getLogger("plughub.scheduler.api")
...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    dispatcher = Dispatcher(pool, settings, app.state.calendar_client, scheduler)
    scheduler.set_fire_callback(dispatcher.on_fire)
    app.state.scheduler  = scheduler
    app.state.dispatcher = dispatcher
    try:
        await scheduler.start()
    except Exception as exc:
        logger.warning("scheduler start failed — timers inactive: %s", exc)
...
app = FastAPI(
    title       = "PlugHub Scheduler API",
    version     = "1.0.0",
    description = (
        "Agenda / scheduling for the PlugHub Platform. A domain-agnostic scheduled "
        "resource that fires a POOL via webhook (Arc 19) at a configured time/mode. "
        "Execution status belongs to the fired session (by reference)."
    ),
    lifespan    = lifespan,
)
...
def run() -> None:
    ...
    settings = get_settings()
    uvicorn.run(
        "plughub_scheduler_api.main:app",
        host    = settings.host,
        port    = settings.port,
