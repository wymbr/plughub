...
plughub/
  ...
  docs/
    ...
    adr/
      ...
      adr-tree-return-continuation.md ← **a folha DEVOLVE**: execucao de agente e ETAPA, nao desfecho. Folha de verdade passa a ser so `escalate` e `complete`. A folha ganha UM campo (`on_return: <question_id>`) apontando a question que o CHAMADOR executa no retorno — **ponteiro, nunca switch**: um switch por codigo de retorno seria branching no JSON e emendaria o invariante *“nodes order IS the flow — no conditional next”*; o ponteiro e aresta unica e nao emenda nada, e por isso `leafPaths`/`is_tree`/achatamento em canal **nao mudam** (a folha continua folha). Questions identificadas com `main` como entrada (o `id` ja existe no schema). Continuacao e RAIZ PROPRIA no `category_path`, o que entrega a tag de origem da demanda por topologia. ⚠️ Emenda a D1 do contrato: a titularidade e **parcial** — o chamado pode encerrar, e e a **finalizacao por ERRO** que torna isso inevitavel, nao uma concessao. Exige cancelar `resume_token` pendente no fechamento, e teto de ciclo por CONTADOR (o `session_timeout` e por inatividade e ciclo ativo nunca bate nele). Decisoes D1–D9, fases R1–R5 — proposto
      ...
      adr-identity-door-evidence.md ← PORTA DE IDENTIDADE: identificar ≠ autorizar — a plataforma resolve, executa mecanismos de prova e REGISTRA evidência; a régua ("quanto basta") é da aplicação, declarada como CONJUNTO de mecanismos, sem score. Porta = pool + runner de plataforma; evidência gravada por QUEM VERIFICA, com a sessão que provou; o piso mora no dispensador de token (`session_token` assinado), não na medição. Emenda `adr-identity-channel-possession` (#4 muda de dono) e absorve §6–7 do `identity-resolver-nivel-b-spec`. Fatias `PID-01..12` + `IDN-06..08` — proposto
```
