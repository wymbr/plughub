...
| **Opt-out global** | elegibilidade (veto) | **3b ✅ via API** — `contact_eligibility_check` consulta o cadastro (`identity.customers.attributes.do_not_contact` `{all?, channels?}`) via channel-gateway `GET …/identity/customers/{id}`; **maior precedência** (antes de calendar/fadiga), veta `opt_out` salvo `campaign.transactional`. `mailing_unsubscribe scope=global` escreve o atributo. Degrada→ALLOW barulhento em erro. Smoke `smoke_outbound_fase3b.sh` — **validado 2026-08-20** (escrito em 07-21 e nunca rodado até então). O gate ganhou na validação as duas metades que faltavam: **testemunha** (cliente sem opt-out, MESMA campanha e MESMO canal → `allowed`) e **opt-out por canal**; sem a testemunha um portão que vetasse TODO cliente ficava verde, porque o único caso permitido era `transactional`, que pula o portão antes de consultar o cadastro (`db.py:863`). Ramo **não** exercitado e declarado na saída: a degradação com o identity fora do ar. |
