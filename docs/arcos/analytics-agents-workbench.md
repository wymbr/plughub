...
| **F10.3b — Cutover F5 (unificação Opção 2)** *(futura)* | hook de NPS de segmento chama `survey_record(grain=segment, segment_id, agent_key)` (expor `$.segment_id` + bridge escreve `session.surveyed_*` via `@ctx`); `_compare_nps_lens` migra para `session_signal` (grão segment); aposenta `seg_signal`/`segments.nps_score`. **Resolve a duplicação** NPS/CSAT entre `segments` e `session_signal`. | bancada lê tudo de `session_signal`; F5 retirada |
