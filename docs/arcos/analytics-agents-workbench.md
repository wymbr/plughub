...
|---|---|---|
| **F1 — Espinha: outcome real** | enums reconciliados (§13.2); `complete` dinâmico (engine); `agente_wrapup_v1` (complete dinâmico + `issue_status` + `handoff_reason`); bridge propaga ao `primary` humano + fia `close_reason`/`handoff_reason`/`issue_status` | ClickHouse mostra outcomes humanos reais/variados + close_reason + issue_status; resolution/escalation por humano com sentido |
| **F2 — Atribuição da qualidade** | join `evaluation_results → segments` por `session_id` (ou desnormalização no ingest) → nota por `agent_key`+`pool_id` | nota de qualidade atribuível por agente (humano e IA) |
| **F3 — Endpoint `/reports/agents/compare`** | multi-entidade × multi-lente; lê `segment.outcome` uniforme; filtros sintéticos; "média dos agentes" + N; lentes resolution/escalation, sessions/AHT, availability (humano), quality | endpoint retorna séries comparáveis por lente |
