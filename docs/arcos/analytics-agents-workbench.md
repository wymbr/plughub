...
| **F8 — Lente `quality_criteria` (eixo dimensão) + radar no detalhe** | **decisões (2026-06-09)**: eixo = **dimensão** (`EvaluationDimensionDef`, não critério cru — critério vira drill-down no detalhe); comparável **só dentro do mesmo formulário** (dimensões diferem entre forms → desabilita/avisa mistura cross-form, igual à regra de domínio das outras lentes). **Dependência de ingest**: per-dimensão NÃO está no ClickHouse (`evaluation_results.overall_score` só) → criar `analytics.evaluation_dimension_scores` (ou nested) alimentado de `EvaluationResult.dimensions[]`/Arc13 `final_scores_by_dimension[]`. Sub-passos: ingest → query (`quality_criteria` sai de `_COMPARE_LENSES_PENDING`, escopado a 1 form) → UI **heatmap** agente×dimensão (cor=nota, guard de form) → **radar** das dimensões no detalhe do agente (F4.4) | heatmap de competências comparável + perfil radar por agente |
