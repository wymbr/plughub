...
| **C1 — Aba `Cliente` (360)** *(nova, ADR §D1/§D3/§D4)* | **ficha** do cliente: **C1a cadastro manual** ✅ (buscar/criar/atacar âncora — §7, net-new = `GET /identity/customers/search`) + **C1b 360 agregado** ✅ (resumo de contatos + **quality** Arc 6 `evaluation_finalized` Oficial + **survey** `session_signal` — `GET /reports/customers/{id}/360`, join por `session_id` ao conjunto de sessões do cliente, `origin=live`, fail-soft por bloco; jornadas ficam só no Histórico/HJ). i18n en+pt-BR. | corrigir identificação errada; ver quality+survey do cliente num lugar |
...
| **H5 — visão por cliente no Analytics** ✅ | página `/analise/customers` (`AnaliseClientesPage`, grupo Analytics, ABAC `contacts.visualizar`): **seletor de cliente** (busca `/identity/customers/search`) → `Customer360Card` (extraído p/ reuso Console↔Analytics) + `HistoricoTab` (jornadas/contatos/busca/drill). Deep-link `?customer=`. **Reuso 100%** — nenhum backend novo. **GIN(tsvector) adiado** (backlog — busca segue ClickHouse substring, H2). | supervisão consulta histórico/jornadas/quality/survey de um cliente fora do atendimento |
