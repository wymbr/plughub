...
Diferenciais secundários mas relevantes: **Skill Flow com 13 tipos de step** incluindo `suspend`, `collect`, `escalate` com timers em horas úteis, coleta assíncrona multicanal e `begin_transaction`/`end_transaction` para captura mascarada de dados sensíveis (PINs, senhas) em bloco atômico — nenhum CCaaS analisado tem um motor de fluxo comparável em expressividade declarativa. **Session Replayer com comparison mode** permite diff entre duas execuções, útil para avaliação de agentes IA pré-produção. **Operator Console com heatmap de sentimento por pool** é uma UI de supervisão comparável ao Enlighten Actions da NICE, mas sem o lock-in CXone — e com um catálogo de dashboards configuráveis (DisplayTool registry com 14 endpoints, incluindo cards de Journey) que elimina a necessidade de ferramenta de BI separada para operação. **Agent Groups com escopo de supervisor por turno**: supervisores recebem no JWT um escopo filtrado pelos grupos e turnos ativos (`supervised_agent_types[]`), com granularidade que o Enlighten Actions não oferece sem customização. **Journey — processo multi-sessão**: unidade de serviço acima da sessão individual que agrupa todos os contatos de um mesmo processo de atendimento — comparable ao conceito de "case" em Salesforce Service Cloud, mas nativo ao roteador e sem exigir CRM.
...
**Para Head de Contact Center / Customer Experience:** *"Os CCaaS te dão roteamento maduro mas agente IA engessado. Os builders hyperscale te dão agente IA potente mas roteamento pobre e lock-in de stack. PlugHub te dá um motor de fluxo declarativo (13 tipos de step, timers em horas úteis, coleta assíncrona multicanal, captura mascarada de dados sensíveis) acoplado a um roteador multicritério, com heatmap de sentimento, session replay e Journey — processos de atendimento que transcendem uma sessão individual, agrupando todo o histórico de contatos de um mesmo caso sem precisar de CRM. Supervisor com escopo por grupo e turno, direto no JWT, sem configuração extra."*
