...
Agentes de IA + Agentes Humanos com MCP · Versão 22.0 · Março 2026

  --------------------- ------------ ------------------------------------- ---------------------------------------------------------------------
  **Versão**            **Data**     **Canais**                            **Status**
  22.0 --- Greenfield   16/03/2026   WhatsApp · SMS · Chat · Email · Voz   Revisão --- Bring-Your-Agent, Assistência ao Humano, sem faseamento
  --------------------- ------------ ------------------------------------- ---------------------------------------------------------------------
...
**Discovery & Management.** Todos os agentes --- internos, externos ou de terceiros, em qualquer framework --- são registrados num repositório central com gestão de ciclo de vida. O SDK de Integraão certifica compatibilidade antes do deploy e garante portabilidade: o agente pode ser desconectado e reconectado a qualquer sistema MCP sem modificação.

**Evaluation & Simulation.** Cada atendimento é avaliado por um Agente Avaliador com templates configuráveis por pool --- os mesmos critérios aplicados a agentes de IA de qualquer framework e a agentes humanos. Um Agente Revisor opera sobre as avaliações, identifica inconsistências e retroalimenta a calibração ao longo do tempo. Qualidade não é um add-on --- é parte da operação desde o primeiro dia.

**Governance & Security.** Workspaces isolados por namespace garantem que dados de departamentos e clientes distintos nunca se cruzam. O circuit breaker isola falhas de sistemas externos sem interromper a operação. Autenticação e conformidade regulatória (LGPD/GDPR) são delegadas ao sistema de identidade do operador via MCP --- a plataforma entrega os sinais, o operador mantém o controle.

**Workflow Automation.** O PlugHub suporta dois modelos de orquestração, adequados à complexidade do processo. Fluxos mais simples são declarados nativamente como grafos de orquestração multi-agente --- steps de task, choice, catch, escalate e complete --- sem hardcode em nenhum agente individual e sem dependência de ferramenta externa. Fluxos mais complexos, já modelados em sistemas de BPM de mercado como Camunda, Pega ou IBM BPM, integram o PlugHub como executor: o processo externo define contexto, sequência e condições de ramificação, e recebe o outcome de volta --- sem precisar ser reconstruído dentro da plataforma. Em ambos os modelos, o Rules Engine decide escalações e prioridades por tenant, e o módulo outbound fecha o ciclo proativo com contato ativo, pendência oportunista e entrega com contexto pelo Notification Agent.

**Context & Memory.** Estado de sessão compartilhado em tempo real entre todos os componentes, sem coordenação explícita. Bases de Conhecimento Verticais acumulam conhecimento setorial --- regulações, terminologia, processos --- refinado continuamente pelo Knowledge Base Analytics a partir de todas as interações em produção. Um ativo que cresce com o uso e não se perde quando agentes ou modelos são substituídos.

**O diferencial:**

Você não está comprando software de atendimento. Está operando um hub --- com os agentes que já tem, sob suas regras de governança, com avaliação uniforme entre IA e humanos, e com dados que pertencem a você. O time-to-operation se mede em semanas porque a infraestrutura já está pronta. O lock-in não existe porque o contrato de integração é aberto.
...
**2.1 Canais Suportados**
  ...
  **Canal**                    **Latência Tolerada**          **Características**
  ...
  WebRTC --- Voz/Texto/Vídeo   \< 3.000ms (estabelecimento)   Canal unificado com capacidades adaptativas. Agentes de IA atendem voz e texto via WebRTC sem alteração de pipeline. Vídeo bidirecional exclusivo para agentes humanos. Degradação graçiosa automática: vídeo → voz → texto na mesma sessão.
  ---------------------------- ------------------------------ ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
Um Agente Revisor opera sobre as avaliações produzidas --- não sobre as transcrições originais --- e identifica inconsistências, casos limitórfes e divergências. Classifica cada avaliação em aprovada automaticamente ou encaminhada para revisão humana. O supervisor recebe apenas o que o revisor sinalizou, com contexto completo para decidir. Aprovações e ajustes humanos retroalimentam a calibração do revisor ao longo do tempo --- a esteira fica mais precisa com o uso.
...
O operador define domínios de conhecimento --- Produto, Cobrança, Regulatório, Procedimentos Internos, por exemplo. Cada domínio é uma base vetorial independente, acessível exclusivamente pelos tipos de agente autorizados. Um agente de cobrança não consulta a base de produto --- e vice-versa. O campo valid\_until em cada entrada garante expiração automática de conhecimento com prazo --- promoções, regulações temporárias, campanhas --- sem intervenção manual.
...
**Oito tipos de step cobrem qualquer fluxo de negócio real:**
  ...
  **Step**   **O que faz**
  ...
  escalate   Deriva para pool humano ou especializado via Rules Engine, com contexto completo do pipeline
...
**2.8b Roteamento Inteligente**

A maioria das plataformas de atendimento roteia por disponibilidade --- o próximo agente livre recebe a próxima conversa. O PlugHub implementa um modelo diferente: cada decisão de alocação considera múltiplas dimensões simultaneamente, com pesos configurados pelo operador para refletir as prioridades do seu negócio.

As dimensões consideradas na decisão são configuráveis pelo operador --- perfil do agente, tempo de espera na fila, SLA do pool, índices de qualidade do agente gerados pelo sistema, entre outros. Cada pool define quais dimensões importam e com qual peso, refletindo as prioridades do seu negócio.

**Dois cenários de decisão com lógicas distintas**

O sistema separa dois problemas que plataformas convencionais tratam como um só:

  ---------------------------------------- ------------------------------------------------- ----------------------------------------------------------------------------------------
  **Cenário**                              **Pergunta**                                      **Critério dominante**
  Contato chega, agentes disponíveis       Qual agente é o melhor match para este contato?   Compatibilidade de perfil --- o agente tem as competências que este atendimento exige?
  Agente fica disponível, fila não vazia   Qual contato este agente vai atender primeiro?    Prioridade efetiva na fila --- tier do cliente + tempo de espera relativo ao SLA
  ---------------------------------------- ------------------------------------------------- ----------------------------------------------------------------------------------------

Para garantir que nenhum contato fique indefinidamente na fila enquanto chegam contatos mais prioritários, o sistema aplica priority aging: a prioridade de qualquer contato cresce automaticamente com o tempo de espera em relação ao SLA do pool. O operador configura a velocidade desse crescimento --- sem intervenção manual, o sistema garante atendimento eventual para todos.

**2.8c Automação de Processos Manuais Repetitivos**

Muitas operações não têm um BPM implantado --- têm analistas que todo dia abrem vários sistemas, coletam informações dispersas, consolidam numa planilha e decidem o que priorizar. É trabalho manual, repetitivo, sujeito a erro e invisível para a gestão. O agente orquestrador nativo resolve exatamente esse cenário: automatiza a consolidação de informações e a tomada de decisão estruturada sem precisar modelar um processo completo antes de começar.

**Coexistência com BPM, não substituição**

O agente orquestrador não é um BPM e não pretende ser. Processos com governança regulatória, aprovações em múltiplas etapas e duração de dias ou semanas pertencem ao BPM. O que o orquestrador resolve é diferente: é a automação do trabalho de consolidação que hoje um humano faz manualmente porque o processo ainda não foi mapeado ou não justifica um projeto de BPM. Onde existe um BPM, os dois convivem --- o BPM orquestra o processo de negócio de ponta a ponta e aciona o PlugHub como uma etapa de atendimento. Onde não existe BPM, o orquestrador assume o papel de coordenar as consultas e ações nos sistemas envolvidos.

**Acionamento por calendário externo**

O mesmo orquestrador que atende um cliente reagindo a um contato pode ser acionado por um agendador externo para executar tarefas programadas. Um cron job, o próprio BPM da organização, ou qualquer scheduler aciona o PlugHub num horário definido --- toda manhã, toda hora, toda semana --- passando os parâmetros da execução. O orquestrador executa o fluxo declarado na skill, consulta os sistemas necessários via MCP Servers, toma decisões e registra o resultado. Sem cliente na conversa, sem atendimento reativo --- é automação de processo.

  -------------------------------------- -------------------------------------------------- -------------------------------------------------------------
  **Tipo de acionamento**                **Quando usar**                                    **Exemplo**
  Reativo --- cliente entra em contato   Atendimento a demanda do cliente                   Cliente solicita portabilidade via WhatsApp
  Agendado --- scheduler externo         Processo repetitivo programado                     Todo dia às 8h: consolidar OS abertas há mais de 5 dias
  Acionado por BPM                       Etapa de atendimento dentro de um processo maior   BPM de crédito aciona atendimento para coleta de documentos
  -------------------------------------- -------------------------------------------------- -------------------------------------------------------------

**O que se automatiza na prática**

Qualquer tarefa que hoje um analista executa abrindo múltiplos sistemas pode ser declarada como um fluxo de orquestração. O agente consulta cada sistema via MCP Server, consolida os resultados, aplica raciocínio estruturado para classificar ou priorizar, e notifica ou registra o resultado. Exemplos comuns: consolidação de status de ordens de serviço espalhadas em sistemas diferentes, relatórios de inadimplência com cruzamento de dados de CRM e cobrança, triagem de tickets por critério de urgência, verificação periódica de conformidade de cadastros, e geração de agendas de visita com base em localização e prioridade.

O fluxo é declarado na skill de orquestração --- sem código, sem projeto de TI de longa duração. O operador conecta os MCP Servers dos sistemas existentes, declara os passos do fluxo e agenda a execução. A rastreabilidade é automática: cada execução registra o que consultou, o que decidiu e o que fez --- auditável sem instrumentação adicional.
...
A metáfora da planta-padrão é útil aqui: concorrentes como Genesys e NICE vendem máquinas específicas --- agentes prontos para domínios pré-definidos, integrados na sua planta fechada. Esta plataforma entrega a planta inteira com pontos de conexão abertos onde qualquer máquina pode ser instalada, incluindo as que o operador já tem. A diferença não é de funcionalidade --- é de modelo: um vendor SaaS controla o que pode ser instalado na sua planta e cobra por isso. Esta plataforma entrega a planta e deixa o controle de instalação com o operador.
  ...
  **Plataforma**      **BPM Aberto**                **MCP Exposto p/ Consumo**         **Multi-Tenant Operável pelo Cliente**   **Base de Conhecimento Multi-Domínio**        **Bring-Your-Agent**
  ...
  Esta Plataforma     SIM --- agnóstico de engine   SIM --- tools nativas MCP          SIM --- híbrido namespace+cluster        SIM --- por tipo de agente com valid\_until   SIM --- SDK de integração com certificação, portabilidade e regeneração de agentes proprietários
  ------------------- ----------------------------- ---------------------------------- ---------------------------------------- --------------------------------------------- --------------------------------------------------------------------------------------------------
...
**3.2 Vantagem do Operador SaaS vs. Revenda de Plataforma**
  ...
  ------------------------ -------------------------------------------------------------- -----------------------------------------------------------------------------------------------------------------------------
  **Dimensão Comercial**   **Genesys / NICE (vendor SaaS)**                               **Esta Plataforma (SaaS próprio)**
  ...
  Modelo de IA             Vendor controla                                                Operador controla --- model\_profile configurável por tenant
...
O argumento central do modelo não é TCO de infraestrutura --- é a margem que o operador captura ao controlar a precificação para seus clientes finais. A comparação relevante é entre revender capacidade de um vendor (margem zero) e operar plataforma própria (margem crescente com escala).
  ...
  --------------------------------- ------------------------------------------ -------------------------------------
  **Métrica**                       **Revenda Genesys / NICE**                 **Esta Plataforma (SaaS próprio)**
...
**4.2 Projeção de Receita e Margem por Escala de Operador**
  ...
  ------------------------- --------------------------------- ------------------------------------------------
  **Métrica**               **Genesys (por tenant)**          **Esta Plataforma (custo/tenant, 10 tenants)**
...
**5.3 O Que Não Muda --- Fundamentos de Longa Duração**
  ...
  **Elemento**                                                                                 **Por Que Resiste ao Tempo**
  ...
  Arquitetura de planta-padrão --- separação entre infraestrutura e o que o operador instala   O conceito de planta não depende de nenhum protocolo ou modelo específico. A separação é estrutural e válida independente do que for instalado.
