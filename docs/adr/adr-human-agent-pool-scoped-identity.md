...
> Status: **aceito · arco completo** · 2026-07-27/28 · **F1 + F2 + F2b + F3 + F4 + F5 ✅ implementadas**
> (ver `CHANGELOG.md`)
...
| **B6** | **@mention no pool errado.** `tools/session.ts:184` resolve `mentionable_pools` pelo `pool_id` **global** da instância, não pelo pool da conversa. *(Hoje o bug é mascarado: é um `HGET` contra uma chave JSON string → WRONGTYPE engolido por `catch {}` → mentions caem em silêncio. Corrigir o tipo sem corrigir o modelo troca um no-op silencioso por um convite ao pool errado.)* **Emenda as-built (F5): eram DUAS implementações** — a do WS de agente (Console) já resolvia pelo pool da conexão e sempre funcionou; o B6 vivia só na tool MCP `message_send`, sem caminho vivo. E havia uma **terceira metade**: o gate de `role` acima dele falhava **aberto** (mesma colisão de chave), de modo que consertar B6 sozinho violaria o invariante *"IA nunca emite @mention"*. Ver `CHANGELOG.md` § F5. | mcp-server |
...
| **F5 — higiene + rede de proteção** ✅ | `pool_id` singular **removido** do `AgentInstance` (modelo, `_upsert_instance`, `registerHumanAgent` e os 2 leitores de fallback no `server.ts`); docstring de `update_instance_meta` corrigido (dizia *"pools and agent_type_id do not change during the instance lifetime"* — falso nos dois termos para humanos, e era a premissa de `remove_conversation` e `crash_detector`); **B6** fechado por **implementação única** (`lib/mention-routing.ts`, pool do remetente como PARÂMETRO) + `lib/routing-ref.ts` (leitor do fato por-(sessão, instância), tolerante ao formato legado) + gate de `role` que passa a **falhar fechado**. Fallout da F4 fechado junto (`supervisor.ts`, `bpm.ts` liam o `snapshot` removido). | **B6**, regressão futura | vários, baixo risco |
