...
| **T6** | ✅ **FEITA em 2026-08-29.** As 6 migradas: `senha`/`codigo_2fa` (×4) e o `pin_input` → **`credential`**; o `cvv` do `dialog_limite_solicitacao` → **`card_cvv`**. Exigiu **dois tipos novos** no catálogo, porque nenhum dos 8 servia — e a separação entre eles é a D1 em ato: política idêntica (ninguém vê, nunca persiste), **classe diferente** (`credencial` × `financeiro`) ⇒ dois tipos. `card_cvv` **não é** `credit_card`: aquele declara `display_partial` com 4 dígitos, e num CVV de 3 isso exibiria quase o valor inteiro. Migrado na AUTORIDADE (`PUT` + `x-skill-publish`) e **re-promovido nos slots**, porque o bridge executa o snapshot, não o `skill.flow`. Provado ao vivo: `masked_types: {senha: credential, codigo_2fa: credential}` na transcrição durável, e dali a classe LGPD alcançável por join | sim |
| **T7-A** | ✅ **FEITA em 2026-08-29** — a ESCRITA fecha. `MaskedDeclarationSchema` deixou de aceitar `true`: **422** no `PUT /v1/skills`. **`false` FICA**, e a assimetria é decidida: ele tem **zero usos** no parque e mesmo assim não sai, porque é a única forma de dizer *"este campo NÃO é mascarado mesmo que o step mascare"* — remover `false` tiraria uma CAPACIDADE, remover `true` remove uma FORMA LEGADA. Ausência de uso ≠ ausência de propósito. Consequência que o build cobrou: **6 fixtures de teste** escreviam `masked: true` e pararam de compilar — migradas. **Reversível** | sim |
| **T7-B** | Remover a tolerância do RUNTIME (`if (d === true)` em `normalizeDecl`). **NÃO AUTORIZADA**, e a razão foi medida — inclusive contra o que eu havia escrito: o snapshot **não passa por Zod na execução** (o `skill-flow-service` é wrapper fino) e o `POST /rollback` **só troca linhas de slot**, sem revalidar. Logo o dano não é de *parse*: é `d.trim()` sobre um booleano — **TypeError no meio de um atendimento**. **Provado ao contrário em 2026-08-29:** com o schema já fechado, um rollback real de `auth_form_ia` para o snapshot pré-T6 **executou e mascarou**, gravando `masked_types: {senha: opaque}` — porque a tolerância estava lá. Pré-condição: nenhum snapshot ALCANÇÁVEL (inclusive `previous`, **por tenant**) com a forma anônima, **ou** aceitar por escrito perder esses rollbacks | **não** |
...
> Este bloco vivia como **uma linha** do índice `docs/` no `CLAUDE.md`, onde ocupava 5685 bytes.
