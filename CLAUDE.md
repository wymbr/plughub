...
ClickHouse tables: `analytics.segments` (`ReplacingMergeTree` ORDER BY `(tenant_id, session_id, segment_id)`), `analytics.session_timeline` (enriched with `segment_id`). **MV sobre `segments` conta VERSÕES, não segmentos** (RMT funde na tabela, nunca na MV): as duas que havia saíram por isso — `mv_agent_performance_daily` (APF-01) e `mv_segment_summary` (APF-02) — e os leitores agregam `segments FINAL`. **Transferência = `close_reason = 'agent_transfer'`**, nunca `outcome` (TRF-01). Endpoints: `GET /reports/segments`, `GET /reports/agents/performance`, `GET /reports/agent-performance/daily`, `GET /reports/sessions/complexity`.
