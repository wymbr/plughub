...
### Detach de hooks de finalização + Pull direcionado + ACW *(desenho fechado 2026-07-23; Camadas A ✅ + B ✅ + D ✅ 2026-07-24; E1 ✅ (Forma A aposentada); E2 wrap-up ✅ (detached + wiring, 2026-07-24/27); **wrap-up UNIFICADO** ✅ Phase 0+1 (2026-07-27) — inline=auto-atendimento sobre a MESMA máquina detached, `acw_pending`/`acw_gate` REVERTIDOS (modelo errado: bloqueavam a instância inteira; capacidade = 1 vaga pelo semáforo `claim_instance`, os dois modos); faltam Phase 2 (hand-off da vaga no close inline, sem corrida) + Phase 3 (remover inline antigo `wrapup_ia`+`wrap_up_pending`) + F validação; ver CHANGELOG)*
