...
- **Never expose the `/v1/*` prefix of the channel-gateway at the edge.** A porta externa de webhook é
  `POST /channel/webhook/{identifier}` (serve só `origin=external`); `/v1/*` é rede interna. Não é
  preferência: dentro dele vive `POST /v1/channels/webhook/pool/{pool_id}`, **anônima por construção** —
  não passa pelo registro (ADR §7.6.1) e por isso **não tem onde pendurar credencial**. Publicar o prefixo
  torna disparável por qualquer um TODO pool webhook do tenant, inclusive os que promovem deploy e
  contatam clientes, e nenhum `auth_required` muda isso. O mesmo prefixo abriga RPC interno com nome
  infeliz (`…/delegate`, `…/collect`, `…/resume/{token}`, `…/identity/*`). **Nenhum teste alcança a
  topologia** — o probe de inventário declara que não a verifica. Ver
  [`docs/guias/webhook-patterns.md`](docs/guias/webhook-patterns.md) § Exposição na borda
