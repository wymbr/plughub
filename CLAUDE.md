...
### Journey (retorno) — modelo de 3 níveis *(J1–J3 ✅ implementados 2026-07-09; J4/J5 pendentes)*
- **Estado:** J1 (espinha `root_session_id` + propagação), J2 (`/reports/journeys` + Vista Processos + drill 3 níveis), J3 (`journey_merge` + `journey.merges` + `journey_aliases` + union-find + `PendingEntry.root_session_id`) **concluídos e validados** (ver `CHANGELOG.md`). Pendente: **J4** (avaliação N3 — `session_signal` grain=`journey`) e **J5** (`@ctx.journey.*` reaceso, ABAC, guards, polish). Decisão as-built: cache `sessions.journey_id` **diferido** (reads por union-find, sem refresh no merge).
