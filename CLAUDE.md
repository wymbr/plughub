...
### Fila de trabalho humano / dispatch pull + inbox no Console *(pull genérico ✅; aprovação = ADR fechado, a implementar)*
...
- **Camadas:** **A** `dispatch: inline|detached` no `PoolHookEntry` (schema, default inline; guard rejeita `detached` em `on_human_start`) ✅ iniciada · **B** pull direcionado (`assigned_to`+`fallback_to_pool_after`, claim-eligibility no Routing Engine, reusa `dispatch_mode: pull`) · **C** `acw_gate: none|soft|hard` (regra de `agent_ready`) · **D** bridge honra `detached` (não conta `hook_pending`; `workflow_trigger(origin+segment ctx)`; fecha contato) — **atualizar `conference-mechanics.md` § Histórico** · **E** migrar wrap-up/survey p/ `detached`, aposentar `skill_survey_v1`/`agente_survey_nps_v1`/pool `survey_collector_ia` · **F** validação (G1, atribuição, smoke pull inbox).
