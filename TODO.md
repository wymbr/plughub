...
**Candidato Fase B/C — gate de validação de dispositivo p/ steps sensíveis (proposta, 2026-07-02):** liberar sequências de workflow **sensíveis** só com uma **validação do dispositivo/identidade que entrou em contato**. **Invariante mantido (princípio 7 da spec):** a plataforma **NÃO** é autoridade de identidade — ela só provê o **gate** (`requires_identity`/`customer_resumable` no `delegate` + política no fluxo) e **dispara a transação de validação** contra **cadastros oficiais** via MCP de domínio (`identity_verify` da retaguarda do tenant); a autoridade da validação é **externa**. A plataforma guarda só o **veredito** (`verified_at` + `confidence` bump na `secondary_key`, ou `external_ref`), nunca a credencial/código (trilho de masked input, `@masked.*` in-memory, não persistido). Um OTP-para-canal só entra se for **emitido/validado pela retaguarda** (a plataforma relaya), não pela própria plataforma como autoridade. Eleva âncora `phone`/`email` de fraca→verificada, reduz mis-merge (handle compartilhado/spoofado) e destrava `resume_policy: auto` + confirmações. Anti-enumeração + consentimento obrigatórios. Depende do wiring de intake (acima) + do `identity_verify` (Fase B). Ver spec §4.4 (dois momentos), §5 (identificação/enriquecimento pela retaguarda), §6/§8 (gate no delegate), princípio 7.
