...
| **J4c** (pendente — ADR `adr-outbound-survey-as-collect-contact.md`) | **Survey web outbound = contato via `collect`, não sinal solto.** Criar canal `survey`/`web` (adapter channel-gateway que renderiza o DialogForm); o agente de survey usa `collect` (Arc 19 suspend/resume, cobre o assíncrono) → **contato-filho membro N1 da journey** (herda `root_session_id`, transcrição própria, stream/masking/audit), sinal grain=journey emitido no close. `survey_link_create`+página tokenizada vira **legado / veículo do sinal anônimo** (survey sem raiz/cliente conhecido). Alinha com o §19 do spec de surveys (que já prescreve outbound via collect). **Impacto a decidir:** metering/billing (survey passa a contar como contato — correto p/ outbound) e estatística operacional. Config por survey: veículo leve (sinal) vs. contato pleno. | J4b |
