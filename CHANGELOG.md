...
- **`@plughub/schemas/outbound.ts`**: `ContactPolicy`/`ContactLog`/`FrequencyCap`/`ChannelCap` + inputs (`CreateContactPolicy`, `EligibilityRequest/Result`, `UnsubscribeInput/Result`). Exports no index.
- **`mailing-api`** (`db.py`+`router.py`): DDL `outbound.contact_log` (índices por `(tenant,customer,contacted_at)` e `(…,channel,…)`) + `outbound.contact_policy` (`UNIQUE(tenant,scope,scope_id)`). CRUD de policy (`POST/GET/PATCH/DELETE /v1/contact-policies`, upsert por escopo). **`POST /v1/contact/eligibility`** → `db_contact_eligibility`: resolve a policy efetiva (campanha sobre tenant), avalia **quarentena → frequency_caps → channel_caps** contra o `contact_log` numa transação, e com `claim=true` **grava o fato** (janela começa no envio); `reason` sempre nomeia a regra (nunca silencioso); `retry_after` best-effort. Parser de janela `24h|7d|60m|30s`|seg. **`POST /v1/unsubscribe`** → `db_unsubscribe`: flip `entry.status='unsubscribed'` por cliente (mailing opcional; o drain já exclui não-`active`).
- **tools MCP** (`tools/outbound.ts`): `contact_eligibility_check` + `mailing_unsubscribe` — wrappers finos, `isError` em não-2xx, auditados via McpInterceptor.
- **Smoke** `infra/test/smoke_outbound_fase2.sh` (via API): policy (cap 24h max 1) → check#1 allowed+claimed → check#2 mesmo cliente/janela negado (`frequency_cap`) → unsubscribe A → drain traz só B. Cliente com timestamp (re-rodável); limpa a policy no fim.
- **Validação (2026-07-21):** `smoke_outbound_fase2.sh` verde — check#1 `allowed:true, claimed:true`; check#2 mesmo cliente/janela `allowed:false, reason:frequency_cap, retry_after:86400`; unsubscribe A → drain traz só B. Dois bugs corrigidos na validação: (1) `_count_contacts` passava um `$3` **não referenciado** no ramo channel-null → Postgres "could not determine data type of parameter" (renumerado por ramo); (2) `router.py` tem `from __future__ import annotations` mas **não importava `datetime`** → `EligibilityBody` "not fully defined" no Pydantic (import adicionado).
- **Deploy:** `build mailing-api && up -d --force-recreate mailing-api` (novo DDL no boot — basta isto para o smoke via API). Para as tools vivas nos skills (Fase 2b): `build mcp-server-plughub && up -d --force-recreate mcp-server-plughub` + recriar `skill-flow-service` e `orchestrator-bridge` (lição da Fase 1 — reconectar MCP pré-aquecido).

---

## Outbound — Fase 1: substrato mínimo (mailing + campaign + drain) ✅ validado E2E (2026-07-21)

Primeira sub-fase do módulo **Outbound** (Fase 4 do arco Scheduler). Substrato **genérico** de contato ativo — `mailing` (audiência) + `campaign` (orquestrador fino, endereça **POOL** — S4) + `campaign_delivery` (estado por-campanha). **Survey é o 1º consumidor (S11 agendado), não o dono.** Design fechado em `docs/product/outbound-mailing-campaign-design.md`; spec de implementação em `docs/product/outbound-fase1-implementation-spec.md`; arco em `docs/arcos/outbound.md`. Escopo aprovado com o usuário: **backend e2e, sem UI** (fatia 1b) + **spec versionado antes de codar**.

- **`@plughub/schemas/outbound.ts`** (novo): contratos Zod `Mailing`/`MailingEntry`/`Campaign`/`CampaignDelivery` + inputs REST (`AddEntry`, `Drain*`, `DeliveryResultInput`). Fonte canônica dos blobs JSONB. Exports no index.
- **`packages/mailing-api`** (novo — Python FastAPI + asyncpg, porta **3660**, schema PG `outbound`): store canônico do domínio (one-source). `db.py` (DDL `mailings`/`mailing_entries`/`campaigns`/`campaign_deliveries` + CRUD + **drain** `FOR UPDATE SKIP LOCKED` + claim `ON CONFLICT DO UPDATE WHERE result='failed' AND attempts<max`), `router.py` (CRUD + `POST /v1/mailings/{id}/entries` = `mailing_add`, `POST /v1/campaigns/{id}/drain`, `POST /v1/deliveries/{id}/result`, `GET …/deliveries`), `config.py`/`main.py`/`Dockerfile`/`pyproject.toml`. `dedup_key` por `dedup_policy` (customer / customer_context=sha256 / none). Serviço adicionado ao `docker-compose.demo.yml`.
- **tools MCP grupo `outbound`** (`mcp-server-plughub/tools/outbound.ts`, novo): `mailing_add` / `campaign_drain` / `campaign_delivery_result` — wrappers finos do mailing-api, `isError` em não-2xx (degradação não silenciosa → `on_failure`), `x-tenant-id`, auditados via McpInterceptor. Registrados nos dois sites do `server.ts` (stdio + SSE); env `MAILING_API_URL` no serviço `mcp-server-plughub`.
- **Demo** (diff zero no scheduler): pool webhook **`outbound_demo`** + **`skill_outbound_demo_v1`** (perfil workflow: `campaign_drain` lendo `@ctx.campaign_id` do payload da agenda → `loop` sobre `drained` → `campaign_delivery_result`). O `loop` é guard de ciclo reconhecido (engine.validateFlow). Seed `infra/test/seed_outbound_demo.sh` (mailing + 3 entries + campaign via REST). Gate `infra/test/smoke_outbound_fase1.sh` (agenda → fire → asserta entregas claimadas + contabilizadas + **re-disparo sem re-drain**).
- **Decisão de fronteira (2026-07-21):** o `contact_eligibility_check` (Fase 2) **substitui** o `survey_eligibility_check` — motor de elegibilidade único e genérico; survey (a implementar **depois** do outbound) consome o genérico. Registrada no design (§4/§7/§9) e no spec.
- **Fix de engine (invoke em loop):** `packages/skill-flow-engine/src/steps/loop.ts` — o `loop` limpava o sentinel `:__notified__` (notify) a cada iteração, mas **não** o `:__invoked__` (invoke). Com um `invoke` no body do loop, o sentinel `"completed"` fazia o step **retornar o resultado cacheado sem re-chamar a tool** a partir da 2ª volta → drenava N mas contabilizava só 1. Adicionado o clear de `:__invoked__` (simétrico ao notify). Bug latente para **qualquer** skill com invoke-em-loop; nenhum skill existente usava esse padrão (survey_multi tem body=menu), então mudança segura. Rebuild: `skill-flow-service` (bundla o engine).
- **Validação E2E (2026-07-21):** `smoke_outbound_fase1.sh` verde — agenda → webhook → `campaign_drain` (claima 3, atômico) → `loop` → `campaign_delivery_result` (3× `contacted`) + re-disparo sem re-drain. Smoke ajustado: `sleep` fixo → **poll até 45s** (a sessão webhook é assíncrona/cold-start).
- **Deploy:** `build mailing-api && up -d mailing-api`; `build mcp-server-plughub && up -d --force-recreate mcp-server-plughub`; `build skill-flow-service && up -d --force-recreate skill-flow-service` (fix do loop) **e em seguida `up -d --force-recreate orchestrator-bridge`** — recriar o skill-flow-service invalida a conexão que o bridge mantém (`Cannot connect to host skill-flow-service:3460`); sem recriar o bridge, o webhook nasce mas o flow não executa. Skill YAML (mount ro) + pool `outbound_demo` via RegistrySyncer no restart do bridge (seed-if-absent: YAML só semeia DB vazio).

---

## Scheduler / Agenda — Fase 3: UI (autoria + Monitor) + fire-now (2026-07-21)

Fecha o arco do Scheduler com as duas superfícies de UI e o endpoint que faltava. Decisões travadas com o usuário: **D1** aba Agendas no Monitor unificado; **D2** ABAC `scheduler.*` grant-first, **sem role default e sem bypass de admin** (só quem recebe o campo em Acesso vê as telas); **D3** seletor de pool = só `channel_types: [webhook]`; **D4** "reagendar" = editar a schedule (PATCH recalcula `next_fire_at`), sem endpoint dedicado.

- **Backend `fire-now`** (`scheduler-api`): `POST /v1/agendas/{id}/fire` — `Dispatcher.fire_manual()` força um disparo imediato (grava `AgendaDispatch` com `scheduled_for=now`) e **não** consome/recalcula a recorrência (a próxima ocorrência armada segue intacta). Agenda `cancelled` → 409. Smoke `infra/test/smoke_scheduler_fire_now.sh` (dispara + confere `next_fire_at` inalterado).
- **ABAC** (`infra/modules.yaml`): módulo `scheduler` com `configurar` (autoria) e `operacao` (Monitor), ambos `default: none`. Gate strict na nav e self-guard `perms.can(...)` nas páginas (sem bypass de admin — D2). Seed `infra/seed/seed_auth.py`: concede `scheduler.configurar`+`operacao` ao **admin demo** explicitamente (grant individual, não role default) p/ testabilidade.
- **Proxy** `/v1/agendas` → `scheduler-api:3650` no Vite (`vite.config.ts`) e no nginx do container (`platform-ui/Dockerfile`), antes do `/v1` genérico; `schedules` adicionado à lista de SPA-routes do nginx.
- **platform-ui**: **`/config/schedules`** (`SchedulesPage`, grupo Configuração) — lista + drawer CRUD; seletor de pool webhook (D3, avisa se vazio); editor de `rule` (frequency/interval/weekdays/monthly by-date|by-position/`times[]`/business_day_policy/month_overflow) + `validity` + `timezone` + `calendar_id` (reusa calendar-api) + `payload` (JSON validado). **Monitor › Agendas** (`SchedulesMonitorPage`, rota `/monitor/schedules`) — agendas vivas + régua de `AgendaDispatch` (lazy no expand) com **drill-through** pra sessão (`/analise/sessions?session_id=…`) + ops **disparar agora / pausar / retomar / cancelar** (reagendar fica na autoria — não duplica) + **filtro** (busca nome/pool + chips de status com contagem). **Refinamento UX (2026-07-21):** `disparar agora` só em agendas **vivas** (`active`/`paused`) — na UI e no backend (`fire` rejeita terminal com 409); disparar `completed`/`expired`/`cancelled` ressuscitaria algo encerrado e produziria dispatch numa agenda "concluída". Cliente/tipos compartilhados em `modules/schedules/api.ts`. i18n namespace `scheduler` (en + pt-BR "Agendas"); nav `nav.schedules` / `nav.monitor.schedules`.
- **Deploy:** `build platform-ui && up -d --force-recreate platform-ui` (nginx novo + bundle); reiniciar `auth-api` + re-rodar `seed_auth.py` p/ o grant do admin; `scheduler-api` já no ar da Fase 1. **Achado (D2):** sem o grant explícito, nem o admin vê as telas — é o comportamento desejado (opt-in por usuário).

---

## Scheduler / Agenda — Fase 2: consumidor deploy (promote agendado) (2026-07-21)

Fecha o gate homolog→prod **de ponta a ponta**: uma **Agenda** dispara um pool webhook cujo skill **EFETIVA** a promoção de OUTRO pool. Antes o promote real não existia como ação de skill (o âncora `skill_gate_promocao_v1` terminava em `efetuar_promocao` = `complete` stub). Spec: `docs/product/scheduler-agenda-spec.md` §6/§7. Decisões §7 respeitadas (endereça POOL nunca skill/versão; **sem pin de versão**; **um** caminho de promote; sem retry no v1).

- **Tool MCP `pool_promote`** (`mcp-server-plughub/tools/deploy.ts`): wrapper do **único** caminho de promote (`POST /v1/pools/:id/promote` — next→current→previous, grava `SkillDeployment`, revalida capacidade). `invoke` alcança o agent-registry por MCP (nunca HTTP arbitrário) → permission/injection/audit do `McpInterceptor`. **Não-2xx (409 next vazio / 422 capacidade / 404) → `isError`** ⇒ o skill-flow-service lança ⇒ o `invoke` roteia `on_failure` (o 409 não some, degradação nunca silenciosa). Envia `x-service-token` quando presente (a rota de slots não é gateada hoje; future-proof).
- **Skill `skill_deploy_promote_v1`** (perfil workflow/webhook — só invoke/complete): `invoke pool_promote` lendo o pool-alvo do **payload da agenda** (`@ctx.target_pool`; a webhook.py grava o payload como tags do ContextStore) + `tenant_id` do trigger; `on_success` → complete(resolved), `on_failure` → complete(failed). Uma única Agenda + este pool promovem **qualquer** pool (alvo no payload, não no skill — invariante S4).
- **Pool webhook `deploy_promote_ia`** (`infra/registry/tenant_demo.yaml`, DB-owned, `channel_types: [webhook]`, `agent_kind: ai`, 1 instância): corpo do job. Payload da agenda = `{ target_pool, action: "promote" }`.
- **Gate E2E** (`infra/test/smoke_scheduled_promote.sh`): (A) alvo com `next` encenado → agenda once no passado dispara (≤15s) → `current` vira o ex-next + `SkillDeployment` (notes=promote); (B) `next` vazio → promote 409 → sessão `failed`, **slot intacto** (nenhuma promoção silenciosa). As-built da camada: `AgendaDispatch=dispatched` (a channel-gateway devolve 201+session_id); a falha do promote vive no **ciclo da sessão** (drill-through via `session_id`), não espelhada no ledger.
- **Deploy:** `build mcp-server-plughub && up -d --force-recreate mcp-server-plughub` (tool nova); `up -d --force-recreate orchestrator-bridge` (RegistrySyncer semeia skill+pool dos YAMLs montados — sem build). **Setup one-time** `infra/test/setup_deploy_promote_pool.sh`: o bridge só instancia pools com slot `current` (`deployed_skill_id`), e o demo já declarava Σ=312 > C=310 (cresceu além da capacidade contratada) → o slot do novo pool tomava 422. O setup sobe a capacidade IA pela **API oficial do pricing** (upsert keyed idempotente, re-sync da quota) e encena o slot `current` de `deploy_promote_ia` (set-next+promote com service-token). **Achado registrado:** `requireResourceWrite` (gate de `/v1/pools`) **vaza** para o sub-path `/slots` pelo mount de prefixo do Express — escritas de slot exigem service-token/Bearer apesar do comentário "não gateado nesta fatia" em `app.ts`.
