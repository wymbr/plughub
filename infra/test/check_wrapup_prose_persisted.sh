...
if [ "${TOTAL:-0}" -eq 0 ]; then
  echo
  echo "   ⚠️  NENHUMA amostra APÓS o corte de deploy — INCONCLUSIVO."
...
if [ "${NR:-0}" -eq 0 ]; then
  ...
  echo "      Submeta um com 'Escalado' ou 'Pendente' para cobrir este lado."
else
  NR_MISSING=$($CH -q "SELECT count() FROM ${CH_DB}.segments FINAL \
               WHERE $NR_WHERE AND ifNull(handoff_reason,'')=''" | tr -d '\r')
  [ "${NR_MISSING:-1}" -eq 0 ] \
    && ok "$NR não-resolvido(s), todos ainda com handoff_reason" \
    || bad "$NR_MISSING de $NR não-resolvidos SEM handoff_reason — regressão no caminho antigo"
fi
...
echo "  passou: $pass    falhou: $fail"
[ "$fail" -eq 0 ] || exit 1
echo "  ✅ prosa do wrap-up persiste em toda disposição"
