...
#
# v1.0 (2026-06-18, qualidade ~0.73) → v2.0 (2026-06-21, qualidade ~0.84):
# mostra a qualidade subindo de uma versão para a outra. N=6/versão (< min_sample
# 30 → o aviso "Low sample" aparece, de propósito/honesto).
#
...
# v1.0 — 6 avaliações em 2026-06-18 (~0.71..0.76)
for i in 1 2 3 4 5 6; do emit 1.0 2026-06-18 "0.7$i" "$i"; done
# v2.0 — 6 avaliações em 2026-06-21 (~0.81..0.86)
for i in 1 2 3 4 5 6; do emit 2.0 2026-06-21 "0.8$i" "$i"; done
...
echo "seeded (ClickHouse, finalizadas): 6×v1.0 (2026-06-18) + 6×v2.0 (2026-06-21) em $POOL/$SKILL ($TENANT)"
...
SELECT 'evinstance_seed1b_v2_'||gs, '$TENANT', camp.id, camp.form_id,
       ...
       '2026-06-21 10:00:00+00', '2026-06-21 10:00:00+00', now()
FROM camp, generate_series(1,8) AS gs
ON CONFLICT (id) DO NOTHING;
"

echo "seeded (Postgres, pendentes): 8×v2.0 in_progress em $POOL ($TENANT)"
