...
| ALW-12 | **O mapa de contexto do tenant não tem caminho de provisionamento** — descoberto em 2026-09-02 ao consertar a ALW-11, e é bloqueador da V4. A ALW-04 encolheu a semente para só-plataforma (decisão certa), mas a contraparte não existe: há `infra/dialog/*.json` e `infra/registry/*.yaml`, e **não há `infra/context-map/`**. As **20 canônicas de domínio de tenant** do demo (`cartao`, `conta`, `portabilidade`, `processo`, `reembolso`, `journey.cartao`, `journey.processo`) existem só porque alguém as escreveu pela API uma vez. Medido: contra a semente — que **é** o estado pós-`--wipe` — **17 das 52** escritas de skill ficam não declaradas. Com o portão da V4 ligado o `RegistrySyncer` as recusaria no boot, e ele **não bloqueia o startup** (`registry_syncer.py:623`): a stack sobe, os skills ficam sem publicar, e o único sinal é o contador `skills_errors`. É a regra do `CLAUDE.md` sobre ambiente que só sobe porque já subiu antes. ⚠️ Os 17 são **os mesmos 17 aliases** que bloqueiam a ALW-05 — migrar os skills de demo para os nomes canônicos ataca as duas, mas **não dispensa** o provisionamento, porque as canônicas também não estão na semente | `aberto` | medido em 2026-09-02 |
