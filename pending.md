...
| AUT-39 | **`config.users` é `scopable: false`, e as rotas de usuário não recortam — ✅ EXPOSIÇÃO MEDIDA AO VIVO em 2026-09-09; a escolha do modelo segue pendente.** `GET /auth/users` (`router.py:390`) lista o tenant INTEIRO; `PATCH /users/{id}` só protege alvo **privilegiado** (`_assert_may_touch`); e `password` **não** é campo de capacidade, por decisão declarada — cujo escopo, lido no `router.py:115`, é **escalação** (*"resetar a senha do admin e entrar como admin"*), não o caso lateral desta ficha. ⚠️ **Medido de ponta a ponta, com alvo descartável:** um supervisor com **ZERO pools** listou os **9** usuários do tenant, trocou a senha de um usuário de outro time (**HTTP 200**), **entrou na conta** com a senha nova e a original deixou de valer — o dono perde o acesso. ⚠️ **E é SILENCIOSO:** o handler do PATCH não emite log nenhum (o `router.py` inteiro tem 4 chamadas de logger, nenhuma nesse caminho) e não há tabela de trilha no schema `auth`. Quem sofre vê só a senha parar de funcionar. **Essa metade é independente da escolha do modelo e vale por si.** ⚠️ **O censo que a saída (a) exigia foi feito, e ele DEPÕE CONTRA ela:** sob `pools(alvo) ⊆ pools(aplicador)`, (i) o `admin@` fica **sem administrador** (41 pools ⊄ ninguém); (ii) ele só administra os outros porque ENUMERA os 41 pools do registry — **um pool novo o tira dessa condição sem erro em lugar nenhum**, que é exatamente o discriminador da AUT-29 e o modo de falha que esta casa mais paga; (iii) o único supervisor real cobre 2 de 41 e passaria a não administrar ninguém. **A saída (b)** (`agent_groups` do Arc 9) segue **greenfield** — reconferido: 0 grupos, 0 membros, 0 supervisores — e é membership EXPLÍCITA, sem esse caminho de degradação muda. População real do tenant hoje: **3** usuários (as outras 6 linhas são fixtures de probe), e **1** supervisor real portador do campo — logo a exposição é real e o dano hoje é ~zero, o que dá tempo de escolher certo. ⚠️ A AUT-22 faz esta pergunta sobre o campo **vizinho** (`config.permissions`), não sobre este | `aberto` — falta a DECISÃO do modelo (a × b); a trilha do reset é fatia independente | medido 2026-09-08, exposição e censo em 2026-09-09 |
