...
| VOZ-06 | **Gravação por segmento, com aviso e opt-out** (V-F3), no AttachmentStore com classe de retenção. O mecanismo já está decidido (config por classe no namespace `storage`); falta o **default de cada classe**, que é decisão de negócio/jurídico — e o A5 registra um conflito doc×doc de 5 anos × 30 dias que nenhum código arbitra | `bloqueado` — **destrava quando** o dono fixar o default de retenção por classe | ADR §8 · A5 |
...
| AUT-52 | **`probe_report_row_scope` está VERMELHO em duas rotas, e a medição diz que o defeito é do INSTRUMENTO.** O ramo B acusa `evaluations/summary` e `evaluations/quality` de não recortar (admin=13 e escopado=13). Mas essas duas — e só essas — são julgadas por `(.data|length)`, que conta **GRUPOS**, não linhas; as outras cinco usam `.meta.total`, e passam. Medido no ClickHouse em 2026-09-12: a instalação tem **15** campanhas em 256 avaliações, e o pool `sac_ia` sozinho carrega **13 campanhas em 141 avaliações** — ou seja, recortar para um pool pode devolver o MESMO número de grupos recortando 100 das linhas. Agravante de desenho: o probe escolhe de propósito o pool com MAIS sessões (para não sair `SEM AMOSTRA`), que é justamente o que maximiza a chance de cobrir todos os grupos. **Não é regressão do produto** — o processo em execução é anterior ao trabalho da AUT-45 e nenhum código do analytics mudou desde 2026-09-11; as duas rotas também não estão em `_SCOPE_EXEMPT` nem em `_SCOPE_DEBT`. Fechar exige dar a essas duas um julgamento que possa REPROVAR: total de linhas em vez de grupos, ou um pool-controle sabidamente sem avaliação, ou um ramo INCONCLUSIVO quando o escopo cobre todos os grupos. ⚠️ **Enquanto isso, o gate fica vermelho por uma proposição que não é a dele** — a mesma família da D14.1 | `aberto` | `CHANGELOG.md` 2026-09-12 (2) |
