...
| ALW-06 | D7 (metade) — tela de proveniência: global × override por nó. ⚠️ **Deixou de ser hipótese em 2026-09-02**: um `PUT` de `masking.types` no escopo GLOBAL respondeu **200 e não teve efeito nenhum** — `tenant_demo` tem override daquela chave, e tenant vence global POR INTEIRO (`LIMIT 1`). Só se descobre contando os tipos nos dois escopos (eram 14 × 13). Um `PUT` que responde 200 e não faz nada é pior que um que recusa; a tela precisa dizer QUAL escopo está em vigor antes de deixar salvar. ⚠️ **O eixo enganou TRÊS vezes em 2026-09-02**, e as outras duas medições valem para quem for fazer: (2) `masking.context_map` tem **só** a linha `__global__` — um tenant inexistente recebe o mesmo mapa, e a CNS-08 recusa com 422 qualquer `PUT` de tenant nessa chave, então o comportamento **difere por chave** dentro do MESMO namespace; (3) o `seed_context_map.py` precisou de guarda PRÉVIA (global × efetivo) porque conferir só o efeito pegava o sintoma depois de já ter gravado. **MESMO EIXO que a CNS-14** — esta é a metade de AFORDÂNCIA, aquela a de AUTORIZAÇÃO | `aberto` | índice `CLAUDE.md` |
...
| CNS-14 | **Quem tem `config.masking` escreve o `__global__`** — mandando `tenant_id: null`, e de lá reescreve o `core.*` que a CNS-05 protegeu do lado do tenant. Fronteira distinta: *quem pode editar o default da PLATAFORMA*. Registrada ao fechar a CNS-05, e deliberadamente não fechada junto — misturaria duas decisões. ⚠️ **MESMO EIXO que a ALW-06, e decidido em 2026-09-02 fazer as duas juntas**: esta é a metade de AUTORIZAÇÃO (quem pode escrever o global) e a ALW-06 é a de AFORDÂNCIA (a tela dizer qual escopo está em vigor antes de deixar salvar). Separá-las entrega meia solução para um eixo que já enganou três vezes no mesmo dia — ver a ALW-06. **Medir antes de fechar:** quantos portadores de `config.masking` existem hoje. Política contra população zero é o erro que este repositório já catalogou | `aberto` | `router.py` `_reject_tenant_core_root` (docstring) |
