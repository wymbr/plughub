...
| id | tarefa | status | referencia |
|---|---|---|---|
| ORQ-01 | **F1 — o eixo de DEMANDA passa a existir.** ⚠️ **PARCIAL em 2026-09-06: o mecanismo esta pronto e verificado; a MEDICAO ainda nao existe.** Entregues: `optionsAtPath` em `@plughub/schemas` (projecao de UM nivel, pura, com o controle negativo — segmento desconhecido devolve vazio, nunca a raiz) · tool `dialog_tree_level` (projeta, faz o APPEND do id escolhido porque o fluxo nao tem primitiva que concatene array, e devolve `is_leaf`/`root_id`/`category_path`) · forma `dialog_navegacao_atendimento_v1` publicada (9 folhas, 2 niveis) · `skill_navegacao_v1.yaml` (dry-run do servidor `valid:true`; o validador de CICLOS do engine aceita o fluxo e **recusa** a mutacao em que o ciclo deixa de passar pelo `menu`) · gate `probe_orchestrator_tree_nav.sh` (4 ramos, com a mutacao DENTRO dele). ⚠️ **Falta o que da nome a fase:** nenhum pool roda a skill, entao `agent_event` de demanda continua ZERO. Fechar exige (a) um pool com a skill deployada e (b) um contato real. Enquanto isso, dizer que o eixo existe seria o *"existe != esta pronto"*. ⚠️ Achados do caminho: esta skill e o **primeiro chamador de `agent_event` do repositorio**, e a `category` tem de comecar pelo pool da SESSAO (isolamento de namespace, verificado pela tool) — por isso ela se compoe por `{{@ctx.core.pool.id}}` e nao por literal no YAML | `aberto` | ADR F1; `CHANGELOG.md` § 2026-09-06 (3) |
| ORQ-06 | **`agent_event` e INALCANCAVEL de um agente nativo de skill-flow — e e por isso que ele tem ZERO chamadores.** Medido em 2026-09-06 ao escrever a F1: a tool exige `session_token` de `agent_login` (`z.string().min(1)`, obrigatorio), e o orchestrator-bridge **nao fornece nenhum** — a palavra `session_token` aparece **0 vezes** no `main.py` dele, e o `session_context` que ele monta tem `contact_id`/`channel`/`tenant_id`/`agent_type`/`session_id` e mais nada. Os DOIS produtores existentes confirmam por contorno: o `agente_avaliacao_v1` faz `invoke agent_login` com um `instance_id` sintetico, e o `segment_outcome_record` **nao chama a tool** — usa o `buildAgentBusinessEvent` interno, com o comentario dizendo em letras claras *"o wrap-up roda como workflow, sem agente logado"*. ⚠️ **Isto bloqueia a entrega da ORQ-01**: a navegacao funciona, o roteamento funciona, e o evento de DEMANDA — o unico numero que a fase existe para mudar — nao emite. Tres saidas, e a escolha e de contrato: (a) o skill faz `agent_login` (ha precedente, mas o agente JA esta alocado e logar de novo mexe em estado vivo do routing); (b) expor um caminho interno equivalente ao `buildAgentBusinessEvent` para skill-flow; (c) `agent_event` aceitar `session_id` + credencial de servico. **Nao escolhi sozinho porque as tres tocam o contrato de agente.** | `bloqueado` | impedimento: decisao de contrato; medido 2026-09-06 |
