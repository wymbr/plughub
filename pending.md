...
| AUT-48 | **A recusa do estado da sessão não tem AVISO na tela — o Console mostra painel vazio.** Metade visível declarada da AUT-47 (2026-09-09). O `useSupervisorState` fazia `if (res.ok)` **sem `else`**: 401/403 não viravam estado nem mensagem, e a tela dizia *"não há contexto"* onde o servidor disse *"você não pode"* — mandando o operador procurar DADO onde falta PERMISSÃO. Já valia para o 401; com o portão de pool passa a valer mais vezes. **O que a AUT-47 fez:** a recusa deixou de ser engolida — viaja no retorno do hook (`recusa: {status, reason}`, com o `reason` do servidor: `pool_not_accessible` · `session_scope_undeterminable` · `tenant_mismatch`) e sai em `console.warn` nomeada. **O que falta:** renderizar. Precedente pronto: `EmptyScopeNotice`, que a AUT-03 criou para o escopo vazio. ⚠️ Não feito junto porque verificação visual em navegador falhou neste repositório antes (o login automatizado não completa), e UI não conferida é promessa — o backend fica fechado e medido, a tela entra com olho humano. Chaves em EN + pt-BR, e **404 (`session_expired_or_unknown`) NÃO é recusa**: as duas ausências não podem chegar à tela com a mesma cara | `aberto` | AUT-47, 2026-09-09 |
