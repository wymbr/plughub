...
| IDN-07 | **A âncora não tinha eixo de PROCEDÊNCIA — a metade ESCRITA foi entregue em 2026-09-13 (PID-12); falta a LEITURA.** Entregue: coluna `provenance` em `identity.customer_secondary_keys` (nullable = *não registrada*; as 93 chaves anteriores ficaram NULL, porque carimbar `declared` no legado inventaria uma origem que ninguém mediu), escritores que carimbam (`declared` do contexto, `channel_origin` do identificador do canal, `authoritative` só a importação) e a trava no índice. **Falta:** `CustomerRef`/`_pg_resolve` não devolvem a procedência, e o índice Redis (`{cid, vc}`) não a carrega — e o Lookup 1 lê o Redis PRIMEIRO. É o que a PID-10 vai consultar, e a escolha de onde ler (PG ou Redis) é parte dela. Valores: `declared \| channel_origin \| authoritative \| operator`. Pré-requisito da PID-10 | `aberto` | `docs/adr/adr-identity-door-evidence.md` D13 · `CHANGELOG.md` § 2026-09-13 (4) |
...
| IDN-09 | **Posse provada é HERDADA por outro cliente na reatribuição — no Postgres, e não no Redis.** O upsert de chave mantém `verification_class = 'possessed'` sempre que a linha já era `possessed`, mesmo quando o `customer_id` muda; `verified_at` idem. O índice Redis faz o certo (`attach_anchor` só preserva `possessed` se `existing[0] == customer_id`), então as duas casas discordam e a durável é a errada — e é a durável que responde quando o Redis esfria. Medido ao escrever a PID-12, que fechou **o mesmo formato** para a procedência (lá com mutação que prova a regra). Não consertado junto por escopo: é o eixo vizinho, e muda o que já está gravado para 31 chaves `possessed`. **Conserto:** a mesma regra — prova de posse só sobrevive para o mesmo cliente | `aberto` | `identity/index.py` `_SQL_UPSERT_KEY` · `CHANGELOG.md` § 2026-09-13 (4) |
