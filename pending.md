...
|---|---|---|---|
| MOD-02 | **G1 — guard de RANK nas DUAS portas** *(destravada em 2026-09-08: a MOD-01 fechou e o instrumento da D7 existe)* (`apply-template` e `PUT /users/{id}/module-config`): `rank(concedido) ≤ rank(detido)` campo a campo, **e** escopo `⊆`. ⚠️ **REESCRITA em 2026-09-08 (emenda E2)** — o `delegable` da D4 SAIU: ele existia para aprovar um pacote que atravessaria o guard, e sem travessia não há o que aprovar; era subespecificado (booleano global não responde *"para quem"*). Três precisões que a implementação não pode perder: **(1)** o guard roda sobre **`preset(role) ∪ module_config`**, nunca sobre o literal — `router.py:383` aplica `apply_role_preset`, então `{role:"admin", module_config:{}}` atravessaria intacto e concederia os 44 campos pela porta do preset; **(2)** `scope: []` é **global** no `abac_can` enquanto `accessible_pools` vazio vai virar **"nenhum"** na AUT-03 — sentidos OPOSTOS para lista vazia no mesmo formulário, e copiar a lógica de um para o outro inverte a regra sem erro em lugar nenhum; **(3)** o `PUT module-config` hoje **não compara nada** com o config do chamador — guard só na rota nova seria *"duas portas para o mesmo dado, e só uma trancada"* | `aberto` | ADR § Emenda 2026-09-08 E2 |
| MOD-04 | **G3 — revogar `config.permissions`** de quem o censo apontar, pela API oficial. ⚠️ **Repontada em 2026-09-08**: era `bloqueado por MOD-03`, que fechou; o bloqueador passa a ser a MOD-02, e o motivo muda — não é mais risco de tirar a contratação (a emenda decidiu que ela vem do guard de rank), é coerência: revogar antes do guard existir deixa o delegado sem caminho nenhum. Medido em 2026-09-08: **5 portadores**, e **3 com papel `supervisor`** (contas de probe) | `bloqueado` por MOD-02 | ADR fase G3 |
