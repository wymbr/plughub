...
| RET-15 | **As tasks EFÊMERAS de trabalho não têm dono nem rede — seis delas, medidas por AST em 2026-09-09.** A RET-13 fechou as de VIDA LONGA (boot); estas nascem dentro de um handler, uma por mensagem, e são outro fenômeno: `asyncio.create_task(...)` como STATEMENT, sem guardar a referência e sem `try/except` cobrindo o corpo da corrotina. São elas: `whatsapp.py:125` (`_process_inbound` — só o `json.loads` está protegido; **mensagem de cliente**), `outbound_consumer.py:90` (`_dispatch`, protegido em parte), `voice.py:374` (`_download_and_store_recording`) e quatro do `webrtc.py` (`_tts_inject`, `_start_stt_pipeline`, `_start_egress`, `_stop_egress_and_store`). ⚠️ **Dois danos distintos, e o segundo não tem log nenhum:** a exceção fica presa no objeto Task e some; e sem referência guardada o loop é o único dono, então o CPython pode **coletar a task no meio da execução** — o aviso que o `CLAUDE.md` registra em § Postura (*"`ensure_future` sem guardar o retorno"*). ⚠️ **`email.py:158`, `sms.py:112` e `webrtc_room_client.py:230` NÃO entram** — o corpo delas é um `try/except` que cobre tudo; o eixo é o corpo desprotegido, não a criação. Trabalho: decidir se o conserto é embrulhar o corpo (barato, local) ou dar dono à task; e estender o gate `probe_background_task_supervision.sh`, cuja população hoje é declaradamente só a de BOOT | `aberto` | medido 2026-09-09 (RET-13) |
