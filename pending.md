...
| id | tarefa | estado | evidência |
|---|---|---|---|
| CNS-15 | **Decidir, caso a caso, o destino dos 12 skills sem deploy vivo** — a CNS-01 mediu e classificou (spec §1.2); o que resta é decisão, não medição. Um é descartável sem ressalva (`journey_demo_v1`); 4 são citados por CÓDIGO vivo (o `revisao_treplica_v1` em 6 sítios, e o `CLAUDE.md` já o declara *legado/superseded, reactive-only por compat com o cenário 28*); 2 são desenho encenado da Fase 2; 2 são exercitados por teste; 3 só aparecem em migration/script histórico. ⚠️ **Não-deployado ≠ abandonado** — só o segundo se apaga | `aberto` | `CHANGELOG.md` § 2026-09-01 CNS-01 |
...
| CNS-12 | **`probe_seed_drift_named` reprova 2 ramos, e um é achado de PRODUTO.** O ramo **E** acusa que o seed **REESCREVEU** a key divergente — contradiz a decisão da D7 (*compara e loga, **nunca** conserta*), que existe porque a divergência tem duas direções e reaplicar descarta o que só está no banco. O ramo **C** cobra que o relatório nomeie o campo alterado e o número de itens descartados; a linha observada nomeia (`DIFEREM=2 [intervalo.label, reuniao.label]`) mas o número não bate com o que o probe espera. ⚠️ **Pré-existente**, medido por `git stash` em 2026-09-01 (falhava igual antes da CNS-11), e **ficou visível** porque o ramo A parou de reprovar por bancada. Decidir qual dos dois está certo — o probe ou o seed — antes de mexer em qualquer um | `aberto` | `infra/test/probe_seed_drift_named.sh` ramos C e E |
...
| CAP-17 | **Teste órfão da remoção do `agent_role`.** `packages/schemas/src/agent-registry.test.ts:292` (*"rejects orchestrator agent without skills"*) espera que o schema RECUSE um agente `orchestrator` sem skills — validação que saiu junto com o eixo em 2026-09-01. **Vermelho pré-existente**, confirmado por `git stash` em 2026-09-01: 1 de 47 no arquivo, 1 de 179 na suíte de schemas, com e sem as mudanças da CNS-04. Não é regressão; é a cauda da remoção. ⚠️ Apagar não basta pensar duas vezes: o teste guardava *"orquestrador sem skill não faz sentido"*, e se essa proposição ainda valer sob outro discriminador, ela precisa de instrumento novo — senão a remoção troca um vermelho por um silêncio | `aberto` | medido 2026-09-01 |
---
