...
|---|---|---|---|
| NIV-02 | **Derivar `requires ⊇ {masked_input}` da declaração `masked:`**, num sítio só. Medido: `select_channel()` existe, é pura, é chamada de `main.py:365`, e `masked_input` já é `ChannelCapability` que **só o webchat tem** — mas o `requires[]` é declarado pelo AUTOR do fluxo e **nenhum YAML do repositório declara `requires:`**. Portão vivo que nunca recebeu exigência real. Não é mecanismo novo: é o INSUMO que falta. *Remover a alternativa custa menos que lembrar de não usá-la* | `aberto` | pré-requisito NIV-01 fechado em 2026-09-03 |
| NIV-03 | **Recusa em DOIS momentos, e o barato vem primeiro.** No **deploy**: skill com campo `masked` promovida a pool sem nenhum canal `masked_input` é decidível estaticamente — mesma forma do portão da V4, e pega antes de haver cliente do outro lado. No **runtime**: conjunto elegível vazio → recusar alto (`masked_fallback: decline` já nomeia o desfecho). Hoje `select_channel` devolvendo `None` gera `logger.warning` + `return`: o collect não acontece, o que não é entrega nem recusa. **Fecha a MSK-01 sistemicamente** (aquela tem mitigação barata própria e não precisa esperar por esta) | `aberto` | pré-requisito NIV-01 fechado em 2026-09-03 |
| NIV-05 | **A capacidade `masked_input` está definida pelo MECANISMO, e isso torna o gatilho de voz inalcançável.** O enum diz `password-overlay masked field (webchat)` — lido ao pé da letra, voz nunca qualifica, ainda que o plano de mídia suba E o DTMF seja suprimido. Achado pelo dono ao perguntar se a NIV-01 mantinha voz elegível; era **erro meu** escrever um gatilho que a própria definição fecha. Capacidade tem de descrever a **GARANTIA** — *"o canal coleta entrada sem que o valor apareça em nenhuma superfície de leitura (transcript, histórico, gravação)"* — e não a implementação: aí webchat qualifica por overlay e voz por DTMF+bipe+supressão, mecanismos diferentes com a mesma garantia. É o mesmo movimento do `EchoMode`: domínio abstrato, cada canal interpreta. ⚠️ **Muda quem pode reivindicar a capacidade** — é decisão de contrato, não ajuste de comentário. **Pré-requisito real** do gatilho de voz | `aberto` | `skill.ts` `ChannelCapabilitySchema` |
| NIV-06 | **Supressão de DTMF — o trabalho que a garantia de voz exige.** Medido em 2026-09-03: **não existe** supressão de DTMF no adapter de voz, nem em transcript nem em gravação. *"Mascarado por natureza"* não é automático — o dígito vaza se a plataforma o transcrever, e **DTMF é decodificável a partir do áudio gravado**, então a perna gravada também precisa ser tratada. É TRABALHO, distinto do provisionamento do plano de mídia (esse é deploy, arco V-F0..V-F5). ⚠️ Não confundir com o TRATAMENTO de eco em voz (`plain` verbaliza o dígito · `masked` bipa · `none` cala), que é o `EchoMode` da ALW-10 e está intacto — o eco existe para dar feedback de tecla, e nada aqui o remove | `bloqueado` | depende do plano de mídia (Arc 15) |
| NIV-04 | **Os `notify` cravados migram para `DialogForm`** — 11 só no `skill_limite_entrada_v1` (*"Olá! Posso te ajudar com o aumento de limite…"*). Roteiro é CONTEÚDO, não I/O nem negócio; se o N1 "vai para o canal" sem esta cláusula, as frases acabam dentro do adapter, versionadas com o deploy do serviço e sem editor. ⚠️ **Caso a caso, quando o fluxo for tocado — não é varredura**, e o gatilho é a próxima edição de cada skill | `adiado` | ADR §5 F4 |

## `docs/guias/masked-input.md` — mascaramento de entrada por canal

| id | tarefa | estado | onde |
|---|---|---|---|
| MSK-01 | **Campo `masked` chega a canal que não sabe mascarar, e nada acontece.** Medido em 2026-09-03: o pool `limite_ia` declara `channel_types: [webchat, whatsapp]` e roda `skill_limite_entrada_v1`, que mascara **CVV**. No WhatsApp isso vira campo de formulário comum — sem fallback, sem aviso, sem recusa. O `supports_masked_input: false` que a tabela de `channel-events.ts:143-148` atribui a whatsapp/sms/email é **comentário sem leitor**. ⚠️ **Exposição × dano, separados de propósito** (D14.1): exposição é REAL e declarada; dano medido é **~zero** — 2 sessões WhatsApp em toda a instalação, contra 565 webhook e 413 webchat, e sem credencial real de provedor aqui. Não é incidente; é porta que existe e não está fechada. **Tem desfecho BARATO e INDEPENDENTE da ALW-15** — tirar `whatsapp` de `limite_ia`, ou fazer o adapter RECUSAR alto o step masked — e é por isso que é item próprio: não precisa esperar o resolvedor por canal. **Gatilho para virar urgente:** tráfego WhatsApp real, ou um segundo pool com masked em canal sem suporte. A **NIV-03** a fecha sistemicamente; esta linha existe porque a mitigação barata não precisa esperar por aquela | `aberto` | `packages/channel-gateway/.../adapters/whatsapp.py:564` |
