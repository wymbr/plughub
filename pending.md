...
|---|---|---|---|
| CTR-02 | **G2 — `provides` simetrico ao `required_context`, com CONFERENCIA.** A metade de entrada existe ponta a ponta (o engine computa `@ctx.__gaps__` antes do primeiro step, `engine.ts:507`) e esta ociosa: **1 de 41** skills declara `required_context`, **0 de 41** le os gaps, **0 de 41** usa o step `resolve`. A metade de saida — o que o skill DEIXA no store — **nao existe**, e sem ela a cadeia orquestrador→especialista→orquestrador nao se compoe. ⚠️ Declarar `provides` e promessa: o mecanismo e comparar o declarado com o que esta no store ao fim do skill e CONTAR a diferenca, senao ele nasce ja no catalogo de declaracoes sem consumidor | `aberto` | ADR G2/D6; medido 2026-09-06 |
| CTR-03 | **G3 — `delegate` no orquestrador: ele mantem `primary`, o especialista entra como `specialist`.** Simetria com o que o orquestrador HUMANO ja faz (medido na trilha do contato `34e20c62`). O verbo e `delegate` (endereca POOL, aceita ref, consome o `navigation_pools` da F3 sem linha nova) e **nunca** `task` — que endereca skill, carrega o YAML do disco ignorando o slot promovido, e **nao acha 3 dos 5 especialistas** (o loader casa por nome de arquivo; 14 de 41 skills tem `id` != nome). Ajuste colateral: o ramo F do gate passa a aceitar `delegate.pool` ⚠️ **A premissa se CONFIRMOU e a ORDEM se refutou (medido 2026-09-06).** A trilha viva prova a D1: `limite_ia primary seq=0` -> `dialog_runner specialist seq=0` -> `limite_ia primary seq=1` (**24 segmentos do alvo, 100% specialist; 49 do chamador, 100% primary**). Mas delegar e SUSPENDER o chamador, e ele so volta com `workflow_resume`: dos **6 destinos distintos** de `navigation_pools`, **zero sao delegaveis** (4 `nao_retorna`, 1 `cadeia_delegate`, 1 `sem_deploy`). Ligar o delegate hoje penduraria o contato ate o `timeout_hours` **sem nada ficar vermelho**. Entregue nesta rodada: o INSTRUMENTO (`probe_orchestrator_delegability.sh`, 3 ramos, verde) que mede a delegabilidade e **trava** quem delegar a alvo que nao devolve — inclusive com `pool` por ref, caso em que cobra o mapa inteiro ✅ **DESTRAVADA em 2026-09-06**: a metade do RETORNO da CTR-04 foi entregue e o gate virou sozinho, **0 -> 4 destinos delegaveis** (`auth_form_ia` · `auth_sac_ia` · `reembolso_ia` · `sac_ia`). Restam fora `portabilidade_ia` (`cadeia_delegate`, CTR-06) e `retencao_humano` (`sem_deploy`, pool humano) — logo o ramo `delegar` do orquestrador precisa do verbo DERIVADO por destino, nunca de um `delegate` unico para `$.pipeline_state.rota.pool` | `aberto` | destravada; ver `probe_orchestrator_delegability.sh` |
| CTR-04 | **G4 — os especialistas viram executores.** Medido: nenhum dos 5 e executor puro (todos encerram sozinhos, 3 escalam, todos perguntam) — heranca de terem nascido como atendentes exclusivos. E ha **3 menus de DEMANDA dentro de folhas**: `sac.menu_motivo`, `auth.menu_continuar` e `sac.menu_resolucao`. ⚠️ **Isto reclassifica a F4**: o atalho por `session.navegacao.path` foi tratado como conserto e e tratamento de SINTOMA — `auth_sac_ia` tem a mesma folha-que-navega e produzira a mesma tela que o dono confundiu com bug ⚠️ **Ganhou uma segunda metade, medida em 2026-09-06:** alem dos menus de demanda, cada desfecho do especialista (resolvido, timeout, escalou ao humano) tem de **devolver o controle** via `workflow_resume` com a sua `decision` — hoje **4 dos 5 nao chamam a tool**, e e isso, nao os menus, que bloqueia a CTR-03. O padrao canonico existe (`skill_dialog_runner_v1`: `retornar` / `retornar_falha`). Verificavel por `probe_orchestrator_delegability.sh`, que vira verde por destino conforme cada um passa a retornar ✅ **Metade do RETORNO ENTREGUE em 2026-09-06** (ver `done.md` CTR-08): os 4 executores devolvem o controle em todos os desfechos, condicionado ao token. **Falta a metade dos MENUS DE DEMANDA**: `sac.menu_motivo` (ja contornado pelo `veio_da_navegacao` da ORQ-04, e FICA por decisao — o pool tem porta propria), `auth.menu_continuar` (nunca observado) e `sac.menu_resolucao` (o ADR quer que vire o RETORNO — com o contrato de saida agora existindo, e o orquestrador que deve perguntar *e agora?*, nao a folha) | `aberto` | metade restante: os 3 menus de demanda |
...
| CTR-06 | **Cadeia `delegate → delegate` COLIDE, e nao ha solucao declarada.** `core.workflow.delegate_resume_token` e tag UNICA da sessao, e `agente_portabilidade_intake_v1` **ja delega** ao `dialog_runner`: orquestrador → portabilidade → dialog_runner sobrescreve o token do orquestrador, que nunca retoma. ⚠️ **E o risco numero um do modelo** e o ADR o registra como questao ABERTA, nao como decisao | `aberto` | ADR § Riscos; medido 2026-09-06 |

## `docs/adr/adr-tree-return-continuation.md` — a folha devolve, o chamador continua

| id | o que falta | estado | ancora |
|---|---|---|---|
| RET-01 | **R1 — `on_return` no `DialogOptionSchema` + convencao `main` + `buildRender` a partir de uma question.** O campo e UM (`on_return: <question_id>`), e `QuestionNode.id` **ja existe** — questions identificadas sao convencao, nao schema. ⚠️ O que muda de verdade e o render: hoje ele assume UMA question por form (`before`/`after` decidido por `seenQuestion`), e passa a receber de qual question partir — mesma funcao que serve o `form_get` **e** a preview do editor, entao as duas superficies mudam juntas | `aberto` | ADR R1/D2/D3/D4 |
| RET-02 | **R2 — `delegate` no orquestrador com verbo DERIVADO por destino**, lendo `on_return` no retorno (e a CTR-03 no formato certo). ⚠️ **Nunca um `delegate` unico** para `$.pipeline_state.rota.pool`: 2 dos 6 destinos nao sao delegaveis (`portabilidade_ia` = `cadeia_delegate`, `retencao_humano` = `sem_deploy`), e o unico penduraria o contato de portabilidade. O criterio ja existe e e derivado do artefato | `bloqueado` | impedimento: RET-01 |
| RET-03 | **R3 — cancelar `resume_token` pendente no fechamento do contato.** Decorre da D6 e **nao e opcional**: chamado que encerra sem devolver deixa o `pipeline_state` do chamador suspenso no Redis e o token no hash, e o timeout scanner tenta retomar **uma sessao que ja fechou**. O segmento nao e o problema (o chamador ja fechou o dele ao delegar); o problema e estado morto com scanner ativo em cima | `bloqueado` | impedimento: RET-02 |
| RET-04 | **R4 — teto do ciclo por CONTADOR de iteracoes** (familia do `max_iterations` do `receive`). ⚠️ **Nao pode ser tempo**: `session_timeout` e por INATIVIDADE e um ciclo ativo nunca bate nele. ⚠️ E nao e teto de licenca — medido: o ciclo **nao cria custo novo**, so permite que o contato dure mais; a licenca ja era retida por sessao aberta | `bloqueado` | impedimento: RET-02 |
| RET-05 | **R5 — `menu_resolucao` e `menu_continuar` viram questions de continuacao**, e o `navigation_pools` ganha os prefixos (D5). E a metade da CTR-04 que ficou parada por nao haver dono para receber a pergunta. ⚠️ `sac.menu_motivo` **NAO entra**: fica por decisao, porque `sac_ia` tem porta propria de canal | `bloqueado` | impedimento: RET-02 |
