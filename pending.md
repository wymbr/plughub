...
| CNS-18 | **JSON não-ASCII em variável de shell corrompe** — causa raiz real, medida na CNS-17 (321 bytes × 325). Varrer `VAR=$(…)` que carregue JSON nos probes e migrar para ARQUIVO. ⚠️ **Não confundir com o `PYTHONIOENCODING`**, que já está nos 26 e conserta outra coisa (o `stdout`); esta é a defesa contra a corrupção. Só morde onde o dado tem acento E volta a ser escrito | `aberto` | `CHANGELOG.md` § 2026-09-02 CNS-17 |
