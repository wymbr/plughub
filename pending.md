...
| AUT-46 | **`approvals.decide` chegou ao supervisor (e ao operator) de CARONA num guard de contenção, e ninguém decidiu isso.** Medido em 2026-09-09, de passagem na AUT-40: **6 usuários do tenant têm o grant, todos `read_write`** (admin, supervisor, operator + 3 fixtures). O campo decide *aprovar / recusar / encaminhar* — inclusive `aprovacao_deploy`, promoção de deploy. O supervisor o recebeu na **MOD-08/G1b (2026-09-08)**, e o comentário do próprio `infra/modules.yaml:451+` diz por quê: o guard de RANK da MOD-02 exige `preset(operator) ⊆ preset(supervisor)`, então dar o campo ao operator **obrigou** a dá-lo ao supervisor — capacidade como efeito colateral de uma regra de CONTRATAÇÃO. ⚠️ E há uma casa afirmando o contrário: o `probe_resume_approver_authz.sh` tem o S2 escrito sobre *"supervisor NÃO tem `approvals.decide`"* (verdade em 2026-08-27), e o comentário do `channel-gateway/main.py:1685` também — quem exercer o S2 com `RESUME_TOKEN` vai ver **vermelho sem defeito**. Três saídas: **(a)** o operator não deve decidir aprovação, e o `role_defaults` dele encolhe (o guard então libera o supervisor); **(b)** os três devem mesmo decidir, e as duas casas de prosa são corrigidas; **(c)** o guard de contenção não deve arrastar CAPACIDADE, e passa a comparar só o que é administrativo. Nenhuma tomada aqui — a (a) e a (b) são decisão de produto | `aberto` | medido 2026-09-09 |
