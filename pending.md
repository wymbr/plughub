...
|---|---|---|---|
| ALW-01 | **V4** — inverter o default: campo sem regra deixa de ser acessível. **DESBLOQUEADA em 2026-09-02**: as dependências de tarefa (ALW-02/03/04) fecharam e a decisão #6 do ADR foi resolvida no mesmo dia — o `preview` era resíduo pré-uniformização, migrou para nomear TIPOS e está declarado (`session.workflow.preview`). Pré-condição MEDIDA com a série cortada e `smoke_limite_tres_acessos` (18✅/0❌): **`unknown = 0`** e `overflow = 0`. ⚠️ Antes de virar, refazer a medição — a amostra é de UM fluxo | `aberto` | `TODO.md:381` |
...
| ALW-05 | V5 (metade) — fechar aliases | `bloqueado` — por **tempo**, não esforço: depende do contador decair (**17 alias tags** em 2026-09-02). ⚠️ A série foi CORTADA três vezes naquele dia, para medir a pré-condição da ALW-01. A perda é pequena e está medida — a série anterior era de um burst de 01/09 18:25, ~1 dia, não meses de acúmulo —, mas quem for avaliar o decaimento precisa saber que o relógio recomeçou ali | índice `CLAUDE.md` |
