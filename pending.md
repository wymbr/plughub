...
| AUT-41 | **O módulo `audit` não existe no catálogo — o item do DPO é invisível e inconcedível.** `infra/modules.yaml` declara 11 módulos e o `auth.module_registry` vivo tem os mesmos 11; **`audit` não está em nenhum dos dois**. Mas `Sidebar.tsx:170` gateia `nav.audit` por `audit.sessions`, e a analytics-api o enforça de verdade (`_check_audit_access`, 5 ramos). Sob o portão grant-first isso significa: o item do DPO é **invisível para todos**, e a tela de Access **não tem como concedê-lo** — o `ModulePermissionForm` renderiza o catálogo, então só um `PUT module-config` à mão o alcança. Medido: **0 portadores**. ⚠️ `arc7-auth.md:473` e o `CLAUDE.md` § Audit LGPD **afirmam que o módulo existe** — promessa sem mecanismo. Terceira testemunha: `access.json` tem **12** entradas em `moduleNames`, incluindo `audit`, ou seja o locale sabe de um módulo que o catálogo não tem. Conserto é uma entrada de YAML + `role_defaults` (decidir quem nasce com ele — provavelmente ninguém, e o DPO recebe por concessão) | `aberto` | medido 2026-09-08 |
