...
**PID-01, PID-02, PID-03, PID-07, PID-06, PID-13, PID-15, PID-08 e PID-04 fechadas** — a pendência com exigência só é liberada e só é retomada por quem provou nesta sessão (ou por principal humano verificado), em todas as portas. **PID-10 depende de IDN-07.** A migração dos dois intakes (PID-04) vem
...
| PID-16 | **Promote em lote sobre slots, com rollback por pool.** A PID-08 aposentou o deploy em lote que gravava "implantado em X" sem tocar o slot; o que sobra é a necessidade real que ele fingia cobrir: um release de skill de plataforma custa um `set-next` + `promote` por porta, e um bug do runner atinge todas ao mesmo tempo. Um lote sobre `PoolSkillSlot` (mesmo snapshot em N pools, cada promote com os portões de capacidade/perfil/piso, e um `SkillDeployment` por pool) com rollback por pool. ⚠️ Nada no parque precisa disso hoje: nenhuma skill roda em várias portas. **Gatilho:** a PID-04 (porta de plataforma) ou a primeira skill de plataforma implantada em mais de um pool de entrada | `aberto` | ADR §4 · PID-08 |
| PID-17 | **O intake da portabilidade migra para a porta de plataforma — e carrega três defeitos medidos.** Ficou fora da PID-04 por decisão do dono (8 sessões, zero pendências vivas, retomada nunca exercida). Medido no `agente_portabilidade_intake_v1`: (1) dispara o processo com `context_json` montado por template com o que o cliente digitou (`numero_atual`, `contact_identifier`) — o mesmo vetor que, no limite, trocou a âncora do processo ao vivo; (2) recusar ou falhar o OTP cai em atendimento NOVO, duplicando o pedido de quem tinha pendência; (3) não une a journey (a correção 2b do limite nunca foi portada); e endereça o processo por `skill_id` legado. O `offer` dela significa "aprovado, confirme" — a continuidade do domínio é `portabilidade_confirmacao` | `aberto` | PID-04 · `agente_portabilidade_intake_v1.yaml` |
