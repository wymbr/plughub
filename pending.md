...
| PRM-02 | **`demo_ia` e `demo_llm_ia` estão parametrizados e publicados — falta ESCOLHER a árvore e promover.** O passo 1 (publicar o `config_params` no registry) foi feito em 2026-09-08 pela `PRM-03`: os dois skills declaram `form_id` como obrigatório e o combo *Árvore de navegação* aparece em *Flow › Deploy*. Faltam **(a)** escolher `dialog_navegacao_atendimento_v1` no combo de cada pool e **(b)** `set-next` + `promote`, que levam snapshot e `config_json` no MESMO slot. ⚠️ **Não há urgência nem risco em esperar:** os dois pools rodam o snapshot anterior, com o `form_id` literal, e funcionam — o `probe_slot_required_params` os lista como *declaração mais nova que o deploy*, informação e não defeito. ⚠️ **E não há como errar para o lado ruim:** desde a `PRM-01` o promote RECUSA (422) enquanto a árvore não for escolhida. **Gatilho:** a próxima janela em que valha promover os dois pools de navegação | `aberto` | `CHANGELOG.md` § 2026-09-08 (12) e (13) |
