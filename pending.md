...
| id | tarefa | estado | evidência |
|---|---|---|---|
| CNS-22 | **Quatro pools ainda executam SNAPSHOT anterior à CNS-11, lendo alias que o produtor pode já não escrever.** O bridge executa o snapshot do slot `current`, não o YAML — e a CNS-11 migrou YAML e código sem re-promover ninguém. Censo de 2026-09-03: **8 pools**; quatro já re-promovidos (`dialog_runner`, `nps_ia`, `limite_retorno`, `portabilidade_confirmacao`) porque bloqueavam o demo. **Restam:** `wrapup_detached_ia` (`session.origin_session_id`, `surveyed_agent_key`, `surveyed_segment_id`) · `outbound_survey_worker` (`survey_grain`) · `survey_multi_ia` (`pool.id`) · `copilot_sac` (`sentimento.categoria`). ⚠️ **Re-promover em bloco é perigoso** — cada leitura só passa a funcionar se o PRODUTOR daquela tag também escrever a canônica; o `wrapup_detached_ia` em especial **funciona hoje**, então promovê-lo às cegas troca um verde por um vermelho. ⚠️ **São TRÊS superfícies de defasagem, não uma** — snapshot de slot, DialogForm publicado (seed-if-absent também) e código. A CNS-11 só alcançou a terceira. ⚠️ Falta o mecanismo: nada reprova artefato publicado que lê alias — sem gate, isto volta | `aberto` | `infra/scripts/deploy_skill_to_slot.sh` · censo em `CHANGELOG.md` § CNS-21/22 |
