...
| DLG-31 | **A D5 tirou uma combinacao que o formulario PLANO gravava — medido, nao hipotetico.** A D5 diz *"multi-selecao e DENTRO de uma pasta"*, e o renderer a cumpre limpando as marcacoes ao navegar. Na forma publicada, `servico` filou as folhas em `cadastro` (segunda_via, troca_titularidade) e `plano` (alteracao, cancelamento) — entao **`segunda_via` + `alteracao` deixou de ser marcavel**. Medido no ClickHouse na epoca anterior ao carimbo: **6 contatos** gravaram exatamente esse par, e **2 deles marcaram as quatro folhas**. A arvore nao reorganizou o vocabulario: ela REMOVEU poder de registro que o dado real exercia. ⚠️ **A regra nao e "uma pasta", e "mesmo pai imediato"** — logo *o que pode ser registrado junto depende de onde o autor arquivou a folha*, que e decisao de arrumacao, nao de dominio: mover uma folha para uma subpasta por organizacao apaga combinacoes, em silencio e sem o autor saber. ⚠️ O ganho analitico citado na D5 **nao exige** a restricao: o rollup por `arrayJoin` de prefixos com `uniqExact` ja conta cross-pasta corretamente (uma vez em cada pasta, UMA vez na raiz) — e a lente ja desenha essa nao-aditividade. A restricao nao acrescenta poder analitico; so remove um caso. **Recomendacao:** o escopo da multi-selecao vira DECLARACAO por pergunta (`sibling` x `any_leaf`), com default `any_leaf`; a cesta ja e visivel sob as colunas, entao o risco de cesta acidental que a D5 cita e tratado por EXIBICAO. Depende da decisao do dono sobre a D5. **Bloqueia/altera:** `DLG-16` (a conferencia de prefixo comum deixaria de ter o que conferir) | `bloqueado` | medido 2026-09-05; impedimento: exige revisao da D5 pelo dono |
