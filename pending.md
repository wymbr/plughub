...
| RET-04 | **R4 — teto do ciclo por CONTADOR de iteracoes** (familia do `max_iterations` do `receive`). ⚠️ **Nao pode ser tempo**: `session_timeout` e por INATIVIDADE e um ciclo ativo nunca bate nele. ⚠️ E nao e teto de licenca — medido: o ciclo **nao cria custo novo**, so permite que o contato dure mais; a licenca ja era retida por sessao aberta | `aberto` | destravada pela RET-02 — e **vem ANTES da RET-05**: e a RET-05 que liga o ciclo |
| RET-05 | **R5 — `menu_resolucao` e `menu_continuar` viram questions de continuacao**, e o `navigation_pools` ganha os prefixos (D5). E a metade da CTR-04 que ficou parada por nao haver dono para receber a pergunta. ⚠️ `sac.menu_motivo` **NAO entra**: fica por decisao, porque `sac_ia` tem porta propria de canal | `bloqueado` | impedimento: **RET-04** (ligar o ciclo sem teto e laco sem fim) |
