...
|---|---|---|---|
| MOD-04 | **G3 — revogar `config.permissions`** de quem o censo apontar, pela API oficial. ⚠️ **Repontada em 2026-09-08**: era `bloqueado por MOD-03`, que fechou; o bloqueador passa a ser a MOD-02, e o motivo muda — não é mais risco de tirar a contratação (a emenda decidiu que ela vem do guard de rank), é coerência: revogar antes do guard existir deixa o delegado sem caminho nenhum. Medido em 2026-09-08: **5 portadores**, e **3 com papel `supervisor`** (contas de probe). ⚠️ **Exige BACKFILL antes, e ele nasceu da MOD-08:** o preset do supervisor ganhou `approvals.decide`, `approvals.operacao` e `evaluation.contestar`, mas preset e de NASCIMENTO — quem já existia não os recebeu. Revogar `config.permissions` de um supervisor sem esses três o rebaixa de master a delegado **incapaz de contratar operador**, e o sintoma seria "a tela parou de deixar". Conferir com a classe C do `probe_config_permissions_census.sh` | `aberto` | ADR fase G3 |
