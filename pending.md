...
| GAT-03 | **Metade (a) FECHADA em 2026-09-07; restam os 8 do conjunto NÃO TRIADO.** Os 5 que viviam DENTRO do manifesto — e que por isso faziam o `run_gates.sh` reprovar sempre, ensinando todo mundo a ignorar o vermelho — foram triados um a um: `gate_supervisor_tenant_guard` (duas premissas mortas: o principal irrestrito que a AUT-15 removeu, e a testemunha `noauth` desarmada pelo próprio shim do arquivo) · `probe_nav_backend_field_agreement` (o ramo de cobertura acusou `config.context_map` sem classificação — e classificá-lo revelou que o campo é função de **(namespace, KEY)** desde a ALW-03) · `probe_role_preset_on_create` (o seed do demo ficou atrás do `role_defaults`) · `gate_sla_segment_target` e `gate_queue_report_per_wait` (os dois pela MESMA raiz: *ver o tenant inteiro enumerando o REGISTRY não é ver todas as LINHAS do ledger*). **Quatro eram defeito do INSTRUMENTO, um era do seed, e um defeito de PRODUTO caiu de lado** — o `PUT /config` descartava o `?tenant_id=` e escrevia no `__global__` com 200. **Restam (b), os 8 do NÃO TRIADO:** `probe_report_row_scope` · `probe_seed_drift_named` · `probe_resume_outlives_meta` · `probe_open_segments_closed_sessions` · `probe_llm_call_paths` · `probe_contacts_count_internal_pools` · `smoke_formfill_renderer` · `probe_mcp_permissions_producer` | `aberto` | `CHANGELOG.md` § 2026-09-07 (11) · `infra/test/gates.manifest` |
