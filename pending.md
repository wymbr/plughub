...
| RET-17 | **O supervisor de tasks de BOOT continua copiado em 4 serviços, agora que existe uma lib para o irmão dele.** A RET-13 copiou `_supervise`/`_supervisionar` em channel-gateway, analytics-api, evaluation-api e workflow-api, com argumento medido: a regra de *"um verificador"* vale para quem DECIDE (o JWT, em `py-authz`), e um alarme não decide nada. A RET-16 não refutou isso — refutou o NÚMERO: quando o helper de tarefas efêmeras chegou a seis consumidores, a cópia virou dívida e ele virou `packages/py-tasks`. O de boot tem quatro, e a casa natural dele é a mesma lib. ⚠️ **Não foi feito junto de propósito:** mover agora seria refatorar quatro serviços onde nenhum defeito foi medido, e *simetria* não é razão medida — os dois eixos já são cobrados por `probe_background_task_supervision.sh` (§ B e § C), que mede COMPORTAMENTO e não origem do símbolo. Gatilho: o quinto serviço que precisar de supervisão de boot, ou a próxima vez que alguém tocar naqueles quatro `lifespan` por outra razão | `aberto` | medido 2026-09-09 (RET-16) |
