...
| RET-11 | **A imposição do prazo de suspend lê o cache, e o prazo durável existe e ninguém o consulta.** Medido em 2026-09-09, a partir da pergunta do dono sobre comandos manuais de processo: **212 sessões `suspended` com `closed_at` NULL**, a mais antiga de 10/08, contra **zero** tokens em `*:resume_tokens`. Dessas, **160 já venceram o prazo e ninguém agiu**. A cadeia: o `suspend` grava token+prazo no Redis **e** uma linha durável em `analytics.session_transitions` (680 linhas, **todas com `resume_expires_at`**); o deploy roda `redis-server --save ""` com `appendonly no`, então qualquer restart apaga os tokens; e `_scan_expired_resume_tokens` varre **`*:resume_tokens` no Redis** — sem token, o scanner não enxerga nada e o prazo durável nunca é lido por quem poderia agir. ⚠️ **O conserto NÃO é ligar persistência**: o registro durável já existe e já é escrito. É a IMPOSIÇÃO que lê o store errado. O alvo é o scanner passar a resolver por `session_transitions` (`resumed_at IS NULL AND resume_expires_at < now()`), com o Redis como caminho rápido — e `handle_resume` reidratando o hash a partir da linha durável, padrão que o `resume_meta` já estabelece. ⚠️ **Nem o `force-complete` alcança esses 160**: o estado 1 dele exige o item no ledger, então sem token e sem pipeline ele responde 404 — botão manual nenhum resolveria | `aberto` | medido 2026-09-09 |
| RET-12 | **Suspensão sem `resume_token` no evento não deixa registro durável — 49 sessões sem rastro.** Das 212 encalhadas, **49 não têm linha em `session_transitions`**, e **41 delas são `limite_entrega`**, espalhadas por 11 dias (as outras 8 são `formfill_demo_ia` de 10/08). Causa medida em `analytics-api/models.py:438`: a linha só é emitida `if payload.get("resume_token")` — evento sem token produz **zero** linha, e o `_log_missing_token` ao lado é a única testemunha. Consequência: essas 49 são invisíveis para qualquer varredura baseada no registro durável, inclusive a que a RET-11 propõe. É preciso medir QUAL caminho de suspensão o `limite_entrega` usa (a hipótese é `delegate`, que tem ciclo de token próprio desde a CTR-06) e então: ou o emissor passa a carregar o token, ou a ausência vira barulho, nunca silêncio | `aberto` | medido 2026-09-09 (RET-11) |
