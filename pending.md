...
| DUR-01 | **`menu` durável: suspend-resume em vez de BLPOP em processo.** ⚠️ **NÃO é fase da ORQ** — o id e o mérito são próprios; mora sob este grupo porque foi a **D11 deste ADR** que o mediu e o deferiu. Medido em 2026-09-06: o `pipeline_state` É durável (escritor único `PipelineStateManager`, salvo a cada transição concluída), o engine **retoma do `current_step_id`** em vez do `entry` (com teste), e `task`/`delegate` são idempotentes por `job_id` persistido — **o que NÃO sobrevive é a ESPERA**: `menu` faz BLPOP em processo, **não** chama `ctx.saveState` antes de bloquear (só `catch`, `collect` e `delegate` chamam), e o bridge apenas faz LPUSH em `menu:result:{sid}:{iid}` — **ninguém reinvoca `run()`**. Morto o processo, a resposta do cliente fica na lista até o TTL. O modelo certo **já existe** no par `collect`/`suspend`/`delegate` (salva → `__suspended__` → evento externo reinvoca). ⚠️ Três consequências que fazem disto arco de PLATAFORMA e não de feature: **(1)** atinge todo skill de agente (17 usam `menu`); **(2)** pela Arc 19 `suspend` devolve o agente ao pool, então liberaria a **licença de IA entre turnos** — ganho de capacidade, mas mexe na semântica de ocupação que a § Operational Visibility publica; **(3)** exige que o bridge REINVOQUE em vez de LPUSH, invertendo o contrato que ele declara hoje. **Ordem:** se virar prioridade, vem **antes da ORQ-04** — senão o orquestrador nasce no modelo antigo e é migrado duas vezes. **Gatilho para ganhar ADR próprio:** a decisão de priorizá-lo | `adiado` | gatilho declarado; ADR § D11, medido 2026-09-06 |
