...
| FMT-10 | **Veredicto estrito sem afordancia e HOSTIL — fixtures vivas so ganham `format` na F4.** Descoberto ao tentar a demonstracao natural da F2: `dialog_limite_solicitacao` tem `numero_cartao`, `vencimento_cartao` e `limite_solicitado` sem regra nenhuma, com contraparte pronta no catalogo (`credit_card`, `card_expiry`, `decimal`). Ligar a forma estrita agora recusaria quem digitasse `4539148803436467` ou `1226` — entradas que a pessoa nao tem como saber que estao erradas, porque **nada a guiou**. E a §D7 mordendo na pratica: as duas metades do catalogo sao para ser ligadas JUNTAS, por superficie. Tarefa: aplicar `format` (ou `masked: "credit_card"`, que o deriva pela D8) aos tres campos **no mesmo passo** em que a mascara de digitacao chega ao Console. ⚠️ `numero_cartao` hoje nao e mascarado e viaja em claro para `pipeline_state` — e fixture de demo, entao nao ha vazamento, mas e o shape do defeito e o melhor argumento da D8 | `aberto` | ADR §D7, §D8; `CHANGELOG.md` § 2026-09-04 F2 |
...
| FMT-08 | **O editor de DialogForm so consegue emitir `masked: true` — a declaracao ANONIMA que o `adr-masked-typed-declaration` existe para abolir.** `DialogFormsPage.tsx:771` e um checkbox (`masked: e.target.checked || undefined`), e `true` significa o tipo `opaque` (*mascarado sem tipo*). O schema suporta a referencia tipada desde a T1 (`MaskedDeclarationSchema = false | string`) e ela E usada — medido 2026-09-04: **1 declaracao viva e ela e TIPADA** (`dialog_limite_solicitacao`, campo `cvv` = `card_cvv`), zero booleanas. Mas veio de JSON semeado a mao: **nenhuma forma autorada na tela pode ser tipada**. *Afordancia ausente nao e neutra — ela escolhe o default*, e aqui o default escolhido e justamente o que o ADR anterior queria eliminar. Vira: seletor de tipo com `opaque` como opcao NOMEADA. Junto entra a recusa de conflito no publish (D8): `masked: "cpf"` + `format: "date_br"` e absurdo que o schema de hoje aceitaria, e escolher vencedor em silencio e como se paga a diferenca depois | `aberto` | ADR §D8; `adr-masked-typed-declaration.md` |
