...
| RET-14 | **Mutirão único: encerrar as 212 sessões que já estão `suspended` sem endereço.** A RET-11 deu registro durável ao parque e reidratação do endereço, mas **só alcança o que nasceu depois dela** — o consumidor é `auto_offset_reset='latest'` de propósito, porque reprocessar o histórico produziria parques que nascem mortos (o Redis daquelas sessões se foi inteiro, `pipeline_state` incluído). ⚠️ **Não é retomada, é encerramento honesto:** essas sessões não têm a que voltar, então fechar tem de dizer isso — **não** pode reusar `flow_complete` (o flow não completou) nem `session_timeout` (que já tem dono: o `orchestrator-bridge` o usa para timeout de TRANSPORTE, `main.py:3830`). Precisa de valor próprio no domínio de `close_reason`, que é decisão a tomar. ⚠️ **E a ação tem de ser GRADUADA:** N sessões virando inalcançáveis ao mesmo tempo é sinal de falha de infraestrutura, não de N processos vencendo — encerrar 200 de uma vez apagaria o incidente com a ferramenta que deveria denunciá-lo. O mutirão roda em dry-run primeiro e reporta antes de escrever | `aberto` | medido 2026-09-09 (RET-11) |
