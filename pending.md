...
| IDN-10 | **O índice Redis aponta âncora para um cliente que o cadastro não reconhece — e o Lookup 1 lê o Redis PRIMEIRO.** Medido em 2026-09-13 ao escrever a leitura da procedência (IDN-07). Dois caminhos: **(1)** a identidade progressiva do `resolve_or_provision` anexa ao vencedor, no Redis, as âncoras que eram MISS **no Redis** — sem perguntar ao cadastro, onde a mesma âncora pode pertencer a outro cliente (o comentário diz *"âncoras que apontam a OUTRO cliente NÃO são tocadas"*, e só confere o Redis); **(2)** a reidratação do caminho `durable` grava no Redis **todas** as âncoras da chamada apontando para o vencedor, inclusive as que o PG atribui a outro. Efeito: o resolve devolve o cliente ERRADO para aquela âncora até o TTL do índice (30 d), e é esse `customer_id` que o Lookup 2 (pendências) e a retomada usam. ⚠️ **A confiança já não vaza por aí**: a leitura da procedência confere o cliente e loga a divergência (`IDN-10` no log) — o que resta é a identidade errada, não a autoridade emprestada. **Conserto:** nos dois caminhos, âncora com dono diferente no cadastro não é anexada; e o log de divergência passa a contador | `aberto` | `identity/index.py` `resolve_or_provision` · `CHANGELOG.md` § 2026-09-13 (6) |
