...
| DLG-21 | **A F6 mediu, e mediu que o Console estava ATRAS do build — falta re-medir com a tela certa.** O contato real de 2026-09-05 17:03 atravessou a cadeia inteira e emitiu 2 eventos com a forma certa (`pool.skill.metrica...`, `l1..l4` decompostos, classificacao virando `outcome: resolved` no segmento), mas o VALOR gravado foi **pasta, nao folha** — a imagem do `platform-ui` era das 09:32 e a F3 (colunas Miller + guarda D7) so entrou depois. Provado por controle positivo no bundle servido: `maskedRefused` aparece 3x, `leafRequired` e `options_tree` **zero**; do outro lado, o `@plughub/schemas` do mcp-server rodando TEM `options_tree` e o `sanitizeCategoryPath` da F4, entao um caminho completo teria chegado com os pontos intactos. **O que falta:** `docker compose build platform-ui && up -d platform-ui`, e um SEGUNDO contato descendo ate folha em `motivo` e marcando 2+ folhas dentro de uma pasta em `servico` — so ai a contagem por prefixo da D10 e conferivel contra o que se quis dizer. ⚠️ Enquanto isso, os 2 eventos de 17:03 sao **dado de pasta numa serie de folha**: mesma familia do `_a_b_` que a F2 removeu. **Gatilho:** rebuild do Console (decisao do dono — derruba a sessao de quem estiver na tela) | `aberto` | medido 2026-09-05, `agent_business_events` do contato `4db6dbff` |
