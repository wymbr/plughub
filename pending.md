...
| id | tarefa | status | referencia |
|---|---|---|---|
| ORQ-01 | **F1 — o eixo de DEMANDA passa a existir.** ⚠️ **PARCIAL em 2026-09-06: o mecanismo esta pronto, ALCANCAVEL e verificado ponta a ponta; falta o contato REAL.** Entregues: `optionsAtPath` em `@plughub/schemas` (projecao de UM nivel, pura, com o controle negativo — segmento desconhecido devolve vazio, nunca a raiz) · tool `dialog_tree_level` · forma `dialog_navegacao_atendimento_v1` publicada (9 folhas, 2 niveis) · `skill_navegacao_v1.yaml` **promovido no pool `demo_ia`** (slot `current`, `previous = skill_navegacao_v1` da versao anterior) · tool `agent_event_record` (resolve a ORQ-06) · gate `probe_orchestrator_tree_nav.sh` com **5 ramos**. **Medido ao vivo em 2026-09-06, com sessao sintetica:** o tool emitiu, a categoria saiu `demo_ia.navegacao.destino.sac.info_plano` com o l1 vindo do META da sessao (o chamador nunca disse `demo_ia`), a linha chegou ao ClickHouse, e **a lente de arvore a desenhou nos tres niveis** (`depth` 3/4/5, `derived_leaf` na folha) — a afirmacao do ADR de que a lente existente cobre esta serie deixou de ser leitura de SQL. A linha sintetica foi APAGADA depois, para nao poluir a arvore do teste real. ⚠️ **Falta o que da nome a fase:** um contato real no `demo_ia` (webchat). Ate la o eixo tem mecanismo e **zero** medicao — dizer que ele existe seria o *"existe != esta pronto"*. ⚠️ Achado que virou ramo de gate: o carimbo de epoca sao `tags['dialog_form_id']` + `tags['dialog_form_version']`, e a primeira versao deste skill escrevia `form_id` — nada fica vermelho, o evento so nasce **sem epoca para sempre** e a arvore devolve `single_vocabulary: false` sem saber dizer por que | `aberto` | ADR F1; `CHANGELOG.md` § 2026-09-06 (3) e (4) |
