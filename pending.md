...
| AUT-38 | **17 rotas do `mcp-server-plughub` gateiam por PAPEL literal, e o gate julga por `roles[0]`.** `requireJwtRole` (`server.ts:921`) lê `payload["role"] ?? roles[0]` — o primeiro é claim que **não existe** (a auth-api emite `roles`, array), então decide sempre pelo PRIMEIRO papel: **a autorização depende da ordem em que os papéis foram digitados**. Medido: `admin@` tem `{admin,developer}` e passa por sorte de ordenação; invertido levaria 403 nas duas rotas `["supervisor","admin"]`. São as rotas do Console/Agent-Assist (`work_queue/*`, `agent_done`, `menu_submit`, `conversation_history`, `agent-pause/resume`, `force-complete`…). ⚠️ **Não são a "cauda de papel" fechada em 08-27** — aquela tabela lista **bypasses** (papel deixando pular o grant); estes são **portões nunca migrados**, e caem entre os três censos (o do verificador é `--include='*.py'`; o de menu casa entrada→campo, e rota sem campo ABAC não entra na população; o de cobertura de rota mediu a analytics-api). ✅ **O alvo já existe e está ÓRFÃO**: `agent_assist.atender` (15 rotas) e `agent_assist.supervisionar` (`force-complete`, `work_queue/expire`) são campos declarados sem consumidor nenhum. ⚠️ É **troca, nunca remoção** — tirar o `requireJwtRole` sem pôr campo no lugar deixa as 17 só com credencial. ⚠️ **Se o corte #1 da MOD-05 produzir `contacts.atender`, esta ficha e a MOD-05 são a MESMA obra** e devem ser fundidas, senão o catálogo fica com dois campos para o mesmo fato | `aberto` | medido 2026-09-08 |
| AUT-39 | **`config.users` é `scopable: false`, e as rotas de usuário não recortam.** `GET /auth/users` (`router.py:390`) lista o tenant INTEIRO sem filtro; `PATCH /users/{id}` só protege alvo **privilegiado** (`_assert_may_touch`); e `password` **não** é campo de capacidade, por decisão declarada. Somados: um supervisor com `config.users` **lista, edita, desativa e reseta a senha do operador de qualquer outro supervisor** — e, com a senha, entra na conta. Não é escalação (o alvo não está acima), é **tomada lateral entre times**, e não existe eixo onde declarar *"administra estas pessoas"*. Duas saídas: **(a)** `config.users` vira `scopable` por pool (alvo editável sse `pools(alvo) ⊆ pools(aplicador)`) — usa dado que já existe, mas exige decidir o INDETERMINÁVEL com censo antes (usuário com pools de dois supervisores fica sem administrador único; precedente: 10 de 947 no analytics); **(b)** `agent_groups` do Arc 9, que é literalmente o organograma — medido **0 grupos, 0 membros**, greenfield. ⚠️ A AUT-22 faz esta pergunta sobre o campo **vizinho** (`config.permissions`), não sobre este | `aberto` | medido 2026-09-08 |
| AUT-40 | **Remover `write_only` — em DOIS passos, nunca por deleção.** Zero domínios o oferecem no catálogo e **zero grants** o usam (128 grants vivos: 106 `read_write`, 22 `read_only`), mas **dois call sites de produção** passam `min_access="write_only"` e `abac_can` **levanta `ValueError`** para valor desconhecido — removê-lo do rank primeiro vira **500, não negação**. Ordem: migrar `channel-gateway/auth.py:47` (seguro por equivalência — o domínio de `approvals.decide` é `[none, read_write]`, logo rank ≥1 e ≥2 selecionam o mesmo conjunto) e `channel-gateway/main.py:1621` (**campo DINÂMICO**, vindo de `session.resume_abac` declarado pelo autor do workflow: a equivalência **não** é garantida e precisa ser medida antes); só então remover do `ACCESS_RANK`, do tipo TS, dos domínios e do locale. ✅ A escrita já é segura: `validate_module_config` (`db.py:673`) recusa `access` fora do domínio com **422**, então a remoção não deixa órfão nascer. ⚠️ São **7 cópias** da tabela de rank — a canônica, 4 em TS (`agent-registry`, `mcp-server-knowledge`, `mcp-server-plughub`, `platform-ui`) e 2 em `infra/test/` — e a da UI (`permissions.ts:23`) é **INDEXADA**, ou seja `write_only > read_only`: é a "divergência 2" que o cabeçalho do `py-authz` dá por fechada, fechada em Python e viva na UI. E a prosa de 6 docs afirma a ordem TOTAL que o canônico não implementa | `aberto` | medido 2026-09-08 |
| AUT-42 | **Os 44 rótulos de campo do ABAC não passam por `t()`.** O nome do MÓDULO resolve por i18n com fallback (`t('moduleNames.<id>', { defaultValue: mod.label })`, `ModulePermissionForm.tsx:191`); o rótulo do CAMPO é `{schema.label}` **cru da API** (`:131`), em português, vindo do `infra/modules.yaml`. Não existe `fieldNames` em locale nenhum. ⚠️ **A paridade EN × pt-BR não detecta** — está "perfeita" porque não há o que comparar, mesmo cego que o `CLAUDE.md` registra para chave duplicada. Conserto barato e com padrão já pronto na linha 191: `t('fieldNames.<module>.<field>', { defaultValue: schema.label })`, com o YAML virando fallback e documentação | `aberto` | medido 2026-09-08 |
| AUT-43 | **O aviso de escopo vazio da AUT-03 NÃO É EMITIDO, e o guarda de pool órfão está anestesiado por uma fixture.** ⚠️ **Ficha reescrita DUAS vezes em 2026-09-08, pelas duas correções do dono — e o que sobrou não é o que eu tinha reportado.** Descartado: *"o seed não foi migrado"* (o `seed_auth.py:225` está **certo** — pool é dado do TENANT, criado dinamicamente, e um seed de plataforma não pode inventá-lo) e *"falta um momento de provisionamento"* (os pools do admin são atribuídos **pós-criação**, operacionalmente, e isso já existe). O `admin@` com `{}` é install não provisionado, não defeito. **O que sobrevive são dois instrumentos quebrados, ambos medidos:** **(1)** a AUT-03 declara no `done.md` que *"o caminho vazio não ficou mudo: virou `logger.info` que nomeia a origem"* — e ele **nunca chega a log nenhum**: no `analytics-api`, `logging.getLogger("plughub_authz")` tem nível efetivo **WARNING**, zero handlers, root sem handler, `isEnabledFor(INFO) = False`. Pior, é assimétrico: o ramo do regime ANTIGO é `logger.warning` e sairia; o do regime novo é `logger.info` e não sai — o aviso emudeceu exatamente quando o ramo que ele descreve virou o vivo. Promessa sem mecanismo, na forma mais barata: a linha existe no fonte e é descartada na emissão. **(2)** o `orphansAfter` da tela de Access só não acusa nada porque **`probe@`, fixture de gate, carrega os 41 pools** (+2 que já não existem); excluindo fixtures, **36 de 41 pools não têm vigia**, e os únicos escopos reais são `operator@` (3) e `supervisor@` (2). Guarda que lê verde por estado de teste não é peça load-bearing — e a E5 do ADR se apoiava nele | `aberto` | medido 2026-09-08 |
...
|---|---|---|---|
| MOD-04 | **G3 — revogar `config.permissions`** de quem o censo apontar, pela API oficial. ⚠️ **Repontada em 2026-09-08**: era `bloqueado por MOD-03`, que fechou; o bloqueador passa a ser a MOD-02, e o motivo muda — não é mais risco de tirar a contratação (a emenda decidiu que ela vem do guard de rank), é coerência: revogar antes do guard existir deixa o delegado sem caminho nenhum. Medido em 2026-09-08: **5 portadores**, e **3 com papel `supervisor`** (contas de probe). ✅ **BACKFILL FEITO para o `supervisor@` em 2026-09-08**, pela API oficial (`infra/scripts/backfill_preset_fields.py`, dry-run por default, nunca rebaixa, idempotente — 2ª execução: *"nada a fazer"*). Ele existia porque o preset da MOD-08 ganhou três campos e **preset é de NASCIMENTO**: revogar `config.permissions` sem eles rebaixaria o supervisor a delegado **incapaz de contratar operador**, com o sintoma *"a tela parou de deixar"*. Verificado por SIMULAÇÃO da própria revogação: sem a chave-mestra, `e_master=False` e ele **ainda contrata operator e ainda dá o próprio pool**. Censo: 3A/5C → **4A/4C**. ⚠️ **Os 4 que sobram em C são FIXTURES** (`aut01probe@`, `probe_rowscope@`, `useradmin@`, `navprobe@`) e foram deixados de propósito: a config deles é montada pelos próprios probes, e completá-la muda o que aqueles probes medem — o `navprobe@` nem é caso de backfill (é `operator` sem campos que o preset de operator SEMPRE declarou, ou seja fixture mínima deliberada). ✅ **As 3 fixtures supervisor foram backfilladas em 2026-09-08**, por decisão do dono — censo dos três campos: 4A/4C → **7A/0B/1C** (o 1 é o `navprobe@`, que não é caso de backfill). ⚠️ **O controle que eu propus não pôde rodar:** os dois probes que consomem essas contas (`probe_report_row_scope.sh`, `probe_config_permissions_split.sh`) saem INCONCLUSIVO por `jq` ausente, e não há `jq` em container nenhum — o risco de ter mudado o que eles medem segue **não verificado**, e a reversão é exata (tirar os 3 campos daquelas 3 contas). ⚠️ **O que sobra decidir:** simulada a revogação, três supervisores PODEM contratar operator e **`useradmin@` continua BLOQUEADO** — faltam-lhe **9** campos do preset, porque ele nunca teve o preset de supervisor: é a fixture MÍNIMA do `probe_config_permissions_split`, cujo assunto é *"administra pessoas e não pode conceder"*. Completá-lo mudaria o sujeito daquele probe | `aberto` | ADR fase G3 |
...
| MOD-10 | **`developer` → `devops`: rename DECIDIDO na E3, adiado por SEQUÊNCIA.** O preset atual de `developer` é de *autor de fluxo* (`skill_flows.*` + `config.context_map`), papel que os DialogForms tornaram obsoleto; o que existe na operação é infraestrutura. ⚠️ **Não é edição de catálogo — a superfície foi levantada em 2026-09-08 e cruza TRÊS pacotes:** `infra/modules.yaml` (5 campos) · `auth-api` (`Role` Literal + `_ROLES_VALIDOS`) · `seed_auth.py` · **7 listas de `requireJwtRole` no `mcp-server-plughub`** · e o `platform-ui` em 7 arquivos (`types/index.ts`, `AccessPage`, `DashboardsPage`, `RoleDefaultsModal`, `AgentFlowDeployPage` — que gateia por `r === 'developer' || r === 'admin'` —, `MaskingPage`, `ContextMapPage`) mais os locales. **O gatilho é a AUT-38**: aquelas 7 listas de `requireJwtRole` são exatamente as que ela vai REMOVER ao migrar os portões de papel para campo ABAC — renomear antes é editar duas vezes o que vai sumir. Custo do dado: **1 linha** (`admin@`, que também tem `admin`, então nada muda para ele; o `upsert_user` do seed manda `roles`, logo re-semear migra). O par `admin → developer` em `hiring_pairs:` é renomeado junto, e o comentário ao lado dele já diz isso | `bloqueado` por AUT-38 | ADR § Emenda 2026-09-08 E3 |
| MOD-09 | **G2 — rota de apply-template + PROVENIÊNCIA carimbada.** Capacidade vem do template, nunca do corpo (D3 sobrevive). Medido em 2026-09-08 que hoje é **cópia pura**: `auth.users` não tem coluna de template, o `applyTemplate` é client-side, a rota `POST /templates/{id}/apply` saiu em 2026-08-30 e existem **zero templates** na instalação. ⚠️ O modelo de *"a referência sobrevive se nada foi alterado, exceto pools"* foi **RECUSADO**: torna o template política viva para um subconjunto imprevisível; a exceção dos pools o faz sobreviver justamente à edição que mais quebra a equivalência; e o resultado da comparação é invisível na tela. Fica `created_from_template_id` + hash da config, **imutáveis e nunca consultados para autorizar** — padrão do `deploy_version`: carimbo, não ponteiro. E o **template não carrega pools**: eles vêm do aplicador, dentre os dele — assim um template serve todos os supervisores e contratar para pool alheio fica impossível por construção | `aberto` | ADR § Emenda 2026-09-08 E4 |
