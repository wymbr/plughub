...
| APR-11 | **Um `suspend reason: approval` é decidido SEM credencial pela rota interna de resume.** Medido na PID-17 (2026-09-14): `POST /v1/channels/webhook/resume/{token}` com `{"decision": "approved"}` e nenhum header respondeu `200` e retomou o `solicitar_operadora` do `skill_portabilidade_demo_v1`. A AUT-46 exige Bearer humano para retomada de tarefa que DECLARA capacidade (`approvals.decide`, o `delegate` de aprovação do limite); o `suspend` puro não declara, e passa a valer o modelo de *capability* do token. Pode ser o certo (quem aprova ali é a OPERADORA, sistema externo, e a porta externa de resume tem o mesmo modelo) ou a mesma espécie errada da APR-09 no sentido inverso — decidir com o dono se `reason: approval` deve exigir principal. `_portabilidade_door_exercise.sh` depende do comportamento de hoje e diz onde apresentar credencial | `aberto` | PID-17 · AUT-46 · APR-09 |
