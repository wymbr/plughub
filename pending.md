...
|---|---|---|---|
| ALW-01 | **V4** — inverter o default: campo sem regra deixa de ser acessível. **Dependências de tarefa LIBERADAS** (ALW-02/03/04 fecharam em 2026-09-02). Pré-condição MEDIDA no mesmo dia, com a série cortada e tráfego real (`smoke_limite_tres_acessos`, 18✅/0❌): `overflow = 0` ✓ e **`unknown = 1`** — e o 1 é `session.preview`, que é a **decisão aberta #6 do ADR** (política de mascaramento guardada como valor de tag), não descuido. Consequência medida: **uma** skill a declara (`skill_limite_processo_v1.yaml:104`, dentro de `delegate.context`), então sob a V4 ela não passaria no publish. Bloqueio real = fechar a #6, que é decisão curta | `bloqueado` pela decisão #6 do ADR | `TODO.md:381` |
