...
| IDN-12 | **Nenhum consumidor do Lookup 1 respeita `matched_by="ambiguous"` — e um deles entrega `resume_token`.** Com âncoras de dois clientes no mesmo score, o resolve devolve `ambiguous` com o `customer_id` do PRIMEIRO candidato; o contrato diz *"o fluxo decide 'ask'"*. Medido em 2026-09-13: **`pending_workflow_get`** (`workflow.ts:419`) só testa `customer_id` e `verification_class` — se o primeiro candidato for `possessed`, devolve as pendências e o token de retomada de um cliente escolhido arbitrariamente; e as três gravações de pendência do webhook (`handle_delegate`, `collect`, conferência) gravam sob ele. ⚠️ **Não é regressão da IDN-11**: antes o caminho frio escolhia pela ORDEM das âncoras, igualmente arbitrário e sem sequer dizer; hoje a ambiguidade é declarada e ninguém lê. Exposição de hoje não medida (depende de chamadas com âncoras de clientes diferentes no mesmo score). **Conserto:** os consumidores tratam `ambiguous` como não-resolvido — `pending_workflow_get` devolve `found: false` com `ambiguous: true`, e a gravação de pendência não escolhe dono | `aberto` | `workflow.ts:419` · `webhook.py` `_resolve_pending_customer` · `CHANGELOG.md` § 2026-09-13 (8) |
