...
| RET-16 | **Os outros 22 disparos efêmeros sem dono — medidos na RET-15, fora do escopo dela por serem de outros serviços.** O censo do repositório inteiro achou `asyncio.create_task(...)` como STATEMENT em **routing-engine (17)**, rules-engine (2), ai-gateway (1), evaluation-api (1) e conversation-writer (1, pacote fóssil em quarentena — este NÃO deve ser religado, ver `CLAUDE.md`). ⚠️ **Quatro deles perdem TAMBÉM a exceção** (o corpo da corrotina não tem `try` pega-tudo), e os quatro são do routing-engine: `evaluation_consumer.py:81` (`_dispatch`), `kafka_listener.py:355` (`_refresh_pool_snapshots`), `kafka_listener.py:362` (`_drain_queue_for_agent`) e `main.py:199` (**`_process_message`** — o processamento de mensagem do roteador). Os outros 18 têm `try` interno e sofrem só o segundo dano: sem referência forte, o CPython pode coletar a task no meio da execução. ⚠️ **A dívida está DECLARADA com teto** em `infra/test/task_ownership.debt`, e o gate reprova quem passar do teto — deixar o gate cronicamente vermelho ensinaria a ignorá-lo. Trabalho: levar `disparar()` (hoje em `channel-gateway/tarefas.py`) aos serviços, começando pelos quatro graves, e baixar o teto a cada migração até removê-lo | `aberto` | medido 2026-09-09 (RET-15) |
