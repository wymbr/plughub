...
**PID-01, PID-02, PID-03, PID-07 e PID-06 fechadas em 2026-09-13** — o token de retomada só sai para quem provou nesta sessão o que a pendência exige; falta a mesma exigência na RETOMADA (PID-13). **PID-10 depende de IDN-07.** A migração dos dois intakes (PID-04) vem
...
| PID-13 | **A exigência de retomada vale só na LIBERAÇÃO do token, não na retomada.** A PID-06 fez o `pending_workflow_get` reter o `resume_token` de quem não provou nesta sessão; o `workflow_resume` não confere nada — quem já tem o token (link de webchat, D10; ou token vazado) retoma sem prova. Aplicar a mesma exigência no `workflow_resume`, aceitando a prova da sessão que retoma OU da continuação direta dela (o filho de `delegate` chama o resume, não quem provou), sem quebrar os 12 skills que usam `workflow_resume` para devolver ao pai (pendência sem exigência passa). Ler a exigência do registro do token (`resume_meta`), que hoje não a carrega. E `resume_door`: por qual pool a retomada entra — não implementado | `aberto` | ADR D6, D7, D10 · PID-06 |
