...
|---|---|---|---|
| ALW-01 | **V4** — e ela **não é a inversão de leitura**: a D9 a redefiniu para *"ligar o **portão de PUBLISH** sobre o cadastro"*, com runtime que nunca rejeita (resolve restritivo e LOGA). As dependências de tarefa (ALW-02/03/04) fecharam e a decisão #6 foi resolvida em 2026-09-02. **A pré-condição foi REMEDIDA em 2026-09-02 e o instrumento estava errado duas vezes** — ver ALW-11 para o conserto. Estado real, contra o mapa VIVO: **plataforma 14 escritas / 1 não declarada** (`core.workflow.reviewer_id`, travada pela decisão #5 do ADR — lacuna de classe LGPD para identidade de usuário da plataforma; e **nenhum portão de publish a alcança**, porque código não se publica) · **tenant 52 escritas / 1 não declarada** (`session.journey_echo`, passo autodeclarado de demo). O `unknown = 0` da auditoria continua válido, mas mede outra coisa: ele observa LEITURA de tráfego que rodou, e a V4 gateia ESCRITA declarada estaticamente. **A ALW-12 fechou em 2026-09-02** e com ela o bloqueio: o mapa do tenant passou a ter provisionamento (`infra/context-map/` + `context-map-seed`), e o pós-`--wipe` deixou de ser 17. **A pré-condição está LIMPA desde 2026-09-02**: o passo `ler_journey_ctx` foi removido (decisão do dono) e o censo passou a medir **tenant 51 escritas / 0 não declaradas** — o portão de publish aceitaria todos os skills hoje. Sobra `core.workflow.reviewer_id`, que **não bloqueia**: é escrita por código (evaluation-api `router.py:2427`, `:2546`) e portão de publish nenhum alcança código; ela é a decisão #5 do ADR (lacuna de classe LGPD para identidade de usuário da plataforma). ⚠️ Antes de virar, **refazer a medição** — ela é estática e barata, e a população de skills muda | `aberto` | `TODO.md:381` |
| ALW-11 | Censo (`infra/test/censo_contextstore_cadastro.py`) media a proposição VIZINHA, e de duas formas ao mesmo tempo — **CORRIGIDO em 2026-09-02**. (1) Lia o mapa **semente** (`DEFAULT_CONTEXT_MAP` do fonte TS, 77/98) e chamava o resultado de "não cadastrado", enquanto o portão consulta o mapa **vivo** (config-api, 97/119): publicava **18 não declaradas onde a resposta era 2**, mandando cadastrar 16 nomes já cadastrados. (2) Somava escrita de CÓDIGO com escrita de SKILL num total só, e o número grande (fixtures descartáveis) escondia o pequeno (código, que carrega peso e que portão de publish nenhum alcança). Hoje o veredicto é **partido por origem**, cada metade julgada pelo mapa certo — plataforma contra a semente (roda para todo tenant), tenant contra o vivo — e o mapa vivo indisponível dá **INCONCLUSIVO nomeando**, nunca queda silenciosa na semente. Falseabilidade provada: config-api inalcançável e `--sem-vivo` reprovam os dois. Resta escolher se isto vira gate ou fica instrumento | `aberto` | ADR §D9 |
| ALW-13 | `journey.origin_process_session` ficou **órfã nas duas pontas**: o escritor (`marcar_journey`, em `skill_journey_demo_v1`) já tinha sido removido, e o leitor (`ler_journey_ctx`) saiu em 2026-09-02. Segue declarada no mapa vivo e em `infra/context-map/tenant_demo.json`. **Não removida por decisão**: tirá-la exigiria tocar o mapa vivo além do arquivo (o ramo C do gate compara semente+arquivo com o vivo), e o nome é um fato de journey plausível que a documentação referencia (`docs/guias/context-store.md:163`). Decidir se vira campo real ou sai | `aberto` | medido em 2026-09-02 |
| ALW-07 | Duas casas para o mesmo `mention_command set_context` (`orchestrator-bridge/main.py` × `mcp-server/tools/bpm.ts`, mesmo `mention_commands` do YAML) — e elas DIVERGEM desde a ALW-02: a TS roteia `journey.*` para o hash do processo, a Python grava no da sessão e avisa. Duas respostas para o mesmo fato significam que a permissiva vale. Decidir qual casa fica | `aberto` | ADR §1.7 |
| ALW-08 | Afordância no EDITOR: mostrar as tags de ctx que o flow aberto usa e quais **não estão cadastradas**, com atalho para `/config/context-map`. É a metade que falta da decisão da ALW-03 — o grant já alcança o autor, mas ele ainda não **vê** o que falta registrar. Medido em 2026-09-02: o editor é um Monaco de YAML (872 linhas) e **não tem noção nenhuma de context tag**, nem valida contra o servidor. A outra metade (o erro de publish dizendo o que registrar) depende da V4, logo é `bloqueado` por ALW-01 | `aberto` | ADR §D9 #2 |
...
| ALW-10 | `display_voice`, `echo_to_customer` e `echo_to_operator`: **zero consumidores**, medido em 2026-09-02. Editáveis na tela de Masking, gravados na config, e ninguém os lê — nem o `channel-gateway`, nem o canal de voz. `display_screen` tem UM (`MaskedToken`). Decidir se são feature futura ou campo a remover; enquanto ficarem, a tela apresenta três interruptores que parecem vivos. ⚠️ E o eixo é **de canal**: eco é conceito de voz, e há canal onde não existe controle de eco (WhatsApp) — então a decisão não é só ligar/desligar, é o que o campo significa onde não se aplica | `aberto` | ADR §D1 (catálogo de tipos) |
| ALW-05 | V5 (metade) — fechar aliases | `bloqueado` — por **tempo**, não esforço: depende do contador decair (**17 alias tags** em 2026-09-02). ⚠️ A série foi CORTADA três vezes naquele dia, para medir a pré-condição da ALW-01. A perda é pequena e está medida — a série anterior era de um burst de 01/09 18:25, ~1 dia, não meses de acúmulo —, mas quem for avaliar o decaimento precisa saber que o relógio recomeçou ali. ⚠️ Os 17 são exatamente as grafias legadas que os skills de demo escrevem (`session.numero_cartao`, `session.limite_solicitado`, …) — migrá-los para as canônicas faz o contador cair por CONSERTO e não por decaimento, e é a mesma ação que a ALW-12 descreve | índice `CLAUDE.md` |
| ALW-06 | D7 (metade) — tela de proveniência: global × override por nó. ⚠️ **Deixou de ser hipótese em 2026-09-02**: um `PUT` de `masking.types` no escopo GLOBAL respondeu **200 e não teve efeito nenhum** — `tenant_demo` tem override daquela chave, e tenant vence global POR INTEIRO (`LIMIT 1`). Só se descobre contando os tipos nos dois escopos (eram 14 × 13). Um `PUT` que responde 200 e não faz nada é pior que um que recusa; a tela precisa dizer QUAL escopo está em vigor antes de deixar salvar | `aberto` | índice `CLAUDE.md` |
