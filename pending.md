...
|---|---|---|---|
| CTR-01 | **G1 — dar MECANISMO a regra de perfil de step.** O schema diz em comentario *"Agents must never use delegate"* e o `CLAUDE.md` (Arc 19) afirma que o perfil e *"validado em parse do YAML + guard no engine"*. **Medido em 2026-09-06: nao existe allowlist de step por perfil em lugar nenhum** — nem no validador do agent-registry, nem no `executor.ts`, nem no `engine.ts`. Promessa sem mecanismo, a familia do DDL de `participation_intervals`. ⚠️ **Vale sozinha e vem ANTES da G3**: decidir usar `delegate` em perfil de agente contra uma regra que ninguem impoe e escolher com base em nada. Precisa de allowlist declarada + gate | `aberto` | ADR G1; medido 2026-09-06 |
