...
| AUT-49 | **Três comentários prometem um poll de 3 s que não existe.** Medido em 2026-09-09, ao verificar a AUT-48 no navegador: o aviso de recusa só apareceu depois de eu REMONTAR a página — o `useSupervisorState` não tem `setInterval` nenhum. Ele busca no mount (+3 retries curtos em 700/1800/3500 ms) e a cada evento de WS (`message.text`, `menu.render`, `supervisor_state.updated`), e nada mais. Mas `ContextoTab.tsx:663` diz *"the 3s polling in useSupervisorState will pick it up"*, `:665` repete *"polls every 3s"* e `ClienteTab.tsx:8` promete *"o próximo poll do supervisor_state re-chaveia"*. ⚠️ **A consequência não é cosmética:** os dois pontos que confiam nisso são write-backs do próprio operador (vincular cliente, gravar tag manual) — casos em que **não há evento de WS**, porque ninguém falou na conversa. Então o painel pode não refletir o que a pessoa acabou de gravar até ela trocar de contato. É a família *"comentário que promete mecanismo sem produtor"*. Duas saídas: **(a)** criar o intervalo (e então medir o custo — é uma chamada por sessão aberta por operador); **(b)** manter event-driven e trocar os três comentários por um `refresh()` explícito no call site do write-back (o hook já expõe `refresh`, e `ClienteTab` já recebe um) | `aberto` | medido 2026-09-09 |
