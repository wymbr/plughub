...
| id | tarefa | estado | evidência |
|---|---|---|---|
| CNS-21 | **O NPS não chega ao cliente: a visibilidade cai para `agents_only` porque o `@ctx.core.contact.customer_participant_id` resolve VAZIO.** Medido em 2026-09-03 em duas sessões (uma ANTES do deploy da CNS-19, logo **não é regressão de hoje**): a mensagem do NPS entra no stream com `visibility: "agents_only"`, e é exatamente esse valor que o filtro do webchat descarta (`stream_subscriber.py:174`). O valor vem do fallback DELIBERADO de `resolveVisibility` (`interpolate.ts:204`), que devolve `agents_only` quando **todos** os itens `@ctx.` do array resolvem vazio — cair em `all` vazaria para todo mundo. O comentário dele já nomeia este caso (*"when @ctx.core.contact.customer_participant_id is not yet in the ContextStore"*). ⚠️ **Consequência medida:** sem cliente para responder, o menu do NPS espera os seus `timeout_s: 30` e só então o contato fecha — o *fecho tardio* é sintoma, não causa. ⚠️ **O que NÃO explica ainda:** a tag **existe** e é canônica no ctx daquela sessão (`core.contact.customer_participant_id` = `cust_…`, `source: bridge:pre_hook`), escrita 60 ms antes de a mensagem entrar no stream. Falta UMA medição instrumentada: rodar o hook com log da resolução, para separar corrida de leitura-na-sessão-errada. Não mexer antes disso | `aberto` | `skill-flow-engine/src/interpolate.ts:204` · `stream_subscriber.py:174` |
| CNS-20 | **A asserção 7b do `smoke_limite_tres_acessos` era VAZIA, e o desarme do pacote pode estar na sessão errada.** Duas coisas, achadas ao consertar a CNS-19 e separadas de propósito. **(a)** O 7b lê `core.workflow.dialog_form_id` para provar que o pacote foi DESARMADO; até a CNS-19 o produtor escrevia `session.dialog_form_id`, então aquele nome **nunca existia** e o `[ -z "$FID" ]` passava por ausência — o defeito que a asserção existe para pegar (o parking se passar por aprovação) estava invisível. Com a CNS-19 o nome passa a existir, mas **a asserção continua sem controle positivo**: ela precisa provar que o pacote esteve ARMADO antes de provar que foi limpo. **(b)** ⚠️ **Suspeita a MEDIR, não conclusão:** o `limpar_form_do_pacote` é um `context_set` que roda na sessão do PROCESSO, enquanto o pacote que o Console lê vive na sessão FILHA criada pelo `delegate` (medido: a sessão que o operador abre traz `core.workflow.origin_session_id` apontando para outra). Se o filho não herdar/refletir a limpeza, o desarme não desarma. Medir antes de mexer — mudar asserção por suposição é pior que deixá-la marcada | `aberto` | `infra/test/smoke_limite_tres_acessos.sh:313` · `skill_limite_processo_v1:232` |
