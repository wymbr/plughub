...
|---|---|---|---|
| MOD-11 | **O módulo `workflows` é órfão inteiro, e as superfícies que ele deveria gatear estão ABERTAS.** Medido em 2026-09-08, ao executar a MOD-06: os seis campos (`operacao`, `visualizar`, `cancelar`, `webhooks`, `journey_read`, `journey_resume`) não têm consumidor nenhum — nem UI nem serviço. O que está aberto, e é o motivo da ficha: **(a)** `POST /v1/workflow/trigger` (workflow-api, porta 3800) **não tem dependência de autorização nenhuma**, e `WorkflowEditorPage` o chama **sem header algum** — quem alcança o serviço dispara qualquer flow; **(b)** `/workflow/editor`, `/workflow/calendar` e `/workflow/triggers` estão em `routes.tsx` **sem `RequireAbac`** e fora do Sidebar, ou seja, alcançáveis por URL por qualquer autenticado (mesma família da AUT-37); **(c)** o CRUD de webhooks usa `_require_admin` com `if settings.admin_token and …` — **vazio libera**, a postura que a § Security nomeia como oposta à aditiva. ⚠️ **Não é corte de granularidade**: dividir um campo sem leitor produziria dois órfãos (ver as-built do corte #1 na D6). O trabalho é decidir entre **ligar** os campos às superfícies (o `webhooks` já existe e é o campo do CRUD) ou **remover** o que a Fase F do Arc 19 aposentou — e os dois `journey_*` nomeiam tools que não existem mais, então para esses a resposta já está medida | `aberto` | medido 2026-09-08 (MOD-06) |
...
| PRM-02 | **O `skill_navegacao_v1` está parametrizado no YAML e AINDA NÃO VALE — falta publicar e deployar.** A `PRM-01` trocou os três `form_id` cravados por `$.config.form_id` e declarou o `config_params`, mas skill é *seed-if-absent* e o pool executa o snapshot do slot: editar o arquivo é no-op. Para valer, três passos nesta ordem — **(1)** `PUT /v1/skills/skill_navegacao_v1` com `x-skill-publish: true` (é isso que leva o `config_params` ao registry, e é o registry que a tela de Deploy lê); **(2)** *Flow › Deploy* no `demo_ia`, escolhendo `dialog_navegacao_atendimento_v1` no combo *Árvore de navegação*; **(3)** `set-next` + `promote`, que levam snapshot e `config_json` no MESMO slot — não existe janela com flow parametrizado e parâmetro vazio. ⚠️ Enquanto o passo 1 não acontecer, o `demo_ia` segue rodando o snapshot antigo com o id cravado, que **funciona** — não há urgência nem risco em esperar. ⚠️ Depois do passo 1, o promote **recusa** até a árvore ser escolhida (é a `PRM-01` fazendo o trabalho dela), então a ordem não é burocracia. ⚠️ O irmão `skill_navegacao_llm_v1` **fica no modelo literal por decisão do dono** (2026-09-08): os dois pools passam a ter modelos diferentes para o mesmo parâmetro, e isso está declarado aqui para não virar surpresa | `aberto` | `CHANGELOG.md` § 2026-09-08 (12) |
