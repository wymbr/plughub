...
| CAP-09 | **As tools da PRÓPRIA plataforma não seguem o modelo da borda — e o `deploy.ts` afirma que seguem.** Censo de 2026-09-01 sobre as **73** tools do `mcp-server-plughub`: permissão **0/73** (há UMA chamada de `judgeInvoke` no repo e é a do próprio `invoke`) · injection guard **16/73** (`withGuard`, só `bpm.ts` e `workflow.ts`) · audit `mcp.audit` **1/73** (o `invoke` é o único produtor do tópico) · `session_token` **ausente em 40**, em 11 arquivos, incluindo `deploy.ts`/`pool_promote`, `bpm.ts`/`conversation_escalate`+`notification_send`, `workflow.ts`(8), `outbound.ts`(5), `work_queue.ts`(3). O caminho nativo confirma: `skill-flow-service.mcpCall` é `client.callTool()` direto. ⚠️ **O agravante é a promessa:** o cabeçalho de `deploy.ts:7-13` afirma que o promote é *"permission-checked, injection-guarded and audited by the McpInterceptor"* — as três falsas (o `McpInterceptor` só existe no exemplo do próprio docstring, nunca é instanciado) —, e a MESMA frase foi copiada para `CHANGELOG.md:18389`. Promessa em duas casas, mecanismo em zero: família do DDL de `participation_intervals`. ⚠️ **NÃO afirmar exposição sem medir topologia:** a porta 3100 está publicada em `0.0.0.0` **neste compose de demo**, e borda é fato de REDE, não de código — falta aqui o equivalente do `probe_edge_surface.sh`, que é o que diria o que o deploy realmente publica. **Primeiro passo é o censo virar gate** (o número por camada, com a lista das isentas DECLARADA, nunca deduzida da ausência — padrão `_SCOPE_EXEMPT`/`_SCOPE_DEBT` da analytics-api); decidir política antes de ter o censo repetiria o erro da D14.1 (confundir exposição com dano) | `aberto` | medido 2026-09-01 |
---
