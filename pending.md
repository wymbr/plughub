...
| ALW-17 | **`echo_to_operator` ficou INERTE, e o campo continua editável na tela.** Desde a MSK-03 (2026-09-10) `none` nao remove mais o campo, e `plain` ja era rebaixado a `masked` pela regra *o tipo aperta* — logo os TRES modos produzem byte a byte a mesma saida, e nenhuma escolha do tenant muda o que o operador le. ⚠️ **A inercia esta fixada como FATO**, nao como prosa: `test_none_e_masked_produzem_a_mesma_saida` reprova se os modos voltarem a divergir sem decisao. O encanamento (a consulta ao catalogo em `main.py`, `masking_types.resolve_echo_operator`, o parametro `echo_policy`) FICA por ser o menor passo reversivel — arranca-lo agora deixaria um campo editavel na `MaskingPage` sem consumidor nenhum, que e a mesma promessa-sem-mecanismo do outro lado. **Duas saidas, e a escolha e de produto:** (a) remover `echo_to_operator` do catalogo e da tela, e com ele a consulta por submissao mascarada; ou (b) dar a `masked` um render PARCIAL vindo do tipo (`***4444` para `credit_card`), o que devolveria a distincao **e** mudaria o que o operador ve — decisao de confidencialidade, nao de codigo. ⚠️ Enquanto nenhuma das duas acontecer, o custo e uma chamada ao config-api por submissao mascarada cujo resultado e descartado. **Gatilho:** o proximo tenant que editar `echo_to_operator` esperando que mude algo | `aberto` | `CHANGELOG.md` § 2026-09-10 (4) |
...
| PUL-05 | **O Console só reencontra um item EM POSSE por uma chave de 300 s, uma por POOL — e o item some da tela do próprio dono.** A reidratação no reconnect lê `pool:pending_assignment:{poolId}` (escrita pelo bridge, TTL 300 s, last-write wins). Item reivindicado que não abre em 5 min — ou que perde a vez para outro contato do mesmo pool — continua em posse (`claim_record`, A5), então **ninguém mais o pega e o dono não o vê**; a única superfície que o mostra é `/monitor/work-items`, que só oferece *Close*. Medido em 2026-09-11: `0596f383` em posse do operador, token vivo, formulário suspenso, **invisível no Console** com 0 chaves `pending_assignment` no Redis; resgatado por `releaseTask` → voltou ao ZSET de `retencao_humano-int` com `assigned_to` preservado. ⚠️ Estava anotado no `TODO.md` como *"melhoria futura"* e **nunca foi fichado** — a frase descrevia a chave, não o dano. **Conserto:** reidratar a partir do LEDGER (`work_task` com `assigned_to` = eu, cruzado com o `claim_record`), que já é o que `/api/work_queue/pending` lê; a chave por pool vira, no máximo, atalho. **Gatilho:** já ocorreu | `aberto` | medido 2026-09-11 (wrap-ups `0596f383` · `5120fe90` · `4841c60d` do operator@) |
| PUL-06 | **Fechar a sessão cancela o TOKEN e deixa o ITEM — o ledger passa a afirmar uma pendência que ninguém pode cumprir.** `cancel_pending_resumes` (consumidor de `session.closed`, RET-03) apaga o `resume_token`, mas `work_task`, `claim_record` e o `pipeline_state` suspenso ficam. Medido em 2026-09-11: `5120fe90` e `4841c60d` fechados 10 s e 68 s depois do claim (`closed=agent_done`), token fora de `resume_tokens`, e os dois exibidos em `/monitor/work-items` como **"Being filled in"** com prazo de ~21 h — valor plausível sobre fato morto. ⚠️ **E o prazo provavelmente nunca dispara:** o scanner de deadline varre `*:resume_tokens` (`webhook.py`, `_scan_expired_resume_tokens`), e o token já não está lá — o item evaporaria pelo TTL sem `acw_expired` nem segmento terminal. *Lido no código, não observado ao vivo: medir quando o prazo de `5120fe90` (2026-09-12 14:20 UTC) passar, se ninguém o fechar antes.* ⚠️ **E o item fica INENCERRÁVEL pela única ação oficial — medido ao vivo no mesmo dia:** o *Close* de `/monitor/work-items` chama `POST /api/work_queue/expire`, que lê o `resume_token` do `work_task` e encerra RETOMANDO por ele (`decision: timeout`); token cancelado ⇒ o gateway responde **404** e a tela mostra *"Could not close the item (HTTP 404)"*. No mesmo clique o `0596f383` (token vivo) fechou e saiu da lista; `5120fe90` e `4841c60d` ficaram. O `work_task_expire` do routing NÃO precisa do token (resolve o dono pela lease → registro → semáforo), então o encerramento sem token já existe uma camada abaixo — falta a rota não depender da retomada quando o token morreu. Dano de capacidade medido: **zero** (o semáforo do operador não retém os dois); o dano é o ledger mentindo e a posse gravada. **Conserto:** a MESMA casa que cancela o token encerra o item (`work_task_expire` com causa própria), pela razão que o docstring do RET-03 já dá — um gancho por lugar, e o esquecido reabre o buraco. **Gatilho:** já ocorreu | `aberto` | medido 2026-09-11 (wrap-ups `0596f383` · `5120fe90` · `4841c60d` do operator@) |
| PUL-07 | **O Console deixa encerrar uma tarefa de FORMULÁRIO sem submeter o formulário — e é assim que a tabulação se perde.** Os dois wrap-ups da PUL-06 morreram porque a tela abriu vazia (CNS-24) e o contato foi fechado: o fechamento é caminho normal de `agent_done`, sem distinguir contato de conversa de tarefa com `dialog_form_id` pendente. A CNS-24 remove a causa de hoje (tela vazia), não a classe: qualquer próxima falha de render repete o dano, **e a tabulação perdida não se recupera** (o token morre no fechamento). **Decisão de produto antes do código:** recusar o fechamento enquanto a tarefa de formulário estiver suspensa, ou pedir confirmação que nomeie a perda, ou converter o fechamento em *Return to queue* (que preserva o token, medido na PUL-05). **Gatilho:** a próxima falha de render de formulário na Console | `aberto` | medido 2026-09-11 (wrap-ups `0596f383` · `5120fe90` · `4841c60d` do operator@) |
...
| AUT-45 | **O `probe_authz_single_verifier` está VERMELHO, e não é da AUT-39 — é anterior.** Achado de passagem em 2026-09-09 ao rodar a vizinhança: o censo C1 encontra **2** implementações independentes do verificador contra uma linha de base de **1** (o EMISSOR, `auth-api/jwt_utils.py`, que fica por decisão). A nova é `analytics-api/pool_auth.py`, que decodifica JWT por conta própria em duas linhas (`:343` e `:423`). O arquivo está limpo na árvore e seu último commit é `4d2f5f5d` (MOD-07), anterior a esta sessão — **não foi introduzida pelo trabalho de hoje**. O próprio probe diz o que fazer: *"se a cópia é deliberada, a linha de base deste probe tem de mudar JUNTO, com o motivo — senão a sétima divergência nasce muda como as seis anteriores"*. Decidir qual dos dois: migrar `pool_auth.py` para `plughub_authz`, ou mover a linha de base declarando por quê | `aberto` | medido 2026-09-09 |
