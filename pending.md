...
| id | tarefa | status | referencia |
|---|---|---|---|
| CTX-03 | **F2 — `by_role.customer` DECLARADO por tipo; a tabela de excecao so para o que sobrar.** ⚠️ **Esta tarefa mudou de forma em 2026-09-04, e a medicao que a mudou vale mais que ela.** Sob o eixo `echo_to_*` a populacao de excecao era **ZERO** — a tabela nasceu vazia e o proprio ramo C do gate a acusou. Sob `by_role` ela e **1**, e nao e excecao: `notificar_aprovado` / `session.limite_aprovado` -> tipo `financial` -> mascara `financial` (`R$ ****,**`), que apagaria o limite que a mensagem existe para anunciar. Causa: `customer` nao e declarado em NENHUM dos 14 tipos, entao o fallback entrega ao cliente a mascara do operador. **Mesmo fallback, um acerto e um erro** (`credit_card` -> `last_4` = o `***4444` correto da tela 2). Logo ele e evidencia boa e aplicador ruim. O remedio e declarar `by_role.customer` por TIPO — grao que a §D6 ja escolheu, mora onde compliance edita, e vale para todo template de uma vez; uma tabela por `(skill, tag)` viveria em codigo e envelheceria por skill. **Vem ANTES da F3** | `aberto` | ADR §D3, §D9.1 |
...
| CTX-07 | **O masker nao e alcancavel a partir do engine — a F3 esta bloqueada nisso.** `applyMaskingTypeToValue` vive em `mcp-server-plughub/src/lib/context-masking.ts` e o gemeo Python em `py-contextstore/masking.py`, com paridade por `probe_masking_display_parity.sh`. O `@plughub/schemas` so exporta o ENUM `ContextMaskingType`, entao o engine hoje consegue decidir QUAL mascara aplicar (`resolveMaskForAudience`, entregue na CTX-01) e **nao** consegue aplica-la. Mover o masker para a casa compartilhada sob o MESMO gate de paridade — nunca uma quarta copia, que e o que o comentario de `_build_pending_preview` existe para impedir | `bloqueado` (bloqueia CTX-04) | ADR §6 |
