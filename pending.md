...
| id | tarefa | status | referencia |
|---|---|---|---|
| ORQ-10 | **A aba Processes lê uma tabela que o Arc 19 esvaziou — 51 processos suspensos existem e a tela diz *"No processes found"*.** Achado pelo dono em 2026-09-09, medido no mesmo dia: `useWorkflowInstances` (`modules/workflows/api/hooks.ts:73`) chama `GET /v1/workflow/instances` → `workflow.instances` no Postgres, que tem **0 linhas**. O produtor daquela tabela é o `POST /v1/workflow/trigger` da workflow-api, e o Arc 19 **substituiu** esse caminho pelo webhook do channel-gateway (o `CLAUDE.md` já declara *"o que é eliminado: workflow-api lifecycle endpoints, WorkflowInstance entidade separada"*) — o INSERT continua no código sem chamador vivo. A verdade dos processos está em `plughub_demo.sessions`: **51 `suspended` + 7 `active`, todas `channel=webhook`**. ⚠️ **O endpoint responde `[]` com HTTP 200** — vazio bem-formado, o *valor plausível* mais barato: a tela diz *"nenhum processo"* e ninguém suspeita da fonte. ⚠️ **Consequência direta na APR-10, entregue horas antes:** a ação de encerrar processo suspenso foi montada no painel de detalhe DESTA aba, então o alcance está provado no backend (401/403/200, 54→53 suspensas ao vivo) e o operador **não chega ao botão**, porque a lista está vazia. A ficha da APR-10 mediu o alcance e não mediu a LISTA — o gate dela lê o código da tela, não o que a tela recebe. Trabalho: reapontar a aba (e o `MonitorTab`) para a fonte de sessões da analytics-api, filtrando `channel_type=webhook`, como a aba Sessions já faz; ⚠️ **Achado junto, e já corrigido:** o cabeçalho de `FlowMonitorPage.tsx` afirmava *"sessions-only scope (Processos moved to `/flow/processos`)"* — as duas metades falsas (o scope está lá; a rota não existe no `routes.tsx`), mandando procurar a tela no lugar errado. Trabalho: reapontar a fonte, e decidir o destino do `/v1/workflow/trigger` e da tabela (a MOD-11 já removeu as rotas de webhook do mesmo serviço por estarem vazias) | `aberto` | medido 2026-09-09 |
