...
| AUT-50 | **`agent.*` é ESCREVÍVEL e ILEGÍVEL: o operador grava a tag, recebe 200, e ela nunca volta.** Medido ao vivo em 2026-09-09, fechando a AUT-49: gravei `agent.aut49_prova` pelo `ManualTagForm` → **HTTP 200**, **14 campos no Redis**, e o `/api/supervisor_state` serve **13** — com `context_withheld: {total: 13, by_rule: [], by_pool_scope: []}`, ou seja a tag **nem é contada como retida**: ela está fora do conjunto de origem. As duas listas do mesmo arquivo não são a mesma: escrita `OPERATOR_WRITABLE_NS = ["agent", "service"]` (`server.ts`, no `/api/inject-context`) × leitura `DEFAULT_OPERATOR_NAMESPACES = ["service", "session"]` (`server.ts:1107`). **`agent` só está na de escrita; `session` só na de leitura.** Vale inclusive para o `admin` — não é masking por papel. ⚠️ É a MESMA família que o comentário logo acima do `DEFAULT_OPERATOR_NAMESPACES` já descreve para o hint da tela (*"promete MAIS visibilidade do que existe... e ninguém abre chamado sobre um campo que não apareceu"*) — aqui quem promete é o próprio formulário, ao aceitar o namespace. Decidir: **(a)** alinhar as duas listas (e então qual vence — `agent` passa a ser legível, ou deixa de ser escrevível?); **(b)** o formulário recusar no cliente o que a leitura não devolve, com o motivo; **(c)** o endpoint responder 4xx nomeando *"escreve mas você não verá"*. ⚠️ O gate tem de cobrir as duas listas juntas — foi a separação delas que produziu o defeito | `aberto` | medido 2026-09-09 |
