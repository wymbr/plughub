...
| ALW-15 | **Eco ao CLIENTE: mandar o MODO, não o tipo.** O `masked_types` já viaja no `interaction.request`, mas o cliente do canal **não pode resolvê-lo** — o catálogo é config da plataforma e o cliente é a webapp do tenant. Para essa ponta o campo honesto é o `EchoMode` já resolvido, e resolver no ENVIO é correto ali (instrução de render é efêmera; congelar política só seria errado em registro durável). Exige um leitor do catálogo no `notification_send`. ⚠️ É **advisory por natureza**: o cliente digitou o valor e já o conhece — a plataforma DECLARA, o cliente obedece; não é fronteira de confidencialidade como a ponta do operador (essa fechou na ALW-10). Cliente de referência: `infra/demo/web/webchat-test.html` | `aberto` | `docs/adr/adr-contextstore-allowlist.md` |
