...
**PID-01, PID-02, PID-03, PID-07, PID-06, PID-13, PID-15, PID-08, PID-04 e PID-17 fechadas** — a pendência com exigência só é liberada e só é retomada por quem provou nesta sessão (ou por principal humano verificado), em todas as portas. **PID-10 depende de IDN-07.** A migração dos dois intakes (PID-04) vem
...
| PID-16 | **Promote em lote sobre slots, com rollback por pool.** A PID-08 aposentou o deploy em lote que gravava "implantado em X" sem tocar o slot; o que sobra é a necessidade real que ele fingia cobrir: um release de skill de plataforma custa um `set-next` + `promote` por porta, e um bug do runner atinge todas ao mesmo tempo. Um lote sobre `PoolSkillSlot` (mesmo snapshot em N pools, cada promote com os portões de capacidade/perfil/piso, e um `SkillDeployment` por pool) com rollback por pool. **⚠️ Gatilho ATINGIDO na PID-17 (2026-09-14):** `skill_intake_runner_v1` roda em duas portas (`limite_ia`, `portabilidade_ia`); o release da PID-17 custou um `set-next` + `promote` por pool, feito à mão. Segue aberta, agora com população | `aberto` | ADR §4 · PID-08 |
