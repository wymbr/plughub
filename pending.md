...
| RSM-01 | **O prazo do resume TOKEN e o do `session:{id}:meta` vêm de fontes que ninguém conciliou.** O token vive `timeout_hours*3600 + 3600` (48 h no default); o meta — de onde saem `tenant_id`/`agent_type_id` — vive 24 h. Quando `timeout_hours` passa de 23, o token sobrevive ao meta: a retomada é ACEITA e falha por tenant desconhecido, que é a recusa `tenant_unknown` do arco P2. ⚠️ **Exposição e dano são dois números, e hoje só há exposição:** medido em 2026-09-07 pelo `probe_resume_outlives_meta`, **1** token exposto com **24 h descobertas** e **zero** órfãos (nenhum meta já morto sob token vivo). O truncamento do meta, que era a causa antiga, está consertado — `probe_meta_ttl_bridge_off` sai verde e a regra do MAIOR TTL é respeitada; o que falta é o caminho do `suspend` informar ao escritor do meta o horizonte do token. **Gatilho:** antes de qualquer fluxo com `timeout_hours` > 23 em produção, ou no primeiro órfão que o gate contar | `aberto` | medido 2026-09-07, `CHANGELOG.md` § 2026-09-07 (14) |
