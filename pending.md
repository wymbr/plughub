...
| ALW-17 | **DECIDIDO pelo dono em 2026-09-12: o campo SAI.** Desde a MSK-03 (2026-09-10) os tres modos de `echo_to_operator` produzem byte a byte a mesma saida, e a inercia esta fixada como fato (`test_none_e_masked_produzem_a_mesma_saida`). A alternativa que ficou para tras — dar a `masked` um render PARCIAL vindo do tipo (`***4444`) — foi recusada: devolveria a distincao ao custo de mudar o que o operador VE, e isso e decisao de confidencialidade que ninguem pediu. Motivo declarado da escolha: entre *marcar cada caso* e *remover a alternativa*, a segunda nao depende da memoria de ninguem. **Trabalho:** tirar `echo_to_operator` do catalogo e da `MaskingPage`, remover a consulta ao config-api por submissao mascarada (`main.py`), o parametro `echo_policy` e o `masking_types.resolve_echo_operator`; o teste de inercia sai junto, porque vigia tres modos que deixam de existir. **Nada muda para o operador** — ele ja le sempre o mascarado; o que muda e a tela parar de oferecer uma escolha que nao existe | `aberto` | `CHANGELOG.md` § 2026-09-12 (4) |
...
| VOZ-06 | **Gravacao por segmento, com aviso e opt-out** (V-F3), no AttachmentStore com classe de retencao. ⚠️ **DESTRAVADA em 2026-09-12: o dono fixou o default de `call_recording` em 30 DIAS** — que e exatamente o que roda hoje (`attachment_expiry_days: int = 30`), entao a escolha **nao muda comportamento nenhum** e o entregavel passa a ser o MECANISMO: uma entrada por classe no namespace `storage` da config-api, com tela, e o numero saindo do env (hoje viola tres invariantes de configuracao de uma vez — env para regra de negocio, um numero unico para todas as classes, e sem superficie de UI). O conflito doc×doc de **5 anos × 30 dias** morre junto: os 5 anos nunca tiveram implementacao, porque a gravacao ja escreve no AttachmentStore (`voice.py:410-416`) e o ciclo unico a apagaria aos 30. Tenant regulado sobe o numero na tela, por escolha registrada | `aberto` | ADR §8 · A5 |
...
| PUL-06 | **Fechar a sessão cancela o TOKEN e deixa o ITEM — o ledger passa a afirmar uma pendência que ninguém pode cumprir.** `cancel_pending_resumes` (consumidor de `session.closed`, RET-03) apaga o `resume_token`, mas `work_task`, `claim_record` e o `pipeline_state` suspenso ficam. Medido em 2026-09-11: `5120fe90` e `4841c60d` fechados 10 s e 68 s depois do claim (`closed=agent_done`), token fora de `resume_tokens`, e os dois exibidos em `/monitor/work-items` como **"Being filled in"** com prazo de ~21 h — valor plausível sobre fato morto. ✅ **A hipótese de que o prazo nunca dispararia foi MEDIDA e REFUTADA (2026-09-12).** Ela dizia que o scanner varre `*:resume_tokens` e, sem o token, o item evaporaria pelo TTL sem `acw_expired`. No prazo real dos dois (14:20:26 e 14:27:27 UTC) o gateway logou *"timeout scanner: endereco REIDRATADO do registro duravel"* → `expiring token` → `webhook resume`, o routing gravou `work_task_expire … reason=acw_expired` e o bridge fechou o contato. O scanner NÃO depende só do hash: ele reidrata o endereço a partir do próprio `work_task`. Lição de método: a hipótese nasceu de ler UMA função (`_scan_expired_resume_tokens`) e não o caminho inteiro — código lido não é comportamento medido, e a ficha dizia "provavelmente" sem ter provado. ⚠️ **O dano real, portanto, é a JANELA**, não a evaporação: por ~24 h o ledger afirma "Being filled in" sobre trabalho morto, o *Close* do admin responde 404, e o item ocupa a lista de pendências de quem não pode fazer nada. ⚠️ **Achado lateral, medido no mesmo fechamento:** *"transição: resume_meta AUSENTE no fechamento … a linha fecha sem motivo nem início reais"* — o `cancel_pending_resumes` apaga também `_resume_meta_key`, então a linha de transição do encerramento sai sem causa nem início. ⚠️ **E o item fica INENCERRÁVEL pela única ação oficial — medido ao vivo no mesmo dia:** o *Close* de `/monitor/work-items` chama `POST /api/work_queue/expire`, que lê o `resume_token` do `work_task` e encerra RETOMANDO por ele (`decision: timeout`); token cancelado ⇒ o gateway responde **404** e a tela mostra *"Could not close the item (HTTP 404)"*. No mesmo clique o `0596f383` (token vivo) fechou e saiu da lista; `5120fe90` e `4841c60d` ficaram. O `work_task_expire` do routing NÃO precisa do token (resolve o dono pela lease → registro → semáforo), então o encerramento sem token já existe uma camada abaixo — falta a rota não depender da retomada quando o token morreu. Dano de capacidade medido: **zero** (o semáforo do operador não retém os dois); o dano é o ledger mentindo e a posse gravada. **Conserto:** a MESMA casa que cancela o token encerra o item (`work_task_expire` com causa própria), pela razão que o docstring do RET-03 já dá — um gancho por lugar, e o esquecido reabre o buraco. **Gatilho:** já ocorreu | `aberto` | medido 2026-09-11 (wrap-ups `0596f383` · `5120fe90` · `4841c60d` do operator@) |
...
| PID-12 | **DECIDIDO pelo dono em 2026-09-12: a IMPORTACAO COM CREDENCIAL DE ADMIN e a unica porta que carimba `authoritative`.** A aba Cliente passa a carimbar `operator` (IDN-08) e o resolver nunca carimba. A confianca passa a ser do PROCESSO de importacao, e isso fica dito em vez de presumido. As duas alternativas continuam nomeadas no §7 do ADR e **nao** foram eleitas: MCP de dominio no CRM do tenant (o `identity_verify` da spec) e federacao de login — a primeira e o alvo arquitetural, mas deixaria o OTP esperando a primeira integracao real existir. **Trabalho:** a porta de importacao grava procedencia, e nenhum outro escritor pode; hoje nenhum dos cinco escritores carimba coisa alguma | `aberto` | ADR D13, §7 |
