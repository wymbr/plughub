...
| VOZ-06 | **Gravacao por segmento, com aviso e opt-out** (V-F3), no AttachmentStore com classe de retencao. ⚠️ **A premissa da destrava de 2026-09-12 foi MEDIDA em 2026-09-13 e caiu em tres pontos.** (1) *"o numero vem do env"* — falso: `resolve_attachment_expiry_days` le `webchat.attachment_expiry_days` da config-api, com tela (`WebChatConfigPage`); o env e so o fallback. (2) *"a gravacao ja escreve no store"* — falso: `voice.py` chamava `self._store.store()`, metodo que o `AttachmentStore` nunca teve, e **nenhuma gravacao de voz jamais foi armazenada**; `whatsapp.py` e `email.py` tinham o mesmo defeito de outra forma (`commit` sem `tenant_id`). Os tres foram consertados na VOZ-08. (3) *"o ciclo unico a apagaria aos 30"* — falso: **nao existe ciclo nenhum** (VOZ-07). Populacao do store: 6 linhas, todas `image/jpeg`. **O que sobra desta ficha e o MECANISMO por classe** (`call_recording` × `webchat_attachment` × …), e ele so tem objeto quando houver gravacao REAL: politica por classe contra populacao zero e o erro que a decisao #4 firmou. O default de 30 dias do dono continua valendo e ja e o que a chave unica carimba | `bloqueado` — ate existir gravacao REAL no store: o primeiro caminho capaz e o egress WebRTC (VOZ-01 + VOZ-04); e a VOZ-07 antes, porque classe de retencao sem expurgo e numero que ninguem aplica | ADR §8 · A5 · `CHANGELOG.md` § 2026-09-13 (1) |
| VOZ-07 | **O expurgo do AttachmentStore NAO EXISTE — `expires_at` e carimbado e nunca aplicado.** O docstring de `attachment_store.py`, o `adr-webchat-channel.md` e o `CLAUDE.md` (§ WebChat) descrevem um cron de dois estagios (soft-delete horario `WHERE expires_at < NOW()`, delete fisico diario +24 h). Medido em 2026-09-13: `soft_expire` tem **zero chamadores**, nao ha SQL com `expires_at <` sobre `session_attachments` em pacote nenhum, e o serving so recusa por `deleted_at` — que ninguem escreve. Ou seja: **anexo nenhum expira**, e o `attachment_expiry_days` editavel na tela e numero que so decora a linha. ⚠️ **Exposicao × dano:** 6 linhas, 0 com `deleted_at`, 0 vencidas hoje — a primeira passa do `expires_at` em **2026-09-20**, e a partir dai o dano e LGPD (minimizacao prometida e nao cumprida), crescendo com o trafego. Conserto: o job dos dois estagios, com contador do que expurgou e log do que falhou, e gate que CONTE linha vencida sem `deleted_at` | `aberto` | `attachment_store.py:21-23` · `upload_router.py:134` · `CHANGELOG.md` § 2026-09-13 (1) |
