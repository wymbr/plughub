...
| AUT-29 | **A recusa por escopo não é viável enquanto o admin não for `unrestricted`.** Medido em 2026-08-31: `admin@plughub.local` carrega uma LISTA de 36 pools, e `accessible_pools is None` só acontece para principal de SERVIÇO — então *"recuse o chamador escopado"* devolve 403 ao administrador de verdade. Foi por isso que a F-C da AUT-01 virou dívida declarada em vez de recusa. Depende da AUT-15 (superfície do `unrestricted`) | `bloqueado` por AUT-15 | `CLAUDE.md` § Security |
