...
| AUT-44 | **Quem CRIA um usuario nao passa a administra-lo.** Resíduo NOMEADO da AUT-39 (2026-09-09): o eixo de administração passou a ser o organograma (`agent_groups`), e a criação não põe o criado em grupo nenhum — então um supervisor cria a pessoa e, no instante seguinte, recebe **403** para editá-la. *Criar e não administrar é incoerente*, e apareceu ao consertar o probe do split, onde o `useradmin@` cria o alvo no S1 e o edita no S3. Três saídas, e nenhuma é óbvia: **(a)** a criação passa a declarar o GRUPO (parâmetro novo, e o criador tem de supervisioná-lo); **(b)** proveniência — quem criou administra —, que é um segundo eixo e reabre parte do lateral; **(c)** criação por não-admin deixa de existir. ⚠️ Hoje o caminho que funciona é o admin criar e vincular; a incoerência não bloqueia ninguém porque a tela de Grupos existe | `aberto` | AUT-39, 2026-09-09 |
...
| AUT-47 | **`GET /api/supervisor_state/{id}` serve QUALQUER sessão a quem tem `agent_assist.atender` — sem recorte de pool.** Medido em 2026-09-09, de passagem na AUT-46: `operator@`, cujos pools são `formfill_demo`/`retencao_humano`/`aprovacao_credito`, leu **200** o estado de uma sessão de `aprovacao_deploy` (duas vezes: uma fechada, uma viva recém-criada). O gate do endpoint é só CAPACIDADE (`requireJwtGrant(..., "agent_assist", "atender")`, `mcp-server-plughub/src/server.ts:1526`) — não há `authorize_session_scope`. É a porta que sobrou da família fechada em 2026-08-30 para a analytics-api (*"duas portas para o mesmo dado e só uma trancada"*), agora noutro serviço: o conteúdo do contato lá recorta por pool desde então, e aqui não. ⚠️ **Exposição ≠ dano, e os dois números estão medidos:** o corpo servido **nao** trouxe o `resume_token` — medido nas duas tentativas; o MOTIVO nao foi apurado (mascaramento por pool? campo nao exposto?), e afirmar a causa sem medir seria inventar a metade que falta — quem fechar esta ficha mede isso primeiro, porque dela depende se o achado e *estado da sessao vaza* ou *a credencial da tarefa tambem vaza*. Entao a cadeia que a AUT-46 fechou não passa por aqui — o que passa é o ESTADO da sessão (contexto do cliente, sentimento, turnos). Decidir: replicar `authorize_session_scope` neste endpoint, ou declarar a isenção com o motivo. Não tocado na AUT-46 porque é outro serviço e outro eixo | `aberto` | medido 2026-09-09 |
