...
| CAP-13 | **Decidir a topologia da porta 3100 — e a decisão muda conforme o deploy seja de rede COMPARTILHADA ou DISTRIBUÍDO.** Medido 2026-09-01: `"3100:3100"` publica em `0.0.0.0` (`demo` e `full`; nenhum compose restringe). **Nenhum contêiner precisa dela** — todos alcançam por nome de serviço (`mcp-server-plughub:3100`), inclusive o nginx do platform-ui e o `rules-engine` (cujo default `localhost` é sobrescrito por env). Os consumidores reais da porta publicada são **de host, e de desenvolvimento**: o proxy do `vite.config.ts` (dev) e a suíte e2e/probes (`MCP_SERVER_URL`). Já existe disciplina de não-publicar: 7 serviços HTTP do demo não publicam (`routing-engine`, `skill-flow-service`, `session-replayer`, `usage-aggregator`, `skill-flow-worker`, `orchestrator-bridge`, `e2e-runner`). ⚠️ **Se o alvo for deploy distribuído, a porta é NECESSÁRIA e a resposta deixa de ser topologia** — vira autenticação de transporte, e o padrão já existe nesta casa (`MCP_INTERNAL_SERVICE_TOKEN`, usado pelas duas `/internal/*`, falhando FECHADO). ⚠️ **Ao remover a porta, NÃO mapear `/sse` e `/messages` no nginx** — mapear tudo move a exposição da 3100 para a 5174 e tira a possibilidade de fechá-la por topologia; o ganho vem justamente de não publicar o transporte. ✅ **Decidido pelo dono em 2026-09-01: passo intermediário `127.0.0.1:3100:3100` ANTES de eliminar.** Ele acaba com a exposição de rede, mantém dev/e2e/probes funcionando (rodam na mesma máquina) e **não exige decidir a questão distribuída ainda** — eliminar de vez fica para depois, junto com mover os probes para dentro da rede. Vem DEPOIS da CAP-12 | `aberto` — depois da CAP-12 | `CHANGELOG.md` 2026-09-01, `infra/test/probe_mcp_rest_surface.sh` |
| CAP-12 | **Nove rotas `/api/*` do mcp-server SEM credencial, e a borda as PUBLICA.** É a superfície realmente exposta — medida ao vivo pela borda (5174) com controle positivo (`/api/instances` e `/api/agent-state` → **401**; as abertas → **200**). São elas: `GET conversation_history/:sessionId` · `GET copilot_state/:sessionId` · `GET supervisor_capabilities/:sessionId` · `GET work_queue/list` · `GET work_queue/pending` · `POST agent_done/:sessionId` · `POST menu_submit/:sessionId` · `POST work_queue/claim/:sessionId` · `POST work_queue/release/:sessionId`. ⚠️ **Exposição e dano são dois números** (D14.1): com a stack ociosa as duas de fila devolveram **vazio**, mas `conversation_history` lê `session:{id}:messages`, que tem **três produtores** (`session.ts:529`, `orchestrator-bridge/main.py:7340`, `channel-gateway/session_registry.py:144`) — *"zero agora"* não é *"inócuo"*, e a rota devolve `content` de mensagem sem filtro nenhum. Agrava: `menu_submit` submete **no lugar do cliente** e `agent_done` **fecha** a sessão; `work_queue/claim`+`release` mexem em fila de trabalho humano. ⚠️ **Ordem:** o irmão gateado do `conversation_history` existe (`analytics /v1/transcript/sessions/{id}`), então fechar esta não tira função de ninguém — é o caso mais barato e o de maior dano. Toda mudança move a linha no `probe_mcp_rest_surface.sh`, que reprova **nos dois sentidos**. ✅ **Ordem decidida pelo dono em 2026-09-01: esta ANTES da CAP-13**, e o motivo é medição, não preferência — estas 9 estão expostas pela **5174** e **continuam expostas com a 3100 fechada**, então topologia não as alcança. Começar por `conversation_history` | `aberto` — próximo da fila | `CHANGELOG.md` 2026-09-01, `infra/test/probe_mcp_rest_surface.sh` |
---
