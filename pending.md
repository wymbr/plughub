...
**PID-01, PID-02, PID-03, PID-07, PID-06 e PID-13 fechadas** — a pendência com exigência só é liberada e só é retomada por quem provou nesta sessão (ou por principal humano verificado), em todas as portas. **PID-10 depende de IDN-07.** A migração dos dois intakes (PID-04) vem
...
| PID-15 | **O cliente não consegue cancelar uma pendência de APROVAÇÃO pelo intake.** Medido em 2026-09-14 na PID-13, e anterior a ela: o `cancelar_solicitacao` do `skill_limite_entrada_v1` chama `workflow_resume` sobre o token de `limite_processo.aprovar`, e a rota interna do gateway exige Bearer humano para toda tarefa que declara capacidade (AUT-46, 2026-09-09: `credential required for approvals.decide tasks`) — o MCP não tem Bearer, então o cancelamento toma 401 e o intake cai no `avisar_erro_retomada`. Nem o atestado de evidência da PID-13 passa, porque o portão do AUT-46 roda antes. Decidir se o cliente comprovado pode encerrar uma tarefa de aprovação (e com qual `decision`), sem reabrir a decisão do aprovador por essa porta | `aberto` | `main.py` `webhook_resume` · AUT-46 · PID-13 |
