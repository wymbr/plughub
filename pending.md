...
| id | tarefa | estado | evidência |
|---|---|---|---|
| CNS-22 | **Seis pools ainda executam SNAPSHOT anterior à CNS-11, lendo alias que o produtor pode já não escrever.** O bridge executa o snapshot do slot `current`, não o YAML — e a CNS-11 migrou YAML e código sem re-promover ninguém. Censo de 2026-09-03: **8 pools** com snapshot de 2026-08-10/12 lendo `@ctx.session.*` cuja canônica é `core.*`. Dois foram re-promovidos (`dialog_runner`, `nps_ia`) porque bloqueavam o demo; **restam seis**: `wrapup_detached_ia` (`session.origin_session_id`, `surveyed_agent_key`, `surveyed_segment_id`) · `outbound_survey_worker` (`survey_grain`) · `portabilidade_confirmacao` e `limite_retorno` (`delegate_resume_token`, `workflow_resume_token`) · `survey_multi_ia` (`pool.id`) · `copilot_sac` (`sentimento.categoria`). ⚠️ **Re-promover em bloco é perigoso**: cada leitura só passa a funcionar se o PRODUTOR daquela tag também escrever a canônica. Medir produtor por produtor antes — foi exatamente a metade que faltou na CNS-11 e o que quebrou o OTP na CNS-19. ⚠️ Falta também o mecanismo: nada reprova snapshot que lê alias — sem ele isto volta | `aberto` | `infra/scripts/deploy_skill_to_slot.sh` · censo em `CHANGELOG.md` § CNS-21/22 |
