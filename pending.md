...
| MOD-08 | **G1b — os presets passam a ser DESENHADOS, com gate do par `⊆`.** Pré-requisito de a MOD-02 entregar contratação: o guard de rank só funciona enquanto valer *"quem contrata ⊇ quem é contratado"*, e medido em 2026-09-08 **não vale** — o guard sobre os `role_defaults` produz a DIAGONAL (cada papel só cria um clone de si). `supervisor → operator` é bloqueado por 3 campos (`evaluation.contestar`, `approvals.decide`, `approvals.operacao`), e o diagnóstico é de **dado**: os presets são engenharia reversa do `seed_auth.py`, e o `arc7-auth.md` já declarava `developer`/`business` como *"declarações mínimas que precisam de decisão"*. Entrega: presets revistos + **gate** que asserta `preset(contratado) ⊆ preset(contratante)` para cada par legítimo — sem ele a próxima edição de preset quebra a contratação em silêncio, e o sintoma não parece "preset errado", parece "a tela não deixa". Inclui `developer` → **`devops`** (o preset atual é de *autor de fluxo*, papel que os DialogForms tornaram obsoleto; custo medido: **1 linha**, `admin@`, que também tem `admin`). ⚠️ **`agente` NÃO é criado** — identificador é inglês por regra da casa, e `operator` já É o agente humano | `aberto` | ADR § Emenda 2026-09-08 E3 |
