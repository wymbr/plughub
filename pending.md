...
| id | tarefa | status | referencia |
|---|---|---|---|
| CTX-06 | **F5 — `$.pipeline_state.*`. O censo estrutural corrigiu o numero E achou o bloqueio.** 228 por regex -> **142** estruturais -> **35** ao cliente (30 chaves); das 35, 16 sao texto de DialogForm ja renderizado. ⚠️ **Aplicar hoje CORROMPERIA valor**: `pendencia.context.*` ja nasce mascarado (o `_build_pending_preview` o escreve, e o skill documenta que o cartao vem `***1234` e que envolve-lo daria `*****4444**`). Olhando so o valor, `***4444` e o numero cru sao indistinguiveis quanto a *ja passou por mascara?* — entao o carimbo de proveniencia deixou de ser refino e virou **requisito de correcao**. Detectar *parece mascarado* esta PROIBIDO pela §D8 | `bloqueado` (por CTX-10) | ADR §D7, §D11 |
...
| CTX-10 | **A decisao que destrava a F5: valor DERIVADO herda o tipo de qual?** Uma concatenacao de dois campos, ou um resumo gerado por LLM, nao tem um tipo — e sao eles a maior parte das chaves `roteiro.render.*` que chegam ao cliente. Sem regra para o caso composto o carimbo de proveniencia nao fecha, e sem carimbo nao ha aplicacao segura (§D11). ⚠️ A saida obvia — detectar se *parece mascarado* — esta PROIBIDA pela §D8: seria detector onde o arco exige declaracao, o defeito que a V2b da allowlist fechou | `aberto` | ADR §D11 |
