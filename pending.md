...
| IDN-08 | **A aba Cliente do Console escreve no cadastro como se fosse fonte autoritativa.** `ClienteTab.tsx:118` cria cliente (`/identity/resolve` com `provision: true`) e grava `attributes` por desenho; operador não é fonte autoritativa. Ou ela carimba procedência `operator`, ou contamina em silêncio a base que o OTP vai confiar. ⚠️ **Medido em 2026-09-13 (PID-12): a aba NÃO escreve âncora durável.** `/identity/resolve` com `provision` grava o prospect e o índice só no Redis, e `/attributes` grava na linha do cliente, não na chave. O risco que esta ficha nomeava — contaminar a base em que o OTP vai confiar — **fechou pela trava da PID-12, independentemente dela**: nenhum escritor além da importação alcança `authoritative`. O que resta é REGISTRAR `operator`, e hoje não há portador durável para o carimbo | `aberto` | `ClienteTab.tsx:118` · ADR §2 (8) |
| IDN-13 | **25 CPFs `possessed` do desafio tautológico continuam abrindo pendências sem prova.** A PID-10 fechou o PRODUTOR (CPF não recebe mais desafio), não o que ele gravou: medido em 2026-09-13, `identity.customer_secondary_keys` tem **25 âncoras `cpf` com `verification_class = possessed`** — todas nascidas do OTP ao CPF, que provava só que se sabia o número digitado — e **1** desses clientes tem pendência indexada viva. Para eles, `pending_workflow_get` com o CPF segue devolvendo as pendências e o `resume_token`, porque o portão lê a classe da âncora. Duas saídas, não excludentes: rebaixar o legado a `claimed` (dado), e o Lookup 1 tratar `possessed` como `claimed` em kind não-entregável (`DELIVERABLE_KINDS`), que remove a alternativa em toda instalação sem depender de migração. O `probe_otp_gate.sh` já informa o número (ramo A, INFO); ao fechar, ele vira veredicto | `aberto` | `probe_otp_gate.sh` ramo A · ADR D8 · CHANGELOG 2026-09-13 (10) |
| IDN-14 | **A normalização de telefone assume DDI presente, e `11 99999-0001` não casa com `+55 11 99999-0001`.** `normalize_anchor` faz `"+" + dígitos`: o mesmo celular digitado com e sem o código do país vira DOIS hashes. Achado na PID-10: o step-up do `skill_limite_entrada_v1` pede o celular cadastrado, e um cliente que omita o `55` tem o desafio recusado (`anchor_not_authoritative`) e é transferido ao SAC — degradação falante, mas evitável. O mesmo corte parte a identidade progressiva entre canais (WhatsApp entrega E.164 com DDI; webchat digitado, não). O prompt pede o código do país como paliativo. Decidir a regra (DDI default por tenant? recusar número sem DDI?) antes de mexer: trocar a normalização muda o hash de toda âncora `phone` já gravada | `aberto` | `normalize.py:49` · `dialog_limite_roteiro` nó `coletar_telefone_cadastrado` |
