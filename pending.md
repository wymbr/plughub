...
| APR-10 | **O Monitor LISTA trabalho suspenso e não deixa agir sobre ele — a lista está numa tela e o botão está noutra.** Achado pela dúvida do dono ao revisar a MOD-11, e é lacuna de SUPERFÍCIE, não de autorização: os dois eixos existem e já são gateados. Medido ao vivo: **212 sessões `webhook` suspensas** (162 + 50 `trigger`) de 688. **Ver** está no Monitor — `MonitorTab` filtra `[all, active, suspended, completed, failed]`, `ListaTab` badgeia `suspended` só para canal `webhook`, e `/analise/sessions` tem a opção no filtro de status. **Agir** está no Console, por sessão: encerrar é `POST /api/force-complete/{sid}` (`agent_assist.supervisionar`, via `OrchestrationTab` e `ParticipantFilterBar`) e decidir é o `ApprovalPanel` do `AgentAssistPage` (`approvals.decide`). Entre as duas telas não há caminho: o Monitor tem **zero ação por linha** — as 4 chamadas de `/instances/{id}/cancel` foram removidas em 2026-08-07 e nada as substituiu ali, e o que restou é um botão que **copia o `resume_token` para a área de transferência** (`MonitorTab.tsx:791`). ⚠️ **Não reabrir os campos do módulo `workflows` para isto** (removidos na MOD-11): eles não dariam botão a ninguém — dariam um segundo portão para uma ação que já tem dono. O trabalho é ação por linha na lista de suspensos, reusando `agent_assist.supervisionar` e `approvals.decide`, mais um caminho da linha para a sessão no Console | `aberto` | medido 2026-09-08 (MOD-11) |
...
| RET-11 | **A imposição do prazo de suspend lê o cache, e o prazo durável existe e ninguém o consulta.** Medido em 2026-09-09, a partir da pergunta do dono sobre comandos manuais de processo: **212 sessões `suspended` com `closed_at` NULL**, a mais antiga de 10/08, contra **zero** tokens em `*:resume_tokens`. Dessas, **160 já venceram o prazo e ninguém agiu**. A cadeia: o `suspend` grava token+prazo no Redis **e** uma linha durável em `analytics.session_transitions` (680 linhas, **todas com `resume_expires_at`**); o deploy roda `redis-server --save ""` com `appendonly no`, então qualquer restart apaga os tokens; e `_scan_expired_resume_tokens` varre **`*:resume_tokens` no Redis** — sem token, o scanner não enxerga nada e o prazo durável nunca é lido por quem poderia agir. ⚠️ **O conserto NÃO é ligar persistência**: o registro durável já existe e já é escrito. É a IMPOSIÇÃO que lê o store errado. ⚠️ **A prescrição inicial desta ficha (*"o scanner lê `session_transitions`"*) foi posta em REVISÃO em 2026-09-09, ao medir a RET-12: `session_transitions` é ClickHouse, ou seja **analytics** — `ReplacingMergeTree` alimentado por Kafka, não fila de trabalho. Usá-lo como fonte de decisão operacional torna a analytics load-bearing para operação, e não há precedente disso no repositório (medido: nem `rules-engine` nem `routing-engine` decidem lendo ClickHouse). A DECISÃO PENDENTE é onde mora o registro operacional do suspend — o `channel-gateway` já tem pool de Postgres (`main.py:212`, hoje só para anexos), que é o candidato natural. O resto do desenho segue de pé: o scanner resolve pelo store durável, o Redis vira caminho rápido, e `handle_resume` reidrata a partir do registro — padrão que o `resume_meta` já estabelece. ⚠️ **Nem o `force-complete` alcança esses 160**: o estado 1 dele exige o item no ledger, então sem token e sem pipeline ele responde 404 — botão manual nenhum resolveria | `aberto` | medido 2026-09-09 |
| RET-12 | **MEDIDO em 2026-09-09: `collect` é um SEGUNDO mecanismo de suspensão, e ele não tem prazo durável em lugar nenhum.** A ficha pedia descobrir qual caminho o `limite_entrega` usa. Resposta: `skill_limite_entrega_v1.yaml:61` é um step **`collect`**, não `suspend` — e `collect.ts:38` parqueia com **`collect_token`**, nunca `resume_token`. Como `analytics-api/models.py:438` só emite a linha de `session_transitions` `if payload.get("resume_token")`, um collect parqueado **não deixa registro**. ⚠️ **E é pior que a hipótese original:** o store durável que o collect teria — `workflow.collect_instances` — está **VAZIO (0 linhas)**, porque a rota `/collect/persist` responde **410** desde a Fase D do Arc 19; `workflow.instances` também está em 0; e `plughub_demo.collect_events` no ClickHouse tem **0 linhas** (o produtor era a workflow-api, que deixou de produzir). Ou seja, o prazo de um collect vive **só no Redis**, que não persiste. ⚠️ **Corolário achado junto:** o `run_timeout_scanner` da workflow-api segue ligado no boot (`main.py:106`) varrendo `workflow.instances` — uma tabela vazia por construção. Scanner que roda para sempre sobre zero linhas é a família *"existe ≠ está pronto"*. Trabalho: decidir se o `collect` passa a emitir o registro durável (e em qual store, ver RET-11) ou se o parque por collect é reconduzido ao mecanismo de `suspend` | `aberto` | medido 2026-09-09 |
| RET-13 | **O scanner de prazo é a única task de background do channel-gateway sem `_supervise`.** `main.py:481` — `timeout_scan_task = asyncio.create_task(...)` cru, enquanto `outbound`, `collect-events`, `config-changed`, `registry-pubsub` e `registry-invalidation` passam todas por `_supervise`, que existe desde 2026-08-07 exatamente para *"fazer a MORTE de uma task de background aparecer"*. ⚠️ **Impacto menor do que parece, e isso foi medido**: o laço tem `except Exception` por iteração, então erro de passada não o mata — só uma exceção fora do `try` (ou no startup) o derrubaria, em silêncio. Não é a causa dos 212 (essa é a RET-11), e a ficha existe porque a exceção à regra é que o alarme não vale para a task cujo silêncio custaria mais caro | `aberto` | medido 2026-09-09 (RET-11) |
...
| GAT-04 | **`probe_ts_suites.sh` não enxerga suíte que falha ao COLETAR.** Medido em 2026-09-08, ao rebuildar o `mcp-server-plughub` na AUT-38: `vitest` reportou `Test Files 1 failed | 18 passed` e `Tests 259 passed` — o gate lê a linha de `passed`/`failed` e concluiu *"nenhum vermelho"*. Um arquivo que estoura no import coleta **zero** testes, então não aparece em contagem nenhuma: é a família *"uma lista parece completa por ser uma lista"*, do lado do RUNNER. O caso concreto é `src/tools/navigation.test.ts`, que lê `infra/test/fixtures/verb_vectors.json` — arquivo que existe no repo e **não é copiado para a imagem** (o mesmo *"declara e não instala"* que o gêmeo Python mediu nas 14 imagens). São DOIS consertos e eles são independentes: **(a)** o gate passar a reprovar `Test Files … failed` além de `Tests … failed`; **(b)** a fixture chegar à imagem (ou o teste passar a derivá-la). ⚠️ Fazer só (b) deixa o buraco do instrumento aberto para o próximo arquivo | `aberto` | medido 2026-09-08 (AUT-38) |
