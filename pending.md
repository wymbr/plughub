...
| IDN-11 | **O caminho `durable` do Lookup 1 escolhe um vencedor entre âncoras de clientes DIFERENTES sem declarar ambiguidade.** O caminho quente (Redis) devolve `matched_by="ambiguous"` quando o maior score empata entre dois `customer_id` e deixa o fluxo perguntar; o `_pg_resolve` pega o primeiro estritamente maior e segue — com cpf de um cliente e e-mail de outro no mesmo score, quem vence é a ORDEM das âncoras na chamada. Duas respostas para *"estas âncoras identificam quem?"*, decididas pela temperatura do Redis. Medido ao escrever o teste da IDN-10 (o próprio teste precisou aceitar os dois vencedores). Não consertado junto: a IDN-10 fechou só o reapontamento da perdedora. **Conserto:** a mesma regra de empate nos dois caminhos | `aberto` | `identity/index.py` `_pg_resolve` · `CHANGELOG.md` § 2026-09-13 (7) |
