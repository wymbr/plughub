...
| id | tarefa | estado | evidência |
|---|---|---|---|
| CNS-22 | **Quatro pools ainda executam snapshot anterior à CNS-11, lendo alias cuja canônica é `core.*`** — `wrapup_detached_ia` (`origin_session_id`, `surveyed_agent_key`, `surveyed_segment_id`) · `outbound_survey_worker` (`survey_grain`) · `survey_multi_ia` (`pool.id`) · `copilot_sac` (`sentimento.categoria`). Quatro outros já foram re-promovidos em 2026-09-03 porque bloqueavam o demo. ⚠️ **Re-promover em bloco é perigoso**: cada leitura só passa a funcionar se o PRODUTOR daquela tag também escrever a canônica, e o `wrapup_detached_ia` **funciona hoje** — promovê-lo às cegas troca um verde por um vermelho. Medir produtor por produtor é a metade que faltou na CNS-11. ✅ **O mecanismo já existe** (`infra/test/gate_published_alias_census.sh`): estes quatro são a **dívida declarada** dele, e quitar uma linha é re-promover **e apagar a linha** — o ramo C reprova quem esquecer da segunda parte | `aberto` | `infra/test/gate_published_alias_census.sh` § DIVIDA |
