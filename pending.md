...
| AUT-53 | **O `probe_rank_grant_guard` reprova por medir a regra ANTERIOR à AUT-44 — instrumento velho, nao defeito.** Medido em 2026-09-12 (1a rodada completa da bateria): os TRES cenarios vermelhos sao os POSITIVOS (`P1` cria operator no proprio pool, `N2c` `roles: []`, `T1` template no alcance), e os tres falham com **422** *"criar usuario exige um grupo, e voce nao supervisiona nenhum"* contra um **201** esperado. A AUT-44 (2026-09-11) passou a exigir o grupo de nascimento de quem administra por DELEGACAO — ou seja, o produto mudou e o controle positivo do probe nao foi junto. ⚠️ **O modo de falha e o pior:** um positivo vermelho PARECE protecao, e foi essa aparencia que a § Security ja registrou como armadilha. **Conserto:** o delegado da fixture passa a supervisionar um grupo e os positivos mandam `group_ids`; os negativos (403) nao mudam. **Gatilho:** ja ocorreu — o probe esta vermelho na bateria | `aberto` | medido 2026-09-12 (bateria completa: 132 verdes, 6 vermelhos) |
| AUT-54 | **O seed do demo concede seis campos de um modulo que NAO EXISTE mais.** O `probe_role_preset_on_create` (S6) mede seed x `role_defaults` e acusa, em `admin@plughub.local`, `workflows.{cancelar,journey_read,journey_resume,operacao,visualizar,webhooks}` concedidas pelo seed e ausentes do preset dos papeis `['admin','devops']` — e o modulo `workflows` **saiu do catalogo na MOD-11** (2026-09-08). Nao e divergencia de politica: e grant ORFAO, apontando para campo sem portao vivo. ⚠️ A frase do proprio probe e a regra: *"divergencia nao anotada e como as duas declaracoes viram tres"*. **Conserto:** remover as seis do seed (e do `module_config` dos portadores, se houver — contar antes), ou declarar a excecao no probe com motivo. **Contar primeiro:** quantos usuarios ainda carregam `workflows.*` no grant. **Gatilho:** ja ocorreu | `aberto` | medido 2026-09-12 |
