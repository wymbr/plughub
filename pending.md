...
| CNS-26 | **`core.workflow.dialog_form_version` e escrita sem estar no cadastro do ContextStore.** O `probe_contextstore_cadastro` reprova com 1 tag de PLATAFORMA nao declarada, escrita por `channel-gateway/adapters/webhook.py` (censo: 14 escritas, 1 nao declarada; do lado do tenant, 52 escritas e 0). O runtime **nunca rejeita** (D9.1), entao a escrita passa e o mapa e que fica mentindo — e o mapa e o que decide mascaramento e rota de escopo. ⚠️ O irmao dela (`core.workflow.dialog_form_id`) E declarado, o que torna a ausencia um esquecimento de cadastro, nao uma decisao. **Conserto:** cadastrar a tag (ou parar de escreve-la, se for resto). **Gatilho:** ja ocorreu | `aberto` | medido 2026-09-12 |
...
| AUT-56 | **O `auth-seed` SOBRESCREVE estado vivo dos usuarios demo a cada subida — e zerou o escopo do `admin@`.** Medido em 2026-09-12, na primeira rodada verde do seed depois da AUT-54 (snapshot antes/depois): `admin@plughub.local` foi de **41 pools para `[]`**, que desde a AUT-03 significa NENHUM pool — o admin deixa de ver linha em relatorio escopado. Restaurado pela API no mesmo minuto (`PATCH /auth/users/{id}`, identico ao snapshot). Duas escritas, as duas incondicionais em toda execucao: (a) `set_scope` aplica o `"accessible_pools": []` declarado na entrada do admin; (b) `set_module_config` grava o config COMPLETO dos tres usuarios, desfazendo qualquer concessao feita pela tela de Acesso. ⚠️ **Nao e novo:** os logs das rodadas que morriam (AUT-54) ja mostram o `escopo de admin@: []` aplicado ANTES do `die` — e ainda assim o admin estava com 41, ou seja, algo repoe o escopo depois (nao medido). O que a AUT-54 mudou foi so o seed chegar ao fim. Contradiz duas regras escritas: *Seed-if-absent / DB-owned* (§ Configuration — o seed semeia banco vazio, o banco vence depois) e *"a atribuicao de pool e PÓS-CRIACAO"* (§ Arc 7). O `[]` do admin foi decisao do dono (2026-08-27, contra 22 pools de residuo), entao **a escolha e do dono**: (1) seed-if-absent de verdade — escopo e `module_config` so no NASCIMENTO (`Ja existe` pula os dois); (2) tirar `accessible_pools` da entrada do admin, e a guarda por presenca do `set_scope` ja o pula, mantendo (b); (3) manter e documentar que `up` reseta os demo. **Contar antes:** quem repoe os 41 pools. **Gatilho:** ja ocorreu — qualquer `up` reaplica | `aberto` | medido 2026-09-12 (snapshot `auth.users` antes/depois do `up --no-deps auth-seed`) |
...
**Contador: 5.** Balde declarado, não omissão. Se crescer, é sinal de que está entrando trabalho
...
| GAT-05 | **`up.sh` le seed MORTO como seed CONCLUIDO.** A conferencia de estado depois do `up -d` (`infra/scripts/up.sh:59-61`) exclui os one-shots (`auth-seed|config-seed|dialog-seed|eval-seed|kafka-init|minio-init|pricing-seed|e2e-runner`) **pelo NOME**, sem olhar o exit code — e um one-shot so tem dois desfechos, `Exited (0)` e `Exited (!=0)`, que o filtro trata igual. Custo medido em 2026-09-12 (AUT-54): o `auth-seed` saiu com **exit 1 em toda subida desde a MOD-11** (2026-09-08), morrendo no admin sem semear `supervisor@`/`operator@`, e o script terminava em *"✅ Stack no ar"*. ⚠️ Mesma familia do comentario da propria secao (*"exit 0 do `up` nao e o veredicto"*): a regra foi escrita para servico long-running e o one-shot ficou fora dela por construcao. **Conserto:** one-shot passa a exigir `Exited (0)` — `exited` com codigo != 0 reprova nomeando o servico e o comando de log; `running` de one-shot (ainda rodando) e INCONCLUSIVO, nao verde. Conferir antes que os oito saem 0 hoje, senao o conserto nasce vermelho por dado velho. **Gatilho:** ja ocorreu | `aberto` | `CHANGELOG.md` § 2026-09-12 (11) |
