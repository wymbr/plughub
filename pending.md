...
| CAP-10 | **Decidir a POLÍTICA de credencial das tools da plataforma — 48 dívidas declaradas.** O censo fechou (CAP-09) e trancou o estado; o que falta é a decisão, que não é derivável do número. Estado travado: `ok=23 · isento=1 · divida=48` de **72**. As dívidas não são homogêneas — `transcript_get` **serve transcrição de contato** e tem irmão gateado (`analytics /v1/transcript/sessions/{id}`), que é o padrão *"duas portas para o mesmo dado e só uma trancada"* de 2026-08-28; `pool_promote`/`skill_deploy` **promovem deploy**; `context_set` **escreve** no ContextStore; `work_task_claim` reclama item de fila humana; `mailing_unsubscribe` grava opt-out. Outras (`calendar_*`, `system_availability_check`) podem ser isenção legítima. ⚠️ **A decisão precisa da topologia, que não foi medida.** A porta 3100 está em `0.0.0.0` **neste compose de demo** e borda é fato de REDE, não de código — não existe aqui o equivalente do `probe_edge_surface.sh` que diga o que o deploy publica. **Decidir por gravidade percebida sem o alcance medido é a D14.1 ao contrário** (lá contou-se exposição e chamou-se de dano; aqui seria decidir dano sem contar exposição). ⚠️ **Ordem:** medir a superfície do mcp-server ANTES de escolher o que fechar; e todo fechamento move a linha de `divida` para `ok` no `probe_mcp_tool_guard_census.sh`, que é o que impede a política de existir só em prosa | `bloqueado` — falta medir a superfície de rede do mcp-server | `CHANGELOG.md` 2026-09-01, `infra/test/probe_mcp_tool_guard_census.sh` |
---
