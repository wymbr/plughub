...
| id | tarefa | estado | evidência |
|---|---|---|---|
| CAP-05 | **A borda `invoke` é INTRANSITÁVEL por construção — e o `CLAUDE.md` a declara "única borda em vigor".** Medido 2026-09-01: `registry-client.ts:64` fixa `permissions: []` → `agent_login` assina `[]` no token → `judgeInvoke([], …)` devolve `permission_denied`. **Toda chamada, sempre.** Não está sem tráfego por acaso: nenhum `external-mcp` configurado (0) e `session_timeline` (destino do `mcp.audit`) com **0 linhas** — então isso nunca doeu, e dói no dia em que alguém configurar o primeiro. ⚠️ O comentário ao lado do `[]` erra DUAS vezes: diz que a config "now lives on the pool's deploy slot" (o `config_json` só tem `form_id` e `max_concurrent_sessions`) e que vazio ⇒ "no MCP tool filtering" (verdade no `inference.py`, **falso** no `invoke`). Defeito independente da decisão A/B abaixo | `aberto` | medido 2026-09-01 |
| CAP-06 | **Dar produtor a `permissions[]` — a corrente tem 5 elos e 4 JÁ EXISTEM.** YAML declara `tools:` (slot `SkillToolSchema {mcp_server,tool,required}` existe, **0 de 44**) → registry guarda (coluna existe) → **`agent_login` assina no JWT ← ÚNICO BURACO** → executor manda o `session_token` (já manda) → MCP decodifica e impõe (`verifySessionTokenSafe` → `judgeInvoke`, já implementado). O conserto é o `getAgentType` mapear o `tools` que ele **já buscou** em vez de devolver `[]`. ⚠️ **NUNCA passar a lista como argumento da chamada** — seria o chamador declarando a própria autorização, o defeito exato do gate removido na CAP-01. Ela viaja ASSINADA ou não viaja. ⚠️ **Ordem é a da AUT-03: POPULAR antes de inverter** — ligar o elo com `tools` 0/44 não muda nada; o dano aparece quando alguém fizer vazio significar "nenhuma tool" no caminho do LLM e todo skill não-populado ficar sem ferramenta, em silêncio. Medir quantas chamadas passariam a ser negadas ANTES de mexer em default | `aberto` — precisa da decisão A/B (ver `TODO.md`) | medido 2026-09-01 |
---
